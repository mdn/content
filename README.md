...
If there is a genuine justification for taking content from other sources and
 putting it on MDN (for example if someone wants to donate an article to MDN
  that they previously published on their blog, or if it makes sense to copy a
   complex reference sheet to MDN that is already available somewhere else), 
   then you should first discuss this with the MDN team:
...
2. If the content is published under a closed license:
   - If you hold the rights to the content, state this, and say that are happy to republish it on MDN
...
3. If the content is published under an open license, say what it is, and link to the license so we can check whether it is compatible with [MDN's license](https://developer.mozilla.org/en-US/docs/MDN/About#using_mdn_web_docs_content).
