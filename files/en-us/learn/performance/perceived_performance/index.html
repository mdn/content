...
<p><span class="seoSummary"><strong><a href="/en-US/docs/Glossary/Perceived_performance">Perceived performance</a></strong> is a subjective measure of website performance, responsiveness, and reliability. In other words, how fast a website seems to the user.</span> It is harder to quantify and measure than the actual speed of operation, but perhaps even more important.</p>
...
<h2 id="performance_metrics">Performance metrics</h2>

<p>There is no single metric or test that can be run on a site to evaluate how a user "feels". However there are a number of metrics that can be "helpful indicators":</p>
...
<dl>
  ...
  <dd>The time until first significant rendering (e.g. of text, foreground or background image, canvas or SVG, etc.). Note that this content is not necessarily useful or meaningful.</dd>
  ...
  <dd>Measures the average time for pixels on the visible screen to be painted.</dd>
...
<p>Making a content request on <code>keydown</code> rather than waiting for <code>keyup</code> can shave 200ms off the perceived load of the content. Adding an interesting but unobtrusive 200ms animation to that <code>keyup</code> event can reduce another 200ms of the perceived load. You're not saving 400ms of time, but the user doesn't feel like they're waiting for content until, well, until they're waiting for content.</p>

<h2 id="conclusion">Conclusion</h2>

<p>By reducing the time that a user has to wait for <em>useful</em> content, and keeping the site responsive and engaging, the users will feel like the site performs better — even the actual time to load resources stays the same.</p>
...
<ul>
 ...
 <li><a href="/en-US/docs/Learn/Performance/javascript_performance">JavaScript performance</a></li>
