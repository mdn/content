...
## CSS rendering and CSSOM optimization
...
Browsers follow a specific rendering path: paint only occurs after layout, which occurs after the render tree is created, which in turn requires both the DOM and the CSSOM trees. To optimize the CSSOM construction:

- Remove unnecessary styles:
- Minify, compress and cache your CSS:
- Split CSS into separate files: CSS not required at page load can be split into additional files to reduce CSS render blocking
- Simplify selectors:
...
Cut down on animations and transitions

## Optimizing element changes with the `will-change` property
...
Selecting fonts that are not too big in file size
Using font files with only the glyphs you need
Limiting the glyphs that are loaded with unicode-range

order of fonts

### Defining font display behavior with the `font-display` property
...
## Limiting styling recalculation scope with the `contain` property
