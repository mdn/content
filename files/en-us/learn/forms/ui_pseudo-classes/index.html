...
<h2 id="What_pseudo-classes_do_we_have_available">What pseudo-classes do we have available?</h2>

<p>The original pseudo-classes available to us (as of <a href="https://www.w3.org/TR/CSS21/selector.html#dynamic-pseudo-classes" rel="external">CSS 2.1</a>) that are relevant to forms are:</p>
...
<p>These basic pseudo-classes should be familiar to you now. More recently, the <a href="https://www.w3.org/TR/css3-selectors/" rel="external">CSS Selector Level 3</a> and <a href="https://dev.w3.org/csswg/css3-ui/#pseudo-classes" rel="external">CSS Basic UI Level 3</a> added more pseudo-classes related to HTML forms that provide several other useful targeting conditions that you can take advantage of. We'll discuss these in more detail in the sections below, but briefly, the main ones we'll be looking at are:</p>
...
<ul>
 ...
 <li>{{cssxref(":checked")}}, {{cssxref(":indeterminate")}}, and {{cssxref(":default")}}: Respectively target checkboxes and radio buttons that are checked, in an indeterminate state (neither checked or not checked), and the default selected option when the page loads (e.g. an <code><a href="/en-US/docs/Web/HTML/Element/input/checkbox">&lt;input type="checkbox"&gt;</a></code> with the <code>checked</code> attribute set, or an <code><a href="/en-US/docs/Web/HTML/Element/option">&lt;option&gt;</a></code> element with the <code>selected</code> attribute set).</li>
</ul>

<p>There are many others too, but the ones listed above are the most obviously useful. Some of the others are aimed at solving very specific niche problems, or not very well supported in browsers yet. The ones listed above all have pretty good browser support, but of course, you should test your form implementations carefully to make sure they work for your target audience.</p>

<div class="notecard note">
<p><strong>Note:</strong> A number of the pseudo-classes discussed here are concerned with styling form controls based on their validation state (is their data valid, or not?) You'll learn much more about setting and controlling validation constraints in our next article — <a href="/en-US/docs/Learn/Forms/Form_validation">Client-side form validation</a> — but for now we'll keep things simple with regards to form validation, so it doesn't confuse things.</p>
</div>

<h2 id="Styling_inputs_based_on_whether_they_are_required_or_not">Styling inputs based on whether they are required or not</h2>
...
<p>{{htmlelement('input')}}, {{htmlelement('select')}}, and {{htmlelement('textarea')}} elements have a <code>required</code> attribute available which, when set, means that you have to fill in that control before the form will successfully submit. For example:</p>
...
<p>You can match these two states using the {{cssxref(':required')}} and {{cssxref(':optional')}} pseudo-classes. For example, if we apply the following CSS to the above HTML:</p>
...
<p>You can also try submitting the form without filling it in, to see the client-side validation error messages browsers give you by default.</p>
...
<div class="notecard note">
<p><strong>Note:</strong> You'll probably not find yourself using the <code>:optional</code> pseudo-class very often. Form controls are optional by default, so you could just do your optional styling by default, and add styles on top for required controls.</p>
</div>

<div class="notecard note">
<p><strong>Note:</strong> If one radio button in a same-named group of radio buttons has the <code>required</code> attribute, all the radio buttons will be invalid until one is selected, but only the one with the attribute assigned will actually match {{cssxref(':required')}}<strong>.</strong></p>
</div>

<h2 id="Using_generated_content_with_pseudo-classes">Using generated content with pseudo-classes</h2>

<p>In previous articles, we've seen the usage of <a href="/en-US/docs/Web/CSS/CSS_Generated_Content">generated content</a>, but we thought now would be a good time to talk about it in a bit more detail.</p>

<p>The idea is that we can use the <code><a href="/en-US/docs/Web/CSS/::before">::before</a></code> and <code><a href="/en-US/docs/Web/CSS/::after">::after</a></code> pseudo-elements along with the <code><a href="/en-US/docs/Web/CSS/content">content</a></code> property to make a chunk of content appear before or after the affected element. The chunk of content is not added to the DOM, so is invisible to screenreaders; it is part of the document's styles. Because it is a pseudo element, it can be targeted with styles in the same way that any actual DOM node can.</p>

<p>This is really useful when you want to add a visual indicator to an element, such as a label or icon, but don't want it to be picked up by assistive technologies. For example, in our <a href="https://mdn.github.io/learning-area/html/forms/styling-examples/radios-styled.html">custom radio buttons example</a>, we use generated content to handle the placement and animation of the inner circle when a radio button is selected:</p>

<pre class="brush: css">input[type="radio"]::before {
  display: block;
  content: " ";
  width: 10px;
  height: 10px;
...
<div class="notecard note">
<p><strong>Note:</strong> This also shows how you can combine a pseudo-class and pseudo-element if required.</p>
</div>
...
<pre class="brush: html">&lt;p&gt;Required fields are labelled with "required".&lt;/p&gt;</pre>
...
<pre class="brush: css">fieldset &gt; div {
  margin-bottom: 20px;
  display: flex;
  flex-flow: row wrap;
}</pre>
...
<h2 id="Styling_controls_based_on_whether_their_data_is_valid">Styling controls based on whether their data is valid</h2>

<p>The other really important, fundamental concept in form validation is whether a form control's data is valid or not (in the case of numerical data, we can also talk about in-range and out-of-range data). Form controls with <a href="/en-US/docs/Web/Guide/HTML/HTML5/Constraint_validation">constraint limitations</a> can be targeted based on these states.</p>

<h3 id="valid_and_invalid">:valid and :invalid</h3>

<p>You can target form controls using the {{cssxref(":valid")}} and {{cssxref(":invalid")}} pseudo-classes. Some points worth bearing in mind:</p>
...
<ul>
 ...
 <li>There are some other ways to make an element matched by <code>:valid</code>/<code>:invalid</code>, as you'll see in the <a href="/en-US/docs/Learn/Forms/Form_validation">Client-side form validation</a> article. But we'll keep things simple for now.</li>
</ul>
...
<h3 id="In-range_and_out-of-range_data">In-range and out-of-range data</h3>
...
<div class="notecard note">
<p><strong>Note:</strong> Numeric input types are <code>date</code>, <code>month</code>, <code>week</code>, <code>time</code>, <code>datetime-local</code>, <code>number</code>, and <code>range</code>.</p>
</div>
...
<p>It is possible for the number input to be both required and out-of-range at the same time, so what happens then? Because the <code>:out-of-range</code> rule appears later in the source code than the <code>:required</code> rule, the <a href="/en-US/docs/Learn/CSS/Building_blocks/Cascade_and_inheritance#understanding_the_cascade">cascade rules</a> come into play, and the out of range message is shown.</p>
...
<div class="notecard note">
<p><strong>Note:</strong> To enter an invalid/out-of-range value, you'll have to actually focus the form and type it in using the keyboard. The spinner buttons won't let you increment/decrement the value outside the allowable range.</p>
</div>

<h2 id="Styling_enabled_and_disabled_inputs_and_read-only_and_read-write">Styling enabled and disabled inputs, and read-only and read-write</h2>
...
<p>Let's have a look at an example that does just this. First of all, the HTML is a simple form containing text inputs, plus a checkbox to toggle disabling the billing address on and off. The billing address fields are disabled by default.</p>
...
<p>It uses the <a href="/en-US/docs/Web/API/HTMLElement/change_event"><code>change</code> event</a> to let the user enable/disable the billing fields, and toggle the styling of the associated labels.</p>
...
<div class="notecard note">
<p><strong>Note:</strong> <code>:enabled</code> and <code>:read-write</code> are two more pseudo-classes that you'll probably rarely use, given that they describe the default states of input elements.</p>
</div>

<h2 id="Radio_and_checkbox_states_—_checked_default_indeterminate">Radio and checkbox states — checked, default, indeterminate</h2>
...
<ul>
 <li>{{cssxref(":default")}}: Matches radios/checkboxes that are checked by default, on page load (i.e. by setting the <code>checked</code> attribute on them) These match the {{cssxref(":default")}} pseudo-class, even if the user unchecks them.</li>
...
<p>The most common use of this is to add a different style onto the checkbox/radiobutton when it is checked, in cases where you've removed the system default styling with <code>appearance: none;</code> and want to build the styles back up yourself. We saw examples of this in the previous article when we talked about <a href="/en-US/docs/Learn/Forms/Advanced_form_styling#using_appearance_none_on_radioscheckboxes">Using <code>appearance: none</code> on radios/checkboxes</a>.</p>
...
<p>Basically, we build the styling for the radio button "inner circle" using the <code>::before</code> pseudo element, but set a <code>scale(0)</code> <code><a href="/en-US/docs/Web/CSS/transform">transform</a></code> on it. We then use a <code><a href="/en-US/docs/Web/CSS/transition">transition</a></code> to make it nicely animate into view when the radio is selected/checked. The advantage of using a transform rather than transitioning <code><a href="/en-US/docs/Web/CSS/width">width</a></code>/<code><a href="/en-US/docs/Web/CSS/height">height</a></code> is that you can use <code><a href="/en-US/docs/Web/CSS/transform-origin">transform-origin</a></code> to make it grow from the center of the circle, rather than having it appear to grow from the circle's corner.</p>

<h3 id="default_and_indeterminate">:default and :indeterminate</h3>

<p>As mentioned above, the {{cssxref(":default")}} pseudo-class matches radios/checkboxes that are checked by default, on page load, even when unchecked. This could be useful for adding an indicator to a list of options to remind the user what the defaults (or starting options) were, in case they want to reset their choices.</p>
...
<p>For the <code>:default</code> example, we've added the <code>checked</code> attribute to the middle radio button input, so it will be selected by default when loaded. We then style this with the following CSS:</p>
...
<pre class="brush: css">input[type="radio"]:indeterminate {
  border: 2px solid red;
...
<div class="notecard note">
<p><strong>Note:</strong> You can find an <a href="/en-US/docs/Web/HTML/Element/input/checkbox#indeterminate_state_checkboxes">interesting example involving <code>indeterminate</code> states</a> on the <code><a href="/en-US/docs/Web/HTML/Element/input/checkbox">&lt;input type="checkbox"&gt;</a></code> reference page.</p>
</div>

<h2 id="More_pseudo-classes">More pseudo-classes</h2>
...
<p>This completes our look at UI pseudo-classes that relate to form inputs. Keep playing with them, and create some fun form styles! Next up, we'll move on to something different — <a href="/en-US/docs/Learn/Forms/Form_validation">client-side form validation</a>.</p>
