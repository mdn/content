...
<p>This aim of this skill test is to assess whether you've understood our <a href="/en-US/docs/Learn/JavaScript/First_steps/Math">Basic math in JavaScript — numbers and operators</a> article.</p>
...
<p>Let's start out by testing your knowledge of basic math operators. You will have to create four numeric values, then add the first two together, then subtract the fourth from the third, then multiply the two secondary results together to get a result of 48. Finally, we need to write a test that proves that this value is an even number.</p>

<p>So, try updating the live code below to recreate the finished example, following these steps:</p>

<ol>
 <li>Create four variables that contain numbers. Call the variables something sensible.</li>
 <li>Add the first two variables together and store the result in another variable.</li>
...
<ol>
 <li>Write a calculation that multiplies <code>result</code> and <code>result2</code> together and assigns the result back to <code>result</code>. This will require assignment shorthand.</li>
