...
<h3>Without a variable</h3>

<p>To understand why this is so useful, let's think about how we'd write this example without using a variable. It would end up looking something like this:</p>
...
<p>{{ EmbedLiveSample('Without_a_variable', '100%', 120) }}</p>
...
<p>To use a variable, you've first got to create it — more accurately, we call this declaring the variable. To do this, we type the keyword <code>var</code> or <code>let</code> followed by the name you want to call your variable:</p>
...
<h2 id="Initializing_a_variable">Initializing a variable</h2>
...
<h2 id="The_difference_between_var_and_let">The difference between var and let</h2>

<p>At this point you may be thinking "why do we need two keywords for defining variables?? Why have <code>var</code> <em>and</em> <code>let</code>?".</p>

<p>The reasons are somewhat historical. Back when JavaScript was first created, there was only <code>var</code>. This works basically fine in most cases, but it has some issues in the way it works — its design can sometimes be confusing or downright annoying. So, <code>let</code> was created in modern versions of JavaScript, a new keyword for creating variables that works somewhat differently to <code>var</code>, fixing its issues in the process.</p>
...
<p>For these reasons and more, we recommend that you use <code>let</code> as much as possible in your code, rather than <code>var</code>. There is no reason to use <code>var</code>, unless you need to support old versions of Internet Explorer with your code (it doesn't support <code>let</code> until version 11; the modern Microsoft Edge browser supports <code>let</code> just fine).</p>

<h2 id="Updating_a_variable">Updating a variable</h2>

<p>Once a variable has been initialized with a value, you can change (or update) that value by giving it a different value. Try entering the following lines into your console:</p>
...
<p>This is using the "less than" operator (<code>&lt;</code>) to test whether 6 is less than 3. As you might expect, it returns <code>false</code>, because 6 is not less than 3! You will learn a lot more about such operators later on in the course.</p>
...
<p>Even if the value contains numbers, it is still a string, so be careful:</p>
...
<pre class="brush: js">const daysInWeek = 7;
const hoursInDay = 24;</pre>

<p><code>const</code> works in exactly the same way as <code>let</code>, except that you can't give a <code>const</code> a new value. In the following example, the second line would throw an error:</p>
