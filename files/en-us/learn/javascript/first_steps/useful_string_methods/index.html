...
<p>Most things are objects in JavaScript. When you create a string, for example by using</p>
...
<p>The length of the string "mozilla" is 7, but because the count starts at 0, the last character's position is 6; using  <code>length-1</code> gets us the last character.</p>
...
<p>{{ EmbedLiveSample('Filtering_greeting_messages', '100%', 600) }}</p>
...
<p>{{ EmbedLiveSample('Fixing_capitalization', '100%', 550) }}</p>
...
<p>{{ EmbedLiveSample('Making_new_strings_from_old_parts', '100%', 600) }}</p>
