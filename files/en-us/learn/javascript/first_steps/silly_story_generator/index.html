...
<ul>
 <li>Go and <a href="https://github.com/mdn/learning-area/blob/master/javascript/introduction-to-js-1/assessment-start/index.html">grab the HTML file</a> for the example, save a local copy of it as <code>index.html</code> in a new directory somewhere on your computer, and do the assessment locally to begin with. This also has the CSS to style the example contained within it.</li>
 <li>Go to the <a href="https://github.com/mdn/learning-area/blob/master/javascript/introduction-to-js-1/assessment-start/raw-text.txt">page containing the raw text</a> and keep this open in a separate browser tab somewhere. You'll need it later.</li>
</ul>

<p>Alternatively, you could use a site like <a class="external external-icon" href="https://jsbin.com/">JSBin</a> or <a class="external external-icon" href="https://glitch.com/">Glitch</a> to do your assessment. You could paste the HTML, CSS and JavaScript into one of these online editors. If the online editor you are using doesn't have a separate JavaScript panel, feel free to put it inline in a <code>&lt;script&gt;</code> element inside the HTML page.</p>
...
<p>You have been provided with some raw HTML/CSS and a few text strings and JavaScript functions; you need to write the necessary JavaScript to turn this into a working program, which does the following:</p>
...
<p><img alt="" src="screen_shot_2018-09-19_at_10.01.38_am.png"></p>

<p>To give you more of an idea, <a href="https://mdn.github.io/learning-area/javascript/introduction-to-js-1/assessment-finished/">have a look at the finished example</a> (no peeking at the source code!)</p>
...
<ol>
 <li>Create a new file called <code>main.js</code>, in the same directory as your <code>index.html</code> file.</li>
 <li>Apply the external JavaScript file to your HTML by inserting a {{htmlelement("script")}} element into your HTML referencing <code>main.js</code>. Put it just before the closing <code>&lt;/body&gt;</code> tag.</li>
</ol>

<p>Initial variables and functions:</p>

<ol>
 <li>In the raw text file, copy all of the code underneath the heading "1. COMPLETE VARIABLE AND FUNCTION DEFINITIONS" and paste it into the top of the <code>main.js</code> file. This gives you three variables that store references to the "Enter custom name" text field (<code>customName</code>), the "Generate random story" button (<code>randomize</code>), and the {{htmlelement("p")}} element at the bottom of the HTML body that the story will be copied into (<code>story</code>), respectively. In addition you've got a function called <code>randomValueFromArray()</code> that takes an array, and returns one of the items stored inside the array at random.</li>
...
<ol>
 ...
 <li>Copy the code found underneath the heading "3. EVENT LISTENER AND PARTIAL FUNCTION DEFINITION" and paste it into the bottom of your <code>main.js</code> file. This:
  <ul>
   ...
   <li>Adds a partially-completed <code>result()</code> function definition to your code. For the remainder of the assessment, you'll be filling in lines inside this function to complete it and make it work properly.</li>
  </ul>
 </li>
</ol>
...
<ol>
 <li>Create a new variable called <code>newStory</code>, and set its value to equal <code>storyText</code>. This is needed so we can create a new random story each time the button is pressed and the function is run. If we made changes directly to <code>storyText</code>, we'd only be able to generate a new story once.</li>
...
<ul>
 ...
 <li><code><a href="/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math/round">Math.round()</a></code> is a built-in JavaScript method that rounds the result of a calculation to the nearest whole number.</li>
...
<ol>
 ...
 <li>Write a post asking for assessment and/or help at the <a href="https://discourse.mozilla.org/c/mdn/learn">MDN Discourse forum Learning category</a>. Your post should include:
  <ul>
   <li>A descriptive title such as "Assessment wanted for Silly story generator".</li>
