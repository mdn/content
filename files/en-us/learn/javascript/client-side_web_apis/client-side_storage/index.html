...
<p>Elsewhere in the MDN learning area we talked about the difference between <a href="/en-US/docs/Learn/Server-side/First_steps/Client-Server_overview#static_sites">static sites</a> and <a href="/en-US/docs/Learn/Server-side/First_steps/Client-Server_overview#dynamic_sites">dynamic sites</a>. Most major modern web sites are dynamic — they store data on the server using some kind of database (server-side storage), then run <a href="/en-US/docs/Learn/Server-side">server-side</a> code to retrieve needed data, insert it into static page templates, and serve the resulting HTML to the client to be displayed by the user's browser.</p>
...
<p>The concept of client-side storage has been around for a long time. Since the early days of the web, sites have used <a href="/en-US/docs/Web/HTTP/Cookies">cookies</a> to store information to personalize user experience on websites. They're the earliest form of client-side storage commonly used on the web.</p>
...
<p>The "easier" features we mentioned above are as follows:</p>
...
<p>You'll learn more about these APIs below.</p>

<h3 id="The_future_Cache_API">The future: Cache API</h3>

<p>Some modern browsers support the new {{domxref("Cache")}} API. This API is designed for storing HTTP responses to specific requests, and is very useful for doing things like storing website assets offline so the site can subsequently be used without a network connection. Cache is usually used in combination with the <a href="/en-US/docs/Web/API/Service_Worker_API">Service Worker API</a>, although it doesn't have to be.</p>

<p>Use of Cache and Service Workers is an advanced topic, and we won't be covering it in great detail in this article, although we will show a simple example in the {{anch("Offline asset storage")}} section below.</p>
...
<ol>
 ...
 <li>
  ...
  <pre class="brush: js">let myName = localStorage.getItem('name');
myName</pre>
...
request.onsuccess = function() {
 ...
 <li>
  <p>Finally for this section, we'll add probably the most important event handler for setting up the database: {{domxref("IDBOpenDBRequest.onupgradeneeded", "request.onupgradeneeded")}}. This handler runs if the database has not already been set up, or if the database is opened with a bigger version number than the existing stored database (when performing an upgrade). Add the following code, below your previous handler:</p>
...
<ol>
 ...
 <li>
  <p>The following snippet is taken from inside <code>fetchVideoFromNetwork()</code> — here we fetch MP4 and WebM versions of the video using two separate {{domxref("fetch()")}} requests. We then use the {{domxref("Response.blob()")}} method to extract each response's body as a blob, giving us an object representation of the videos that can be stored and displayed later on.</p>
...
Promise.all([mp4Blob, webmBlob]).then(function(values) {
 ...
 <li>
  <p>Let's look at <code>storeVideo()</code> first. This is very similar to the pattern you saw in the previous example for adding data to the database — we open a <code>readwrite</code> transaction and get a reference to our <code>videos_os</code> object store, create an object representing the record to add to the database, then add it using {{domxref("IDBObjectStore.add()")}}.</p>
...
<p>The Cache API is another client-side storage mechanism, with a bit of a difference — it is designed to save HTTP responses, and so works very well with service workers.</p>
...
<pre class="brush: js">  // Register service worker to control making site work offline
...
<p>Inside this block we use {{domxref("CacheStorage.match()")}} to check whether a matching request (i.e. matches the URL) can be found in any cache. This promise fulfills with the matching response if a match is found, or <code>undefined</code> if it isn't.</p>

<p>If a match is found, we return it as the custom response. If not, we <a href="/en-US/docs/Web/API/fetch">fetch()</a> the response from the network and return that instead.</p>
