...
<pre class="brush: js">let myText = 'I am a string';
...
<pre class="brush: js">let myArray = ['I', 'love', 'chocolate', 'frogs'];
...
<pre class="brush: js">let myNumber = Math.random();
...
<p>This function draws 100 random circles inside a {{htmlelement("canvas")}} element. Every time we want to do that, we can just invoke the function with this:</p>
...
<pre class="brush: js">const myButton = document.querySelector('button');
...
<pre class="brush: js">const myGreeting = function() {
  alert('hello');
}</pre>
...
<p>Both functions you want to call are called <code>greeting()</code>, but you can only ever access the <code>first.js</code> file's <code>greeting()</code> function (the second one is ignored). In addition, attempting to declare the <code>name</code> variable a second time with the <code>let</code> keyword in the <code>second.js</code> file results in an error. </p>

<div class="note">
<p><strong>Note:</strong> You can see this example <a href="https://mdn.github.io/learning-area/javascript/building-blocks/functions/conflict.html">running live on GitHub</a> (see also the <a href="https://github.com/mdn/learning-area/tree/master/javascript/building-blocks/functions">source code</a>).</p>
</div>
...
<p>The zoo keeper is like the global scope — they have the keys to access every enclosure, to restock food, tend to sick animals, etc.</p>
...
<ol>
 ...
 <li>Open the JavaScript console in your browser developer tools. In the JavaScript console, enter the following command:
  ...
  You should see the value of variable <code>x</code> printed to the browser viewport.</li>
...
b();</pre>
  You should see the <code>y</code> and <code>z</code> values printed in the browser viewport. This works fine, as the <code>output()</code> function is being called inside the other functions — in the same scope as the variables it is printing are defined in, in each case. <code>output()</code> itself is available from anywhere, as it is defined in the global scope.</li>
...
function b() {
 ...
 <li>Save and reload again, and try this again in your JavaScript console:
  <pre class="brush: js">a();
b();</pre>
  Both the <code>a()</code> and <code>b()</code> call should print the value of x to the browser viewport. These work fine because even though the <code>output()</code> calls are not in the same scope as <code>x</code> is defined in, <code>x</code> is a global variable so is available inside all code, everywhere.</li>
