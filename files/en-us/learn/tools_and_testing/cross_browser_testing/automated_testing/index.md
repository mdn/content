...
![A code editor screen showing the output of the gulp command. Each line begins with brackets containing the time a task starts or finishes. Each bracket is followed by the word 'Starting' or 'Finished' and then the name of the task function in the series of tasks: 'html', 'css', 'js'. Bellow the line that logs the start of the 'js' task, an error is logged for file src/main.js: line 4, col 23 'arrow function syntax (=>)' is only available in ES6 (use 'esversion: 6'). In the lines that log the 'Finished' tasks, the name of each task is followed by the word 'after' and the duration of the task in milliseconds. The last 2 lines of the output log the start and end of 'default' tasks.](gulp-output.png)
