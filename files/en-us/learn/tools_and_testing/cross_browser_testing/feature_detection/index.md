...
This at-rule block applies the CSS rule within only if the current browser supports both the `grid-template-columns: subgrid;` declaration. For a condition with a value to work, you need to include a complete declaration (not just a property name) and NOT include the semicolon on the end.
