...
For example, Firefox uses `-moz-` and Chrome/Edge/Opera/Safari use `-webkit-`. Other prefixes you may encounter in old code include `-ms-` which was used by Internet Explorer and early versions of Edge used `-ms-` and `-o` which was used in the origin versions of Opera.
