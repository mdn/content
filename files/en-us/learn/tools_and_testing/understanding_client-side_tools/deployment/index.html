...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>To finish working through our complete toolchain case study, focusing on deploying the app.</td>
  </tr>
 </tbody>
</table>

<h2 id="Post_development">Post development</h2>
...
<h2 id="The_build_process">The build process</h2>
...
<ol>
 <li>
  <p>Open the <code>package.json</code> file in your project's root directory, and find the <code>scripts</code> property.</p>
 </li>
 <li>
  <p>We'll add a <code>build</code> command that we can run to build our code. Add the following line to your project now:</p>

  <pre class="brush: bash">"scripts": {
  ...
  <strong>"build": "parcel build src/index.html"</strong>
}</pre>
 ...
 <li>
  <p>You should now be able to run the following command in the root of your project directory to run the production build step (first quit the running process with <kbd>Ctrl</kbd> + <kbd>C</kbd> if you need to):</p>

  <pre class="brush: bash">npm run build</pre>
  ...
  <pre class="brush: bash">dist/src.99d8a31a.js.map       446.15 KB     63ms
...
dist/index.html                    354 B    944ms</pre>
...
<h2 id="Committing_changes_to_GitHub">Committing changes to GitHub</h2>
...
<ol>
 <li>
  <p>To add changes, run the following command:</p>

  <pre class="brush: bash">git add .</pre>

  <p>Note the period at the end, it means “everything in this directory”. The <code>git add .</code> command is a bit of a sledgehammer approach — it will add all local changes you’ve worked on in one go. If you want finer control over what you add, then use <code>git add -p</code> for an interactive process, or add individual files using <code>git add path/to/file</code>.</p>
 </li>
 <li>
  <p>Now all the code is staged, we can commit; run the following command:</p>

  <pre class="brush: bash">git commit -m ’committing initial code’</pre>
 ...
 <li>
  <p>Finally the code needs to be pushed to your GitHub hosted repository. Let's do that now.</p>

  <p>Over at github, visit <a href="https://github.com/new">https://github.com/new</a> and create your own repository to host this code.</p>
 </li>
 <li>
  <p>Give your repository a short, memorable name, without spaces in it (use hyphens to separate words), and a description, then click <em>Create repository</em> at the bottom of the page.</p>

  <p>You should now have a "remote" URL that points to your new GitHub repo.</p>

  <p><img alt="github screenshot showing remote URLs you can use to deploy code to a GitHub repo" src="github-quick-setup.png"></p>
 </li>
 <li>
  <p>This remote location needs to be added to our local git repository before we can push it up there, otherwise it won't be able to find it. You'll need to run a command with the following structure (use the provided HTTPS option for now — especially if you are new to GitHub — not the SSH option):</p>

  <pre class="brush: bash">git remote add github https://github.com/<em>yourname</em>/<em>repo-name</em>.git</pre>
  ...
  <pre class="brush: bash">git remote add github https://github.com/remy/super-website.git</pre>

  <p>Change the URL to your own repository, and run it now.</p>
 </li>
 <li>
...
<h2 id="Using_Netlify_for_deployment">Using Netlify for deployment</h2>
...
<ol>
 <li>
  <p>Go to <a href="https://app.netlify.com/start">https://app.netlify.com/start</a>.</p>
 </li>
 <li>
  <p>Press the Github button underneath the <em>Continuous Deployment</em> heading. "Continuous Deployment" means that whenever the code repository changes, Netlify will (try) to deploy the code, thus it being "continuous".</p>

  <p><img alt="netlify deployment options, as described in the surrounding text" src="netlify-deploy.png"></p>
 </li>
 <li>
  <p>Depending on whether you authorized Netlify before, you might need to authorize Netlify with GitHub, and choose what account you want to authorize it for (if you have multiple GitHub accounts or orgs). Choose the one you pushed your project to.</p>
 </li>
 <li>
  <p>Netlify will prompt you with a list of the GitHub repositories it can find. Select your project repository and proceed to the next step.</p>
 </li>
 <li>
  <p>Since we've connected Netlify to our Github account and given it access to deploy the project repository, Netlify will ask <em>how</em> to prepare the project for deployment and <em>what</em> to deploy.</p>

  <p>You should enter the command <code>npm run build</code> for the <em>Build command</em>, and specify the <code>dist</code> directory for the <em>Publish directory</em> — this contains the code that we want to make public.</p>
 </li>
 <li>
  <p>To finish up, click <em>Deploy site</em>.</p>

  <p><img alt="netlify distribution options, as described in the surrounding text" src="netlify-dist.png"></p>
 </li>
 <li>
  <p>After a short wait for the deployment to occur, you should get a URL that you can go to, to see your published site — try it out!</p>
 </li>
 <li>
  <p>And even better, whenever we make a change and <em>push</em> the change to our remote git repository (on GitHub), this will trigger a notification to Netlify which will then run our specified build task and then deploy the resulting <code>dist</code> directory to our published site.</p>
...
<ol>
 <li>
  <p>Go to your <code>package.json</code> file and open it up.</p>
 </li>
 <li>
  <p>Find your <code>scripts</code> member, and update it so that it contains the following test and build commands:</p>
 ...
 <li>
  <p>Now of course we need to add the test to our codebase; create a new directory in your root directory called tests:</p>

  <pre class="brush: bash">mkdir tests</pre>
 </li>
 <li>
  <p>Inside the new directory, create a test file:</p>

  <pre class="brush: bash">cd tests
touch nasa-feed.test.js  </pre>
 </li>
 <li>
  <p>Open this file, and add the contents of <a href="https://raw.githubusercontent.com/remy/mdn-will-it-miss/master/tests/nasa-feed.test.js">nasa-feed.test.js</a> to it:</p>
 </li>
 <li>
  <p>This test uses the axios package to fetch the data feed we want to test; to install this dependency, run the following command:</p>

  <pre class="brush: bash">npm install --save-dev axios</pre>

  <p>We need to manually install axios because Parcel won't help us with this dependency. Our tests are outside of Parcel's view of our system — since Parcel never sees nor runs any of the test code, we're left to install the dependency ourselves.</p>
 </li>
 <li>
  <p>Now to manually run the test, from the command line we can run:</p>

  <pre class="brush: bash">npm run test</pre>
  ...
  <p>In our build script, <code>npm run test</code> is called, then you see the string <code>&amp;&amp;</code> — this means "if the thing on the left succeeded (exited with zero), then do this thing on the right". So this translates into: if the tests pass, then build the code.</p>
 </li>
 <li>
  <p>You'll have to upload your new code to GitHub, using similar commands to what you used before:</p>
