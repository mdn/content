...
Many of the functioning Python files generated by `startproject` will likely have "errors" such as objects not containing members and functions not returning expected types. These errors are being generated because `pylint`, the standard error detection tool for Python, needs to be changed to `pylint-django`. The steps to activate `pylint-django` depend on your development environment and are thus beyond this tutorial, but this should give you an idea of where to start.
