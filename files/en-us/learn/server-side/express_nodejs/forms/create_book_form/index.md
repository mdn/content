...
- The authors are ordered alphabetically using unbuffered code, before adding them as options in the drop-down list. Code which starts with `-` in pug doesn´t add anything to the view.

```pug
 - authors.sort(function(a, b) {let textA = a.family_name.toUpperCase(); let textB = b.family_name.toUpperCase(); return (textA < textB) ? -1 : (textA > textB) ? 1 : 0;}); // This piece of code orders the Authors set but doesn't add anything to the view
 ```
