...
![CSS grid with twelve columns and two rows. The top row demonstrates the size of individual columns. The second row demonstrates some different sized areas on the grid.](simple-grid-finished.png)
...
![The grid has 2 rows. Frist row has 12 children and the second row has 4 children. The items with label 14 is situated in the second ro.w. 'offset-by-one' class is applied to the fourteenth item which leaves one column wide empty space on the left-hand side of it. ](offset-grid-finished.png)
...
![The grid has two rows. Each row is a flex-container. First row has twelve flex-items and the second row has four flex-items. The items in a row shrink and flex equally when the viewport width changes. The width of all the items in a row is the same.](flexbox-grid-incomplete.png)
