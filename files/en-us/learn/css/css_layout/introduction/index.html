...
<table>
 <tbody>
  <tr>
   ...
   <td>The basics of HTML (study <a href="/en-US/docs/Learn/HTML/Introduction_to_HTML">Introduction to HTML</a>), and an idea of How CSS works (study <a href="/en-US/docs/Learn/CSS/First_steps">Introduction to CSS</a>.)</td>
  </tr>
  <tr>
...
<ul>
 <li>Normal flow</li>
 <li>The {{cssxref("display")}} property</li>
 <li>Flexbox</li>
 <li>Grid</li>
 <li>Floats</li>
 <li>Positioning</li>
...
<p>Note how the HTML is displayed in the exact order in which it appears in the source code, with elements stacked on top of one another — the first paragraph, followed by the unordered list, followed by the second paragraph.</p>
...
<ul>
 <li><strong>The {{cssxref("display")}} property</strong> — Standard values such as <code>block</code>, <code>inline</code> or <code>inline-block</code> can change how elements behave in normal flow, for example, by making a block-level element behave like an inline-level element (see <a href="/en-US/docs/Learn/CSS/Building_blocks/The_box_model#block_and_inline_boxes">Types of CSS boxes</a> for more information). We also have entire layout methods that are enabled via specific <code>display</code> values, for example, <a href="/en-US/docs/Learn/CSS/CSS_layout/Grids">CSS Grid</a> and <a href="/en-US/docs/Learn/CSS/CSS_layout/Flexbox">Flexbox</a>, which alter how child elements are laid out inside their parents.</li>
 <li><strong>Floats</strong> — Applying a {{cssxref("float")}} value such as <code>left</code> can cause block-level elements to wrap along one side of an element, like the way images sometimes have text floating around them in magazine layouts.</li>
 <li><strong>The {{cssxref("position")}} property</strong> — Allows you to precisely control the placement of boxes inside other boxes. <code>static</code> positioning is the default in normal flow, but you can cause elements to be laid out differently using other values, for example, as fixed to the top of the browser viewport.</li>
 ...
 <li><strong>Multi-column layout</strong> — The <a href="/en-US/docs/Web/CSS/CSS_Columns">Multi-column layout</a> properties can cause the content of a block to layout in columns, as you might see in a newspaper.</li>
</ul>
...
<p>The {{cssxref("float")}} property has four possible values:</p>
...
<p>To turn a block into a multi-column container, we use either the {{cssxref("column-count")}} property, which tells the browser <em>how many</em> columns we would like to have, or the {{cssxref("column-width")}} property, which tells the browser to fill the container with as many columns as possible of a <em>specified width</em>.</p>
