...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>To learn how to create floated features on webpages and to use the clear property as well as other methods for clearing floats.</td>
  </tr>
 </tbody>
</table>
...
<p>Floats have commonly been used to create entire web site layouts featuring multiple columns of information floated so they sit alongside one another (the default behavior would be for the columns to sit below one another in the same order as they appear in the source). There are newer, better layout techniques available. Using floats in this way should be regarded as a <a href="/en-US/docs/Learn/CSS/CSS_layout/Legacy_Layout_Methods">legacy technique</a>.</p>
...
<p>We've seen that a float is removed from normal flow and that other elements will display beside it. If we want to stop the following element from moving up, we need to <em>clear</em> it; this is achieved with the {{cssxref("clear")}} property.</p>
...
<p>The way that this situation has traditionally been dealt with is to use something known as a "clearfix hack". This involves first inserting some generated content after the box that contains both the float and content wrapped around it, then setting that generated content to clear both.</p>
...
<p>Remove the clearfix CSS you added in the last section; instead add <code>overflow: auto</code> to the rules for wrapper. Once again, the box should clear.</p>
