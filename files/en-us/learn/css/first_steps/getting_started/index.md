...
If the level one heading at the top of the document is now red, congratulations — you have successfully applied CSS to an HTML document.

We have linked the stylesheet to the HTML file and are now done editing the HTML file. To continue with this tutorial, you can continue to work in your local `styles.css` file or you can use our interactive editor. 
...
When we look at a well-marked-up HTML document with no added CSS (like our example), we can see how the browser makes the HTML code readable. Headings are large and bold and lists have bullets. This happens because all modern browsers apply their own internal stylesheets with default styles to all pages.

To change the default styles, you can choose the HTML element that you want to style and apply a CSS rule to it. A good example is `<ul>`, an unordered list. By default, it has bullets, but you can remove them like so:
...
The most common way to do this is to add a [`class` attribute](/en-US/docs/Web/HTML/Global_attributes/class) to your HTML element and target that class.
...
Sometimes, you will want something to look different based on where it is in the document. There are a number of selectors that can help you here, but for now, we will look at just a couple.
