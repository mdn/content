...
Another advanced topic that you might not use right away but may need to understand in the future is [`@scope`](/en-US/docs/Web/CSS/@scope). This is an [at-rule](/en-US/docs/Web/CSS/At-rule) that enables you to create a block of rules that will only apply to a specific subsection of the HTML on your page. For example, to specify some styles that will only apply to {{htmlelement("img")}} elements nested inside elements with a class of `feature`:
