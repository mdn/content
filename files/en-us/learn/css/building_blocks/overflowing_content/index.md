...
<table>
  <caption>Prerequisites &amp; Objectives</caption>
  <tbody>
    <tr>
...
The {{cssxref("overflow")}} property is how you take control of an element's overflow. It is the way you instruct the browser how it should behave. The default {{CSSXref("overflow_value")}} value of overflow is `visible`. With this default, we can see content when it overflows.
...
When you use an `<overflow>` value such as `scroll` or `auto`, you create a [**Block Formatting Context** (BFC)](/en-US/docs/Web/Guide/CSS/Block_formatting_context). The content of the box that you have changed the value of `overflow` for acquires a self-contained layout. Content outside the container cannot poke into the container, and nothing can poke out of that container into the surrounding layout. This enables scrolling behavior, as all box content needs to be contained and not overlap to create a consistent scrolling experience.
