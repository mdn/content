...
<p>In this lesson, we examine some of the simplest selectors, which you will probably use most frequently in your work.</p>
...
<p>A <strong>type selector</strong> is sometimes referred to as a <em>tag name selector</em> or <em>element selector</em> because it selects an HTML tag/element in your document. In the example below, we have used the <code>span</code>, <code>em</code> and <code>strong</code> selectors.</p>
...
<p>The universal selector is indicated by an asterisk (<code>*</code>). It selects everything in the document (or inside the parent element if it is being chained together with another element and a descendant combinator). In the following example, we use the universal selector to remove the margins on all elements. Instead of the default styling added by the browser — which spaces out headings and paragraphs with margins — everything is close together.</p>

<p>{{EmbedGHLiveSample("css-examples/learn/selectors/universal.html", '100%', 750)}}</p>

<p>This kind of behavior can sometimes be seen in "reset stylesheets", which strip out all of the browser styling. Since the universal selector makes global changes, we use it for very specific situations, such as the one described below.</p>
...
<p>One use of the universal selector is to make selectors easier to read and more obvious in terms of what they are doing. For example, if we wanted to select any descendant elements of an <code>&lt;article&gt;</code> element that are the first child of their parent, including direct children, and make them bold, we could use the {{cssxref(":first-child")}} pseudo-class. We will learn more about this in the lesson on <a href="/en-US/docs/Learn/CSS/Building_blocks/Selectors/Pseudo-classes_and_pseudo-elements">pseudo-classes and pseudo-elements</a>, as a descendant selector along with the <code>&lt;article&gt;</code> element selector: </p>

<pre class="brush: css">article :first-child {
  font-weight: bold;
}</pre>

<p>However, this selector could be confused with <code>article:first-child</code>, which will select any <code>&lt;article&gt;</code> element that is the first child of another element.</p>

<p>To avoid this confusion, we can add the universal selector to the <code>:first-child</code> pseudo-class, so it is more obvious what the selector is doing. It is selecting <em>any</em> element which is the first-child of an <code>&lt;article&gt;</code> element, or the first-child of any descendant element of <code>&lt;article&gt;</code>:</p>
...
<p>The class selector starts with a dot (<code>.</code>) character. It will select everything in the document with that class applied to it. In the live example below we have created a class called <code>highlight</code>, and have applied it to several places in my document. All of the elements that have the class applied are highlighted.</p>
...
<p>An ID selector begins with a <code>#</code> rather than a dot character, but is used in the same way as a class selector. However, an ID can be used only once per page, and elements can only have a single <code>id</code> value applied to them. It can select an element that has the <code>id</code> set on it, and you can precede the ID with a type selector to only target the element if both the element and ID match. You can see both of these uses in the following example:</p>

<p>{{EmbedGHLiveSample("css-examples/learn/selectors/id.html", '100%', 750)}}</p>

<div class="notecard warning">
<p><strong>Warning:</strong> Using the same ID multiple times in a document may appear to work for styling purposes, but don't do this. It results in invalid code, and will cause strange behavior in many places.</p>
</div>

<div class="notecard note">
<p><strong>Note:</strong> As we learned in the lesson on specificity, an ID has high specificity. It will overrule most other selectors. In most cases, it is preferable to add a class to an element instead of an ID. However, if using the ID is the only way to target the element — perhaps because you do not have access to the markup and cannot edit it — this will work.</p>
</div>
