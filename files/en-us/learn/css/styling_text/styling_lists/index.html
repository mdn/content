...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>To become familiar with the best practices and properties related to styling lists.</td>
  </tr>
 </tbody>
</table>

<h2 id="A_simple_list_example">A simple list example</h2>

<p>To begin with, let's look at a simple list example. Throughout this article, we'll look at unordered, ordered, and description lists — all have styling features that are similar, as well as some that are particular to themselves. The unstyled example is <a href="https://mdn.github.io/learning-area/css/styling-text/styling-lists/unstyled-list.html">available on Github</a> (check out the <a href="https://github.com/mdn/learning-area/blob/master/css/styling-text/styling-lists/unstyled-list.html">source code</a> too.)</p>

<p>The HTML for our list example looks like so:</p>
...
<ul>
 ...
 <li>Rules 5 and 6 apply to the description list. We set the same <code>line-height</code> on the description list terms and descriptions as we did with the paragraphs and list items. Again, consistency is good! We also make the description terms have bold font, so they visually stand out easier.</li>
</ul>

<h2 id="List-specific_styles">List-specific styles</h2>

<p>Now that we've looked at general spacing techniques for lists, let's explore some list-specific properties. There are three properties you should know about to start with, which can be set on {{htmlelement("ul")}} or {{htmlelement("ol")}} elements:</p>
...
<ul>
 ...
 <li>{{cssxref("list-style-position")}}: Sets whether the bullets, at the start of each item, appear inside or outside the lists.</li>
...
<p><img alt="an ordered list with the bullet points set to appear outside the list item text." src="outer-bullets.png"></p>
...
<p><img alt="an ordered list with the bullet points set to appear inside the list item text." src="inner-bullets.png"></p>
...
<p>However, this property is a bit limited in terms of controlling the position, size, etc. of the bullets. You are better off using the {{cssxref("background")}} family of properties, which you've learned in the <a href="/en-US/docs/Learn/CSS/Building_blocks/Backgrounds_and_borders">Backgrounds and borders</a> article. For now, here's a taster!</p>
...
<p><img alt="an unordered list with the bullet points set as little star images" src="list_formatting.png"></p>
...
<p>{{ EmbedLiveSample('Active_learning_Styling_a_nested_list', 700, 800) }}</p>
...
<p>Lists are relatively easy to get the hang of styling once you know a few associated basic principles and specific properties. In the next article we'll move on to link styling techniques.</p>
