...
> **Note:** You may have noticed that the list items in the HTML are separated by comments. This is done because any uncommented spaces/line breaks in between inline block elements create spaces on the page, just like the spaces in between words. Such spaces would break our horizontal navigation menu layout, because their width would cause the last button to overflow and fall onto the next line. You can find more information about this problem (and some other solutions) at [Fighting the space between inline block elements](https://css-tricks.com/fighting-the-space-between-inline-block-elements/).
