...
<p>Description lists use a different wrapper than the other list types — {{htmlelement("dl")}}; in addition each term is wrapped in a {{htmlelement("dt")}} (description term) element, and each description is wrapped in a {{htmlelement("dd")}} (description definition) element.
...
<p>{{EmbedLiveSample('Description_list_example', '100%', '285px')}}</p>

<h3>Multiple descriptions for one term</h3>
...
<p>{{EmbedLiveSample('Multiple_descriptions_for_one_term', '100%', '193px')}}</p>
...
<p>{{ EmbedLiveSample('Active_learning_Marking_up_a_set_of_definitions', 700, 350) }}</p>
...
<pre class="brush: html">&lt;p&gt;Here below is a blockquote...&lt;/p&gt;
...
<p>{{EmbedLiveSample('Blockquotes', '100%', '200px')}}</p>
...
<p>{{EmbedLiveSample('Inline_quotations', '100%', '78px')}}</p>
...
<p>There is a {{htmlelement("cite")}} element, but this is meant to contain the title of the resource being quoted, e.g. the name of the book. There is no reason, however, why you couldn't link the text inside <code>&lt;cite&gt;</code> to the quote source in some way:</p>

<pre class="brush: html">&lt;p&gt;According to the &lt;a href="/en-US/docs/Web/HTML/Element/blockquote"&gt;
...
<p>{{EmbedLiveSample('Citations', '100%', '179px')}}</p>
...
<ol>
 ...
 <li>Turn "The Need To Eliminate Negative Self Talk" in the third paragraph into an inline quote, and include a <code>cite</code> attribute.</li>
 <li>Wrap the title of each source in <code>&lt;cite&gt;</code> tags and turn each one into a link to that source.</li>
</ol>

<p>The citation sources you need are:</p>

<ul>
 <li>http://www.brainyquote.com/quotes/authors/c/confucius.html for the Confucius quote</li>
 <li>http://example.com/affirmationsforpositivethinking for "The Need To Eliminate Negative Self Talk".</li>
</ul>
...
<p>{{ EmbedLiveSample('Active_learning_Who_said_that', 700, 450) }}</p>

<h2 id="Abbreviations">Abbreviations</h2>

<p>Another fairly common element you'll meet when looking around the Web is {{htmlelement("abbr")}} — this is used to wrap around an abbreviation or acronym, and provide a full expansion of the term (included inside a {{htmlattrxref("title")}} attribute.)

<h3>Abbreviation example</h3>

<p>Let's look at an example.</p>

<pre class="brush: html">&lt;p&gt;We use &lt;abbr title="Hypertext Markup Language"&gt;HTML&lt;/abbr&gt; to structure our web documents.&lt;/p&gt;
...
<p>{{EmbedLiveSample('Abbreviation_example', '100%', '150')}}</p>
...
<p>{{ EmbedLiveSample('Active_learning_marking_up_an_abbreviation', 700, 300) }}</p>

<h2 id="Marking_up_contact_details">Marking up contact details</h2>

<p>HTML has an element for marking up contact details — {{htmlelement("address")}}. This wraps around your contact details, for example:</p>
...
<div class="notecard note">
  <p><strong>Note:</strong> The {{htmlelement("address")}} element should only be used to provide contact information for the document contained with the nearest {{htmlelement("article")}} or {{htmlelement("body")}} element. It would be correct to use it in the footer of a site to include the contact information of the entire site, on inside an article for the contact details of the author, but not to mark up a list of addresses unrelated to the content of that page.</p>
</div>
...
<p>{{ EmbedLiveSample('Superscript_and_subscript', '100%', '141px') }}</p>
...
<pre class="brush: html">&lt;time datetime="2016-01-20"&gt;20 January 2016&lt;/time&gt;
</pre>
...
<p>But these different forms cannot be easily recognized by computers — what if you wanted to automatically grab the dates of all events in a page and insert them into a calendar? The {{htmlelement("time")}} element allows you to attach an unambiguous, machine-readable time/date for this purpose.</p>
