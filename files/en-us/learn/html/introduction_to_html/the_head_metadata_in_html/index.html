...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>To learn about the HTML head, its purpose, the most important items it can contain, and what effect it can have on the HTML document.</td>
  </tr>
 </tbody>
</table>

<h2 id="What_is_the_HTML_head">What is the HTML head?</h2>

<p>Let's revisit the simple <a href="/en-US/docs/Learn/HTML/Introduction_to_HTML/Getting_started#anatomy_of_an_html_document">HTML document we covered in the previous article</a>:</p>
...
<p>The HTML head is the contents of the {{htmlelement("head")}} element — unlike the contents of the {{htmlelement("body")}} element (which are displayed on the page when loaded in a browser), the head's content is not displayed on the page. Instead, the head's job is to contain {{glossary("Metadata", "metadata")}} about the document. In the above example, the head is quite small:</p>
...
<p>In larger pages however, the head can get quite full. Try going to some of your favorite websites and use the <a href="/en-US/docs/Learn/Common_questions/What_are_browser_developer_tools">developer tools</a> to check out their head contents. Our aim here is not to show you how to use everything that can possibly be put in the head, but rather to teach you how to use the major elements that you'll want to include in the head, and give you some familiarity. Let's get started.</p>

<h2 id="Adding_a_title">Adding a title</h2>

<p>We've already seen the {{htmlelement("title")}} element in action — this can be used to add a title to the document. This however can get confused with the {{htmlelement("h1")}} element, which is used to add a top level heading to your body content — this is also sometimes referred to as the page title. But they are different things!</p>

<ul>
 <li>The {{htmlelement("h1")}} element appears on the page when loaded in the browser — generally this should be used once per page, to mark up the title of your page content (the story title, or news headline, or whatever is appropriate to your usage.)</li>
 <li>The {{htmlelement("title")}} element is metadata that represents the title of the overall HTML document (not the document's content.)</li>
</ul>
...
<ol>
 ...
 <li>Now open the file in your browser. You should see something like this:
  <p><img alt="A simple web page with the title set to &lt;title&gt; element, and the &lt;h1&gt; set to &lt;h1&gt; element." src="title-example.png">It should now be completely obvious where the <code>&lt;h1&gt;</code> content appears, and where the <code>&lt;title&gt;</code> content appears!</p>
 </li>
...
<p><img alt="A webpage being bookmarked in firefox; the bookmark name has been automatically filled in with the contents of the &lt;title&gt; element " src="bookmark-example.png"></p>

<p>The <code>&lt;title&gt;</code> contents are also used in search results, as you'll see below.</p>

<h2 id="Metadata_the_&lt;meta&gt;_element">Metadata: the &lt;meta&gt; element</h2>
...
<h3 id="Specifying_your_documents_character_encoding">Specifying your document's character encoding</h3>

<p>In the example we saw above, this line was included:</p>

<pre class="brush: html">&lt;meta charset="utf-8"&gt;</pre>

<p>This element specifies the document's character encoding — the character set that the document is permitted to use. <code>utf-8</code> is a universal character set that includes pretty much any character from any human language. This means that your web page will be able to handle displaying any language; it's therefore a good idea to set this on every web page you create! For example, your page could handle English and Japanese just fine:</p>

<p><img alt="a web page containing English and Japanese characters, with the character encoding set to universal, or utf-8. Both languages display fine," src="correct-encoding.png">If you set your character encoding to <code>ISO-8859-1</code>, for example (the character set for the Latin alphabet), your page rendering may appear all messed up:</p>

<p><img alt="a web page containing English and Japanese characters, with the character encoding set to latin. The Japanese characters don't display correctly" src="bad-encoding.png"></p>

<div class="note">
<p><strong>Note:</strong> Some browsers (like Chrome) automatically fix incorrect encodings, so depending on what browser you use, you may not see this problem. You should still set an encoding of <code>utf-8</code> on your page anyway to avoid any potential problems in other browsers.</p>
</div>
...
<h3 id="Adding_an_author_and_description">Adding an author and description</h3>
...
<p>Specifying an author is beneficial in many ways: it is useful to be able to understand who wrote the page, if you have any questions about the content and you would like to contact them. Some content management systems have facilities to automatically extract page author information and make it available for such purposes.</p>
...
<ol>
 ...
 <li>Now search for "MDN Web Docs" in your favorite search engine (We used Google.) You'll notice the description <code>&lt;meta&gt;</code> and <code>&lt;title&gt;</code> element content used in the search result — definitely worth having!
  <p><img alt='A Yahoo search result for "Mozilla Developer Network"' src="mdn-search-result.png"></p>
 </li>
</ol>
...
<p>One effect of this is that when you link to MDN Web Docs on facebook, the link appears along with an image and description: a richer experience for users.</p>

<p><img alt="Open graph protocol data from the MDN homepage as displayed on facebook, showing an image, title, and description." src="facebook-output.png"></p>

<p>Twitter also has its own similar proprietary metadata called <a href="https://developer.twitter.com/en/docs/tweets/optimize-with-cards/overview/abouts-cards">Twitter Cards</a>, which has a similar effect when the site's URL is displayed on twitter.com. For example:</p>

<pre class="brush: html">&lt;meta name="twitter:title" content="Mozilla Developer Network"&gt;</pre>

<h2 id="Adding_custom_icons_to_your_site">Adding custom icons to your site</h2>

<p>To further enrich your site design, you can add references to custom icons in your metadata, and these will be displayed in certain contexts. The most commonly used of these is the <strong>favicon</strong> (short for "favorites icon", referring to its use in the "favorites" or "bookmarks" lists in browsers).</p>
...
<p><img alt="The Firefox bookmarks panel, showing a bookmarked example with a favicon displayed next to it." src="bookmark-favicon.png"></p>
...
<h2 id="Applying_CSS_and_JavaScript_to_HTML">Applying CSS and JavaScript to HTML</h2>
...
<ul>
 <li>
  <p>The {{htmlelement("link")}} element should always go inside the head of your document. This takes two attributes, <code>rel="stylesheet"</code>, which indicates that it is the document's stylesheet, and <code>href</code>, which contains the path to the stylesheet file:</p>

  <pre class="brush: html">&lt;link rel="stylesheet" href="my-css-file.css"&gt;</pre>
 </li>
 <li>
  <p>The {{htmlelement("script")}} element should also go into the head, and should include a <code>src</code> attribute containing the path to the JavaScript you want to load, and <code>defer</code>, which basically instructs the browser to load the JavaScript after the page has finished parsing the HTML. This is useful as it makes sure that the HTML is all loaded before the JavaScript runs, so that you don't get errors resulting from JavaScript trying to access an HTML element that doesn't exist on the page yet. There are actually a number of ways to handle loading JavaScript on your page, but this is the most foolproof one to use for modern browsers (for others, read <a href="/en-US/docs/Learn/JavaScript/First_steps/What_is_JavaScript#script_loading_strategies">Script loading strategies</a>). </p>

  <pre class="brush: html">&lt;script src="my-js-file.js" defer&gt;&lt;/script&gt;</pre>
...
<p><img alt="Example showing a page with CSS and JavaScript applied to it. The CSS has made the page go green, whereas the JavaScript has added a dynamic list to the page." src="js-and-css.png"></p>

<ul>
 <li>The JavaScript has added an empty list to the page. Now when you click anywhere outside the list, a dialog box will pop up asking you to enter some text for a new list item. When you press the OK button, a new list item will be added to the list containing the text. When you click on an existing list item, a dialog box will pop up allowing you to change the item's text.</li>
...
<p>You can also set subsections of your document to be recognized as different languages. For example, we could set our Japanese language section to be recognized as Japanese, like so:</p>
