...
<p>{{ EmbedLiveSample('Active_learning_classic_embedding_uses', 700, 600) }}</p>
...
<dl>
 <dt><a href="/en-US/docs/Web/CSS/border"><code>border: none</code></a></dt>
 <dd>If used, the <code>&lt;iframe&gt;</code> is displayed without a surrounding border. Otherwise, by default, browsers display the <code>&lt;iframe&gt;</code> with a surrounding border (which is generally undesirable).</dd>
 <dt>{{htmlattrxref('allowfullscreen','iframe')}}</dt>
 <dd>If set, the <code>&lt;iframe&gt;</code> is able to be placed in fullscreen mode using the <a href="/en-US/docs/Web/API/Fullscreen_API">Fullscreen API</a> (somewhat beyond the scope of this article.)</dd>
...
<div class="note">
<p><strong>Note:</strong> In order to improve speed, it's a good idea to set the iframe's <code>src</code> attribute with JavaScript after the main content is done with loading. This makes your page usable sooner and decreases your official page load time (an important {{glossary("SEO")}} metric.)</p>
</div>
...
<div class="note">
<p><strong>Note:</strong> <a href="/en-US/docs/Learn/Common_questions/Using_Github_pages">GitHub pages</a> allow content to be served via HTTPS by default, so it is useful for hosting content. If you are using a different hosting provider and are not sure, ask them about it.</p>
</div>
