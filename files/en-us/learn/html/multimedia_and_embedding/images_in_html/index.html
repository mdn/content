...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>To learn how to embed simple images in HTML, annotate them with captions, and how HTML images relate to CSS background images.</td>
  </tr>
 </tbody>
</table>

<h2 id="How_do_we_put_an_image_on_a_webpage">How do we put an image on a webpage?</h2>

<p>In order to put a simple image on a webpage, we use the {{htmlelement("img")}} element. This is an {{glossary("empty element")}} (meaning that it has no text content or closing tag) that requires a minimum of one attribute to be useful — <code>src</code> (sometimes spoken as its full title, <em>source</em>). The <code>src</code> attribute contains a path pointing to the image you want to embed in the page, which can be a relative or absolute URL, in the same way as <code>href</code> attribute values in {{htmlelement("a")}} elements.</p>

<div class="note">
<p><strong>Note:</strong> You should read <a href="/en-US/docs/Learn/HTML/Introduction_to_HTML/Creating_hyperlinks#a_quick_primer_on_urls_and_paths">A quick primer on URLs and paths</a> to refresh your memory on relative and absolute URLs before continuing.</p>
</div>

<p>So for example, if your image is called <code>dinosaur.jpg</code>, and it sits in the same directory as your HTML page, you could embed the image like so:</p>
...
<div class="note">
<p><strong>Note:</strong> Search engines also read image filenames and count them towards SEO. Therefore, you should give your image a descriptive filename; <code>dinosaur.jpg</code> is better than <code>img835.png</code>.</p>
</div>

<p>You could embed the image using its absolute URL, for example:</p>

<pre class="brush: html">&lt;img src="https://www.example.com/images/dinosaur.jpg"&gt;</pre>

<p>But this is pointless, as it just makes the browser do more work, looking up the IP address from the DNS server all over again, etc. You'll almost always keep the images for your website on the same server as your HTML.</p>

<div class="warning">
<p><strong>Warning:</strong> Most images are copyrighted. Do <strong>not</strong> display an image on your webpage unless:</p>

<ul>
 <li>You own the image.</li>
 <li>You have received explicit, written permission from the image owner.</li>
 <li>You have ample proof that the image is, in fact, in the public domain.</li>
</ul>

<p><br>
 Copyright violations are illegal and unethical. In addition, <strong>never</strong> point your <code>src</code> attribute at an image hosted on someone else's website that you don't have permission to link to. This is called "hotlinking". Again, stealing someone's bandwidth is illegal. It also slows down your page, leaving you with no control over whether the image is removed or replaced with something embarrassing.</p>
</div>

<p>Our above code would give us the following result:</p>

<p><img alt='A basic image of a dinosaur, embedded in a browser, with "Images in HTML" written above it' src="basic-image.png"></p>

<div class="note">
<p><strong>Note:</strong> Elements like {{htmlelement("img")}} and {{htmlelement("video")}} are sometimes referred to as <strong>replaced elements</strong>. This is because the element's content and size are defined by an external resource (like an image or video file), not by the contents of the element itself. You can read more about them at <a href="/en-US/docs/Web/CSS/Replaced_element">Replaced elements</a>.</p>
</div>

<div class="note">
<p><strong>Note:</strong> You can find the finished example from this section <a href="https://mdn.github.io/learning-area/html/multimedia-and-embedding/images-in-html/index.html">running on Github</a> (see the <a href="https://github.com/mdn/learning-area/blob/master/html/multimedia-and-embedding/images-in-html/index.html">source code</a> too.)</p>
</div>

<h3 id="Alternative_text">Alternative text</h3>

<p>The next attribute we'll look at is <code>alt</code>. Its value is supposed to be a textual description of the image, for use in situations where the image cannot be seen/displayed or takes a long time to render because of a slow internet connection. For example, our above code could be modified like so:</p>

<pre class="brush: html">&lt;img src="images/dinosaur.jpg"
     alt="The head and torso of a dinosaur skeleton;
          it has a large head with long sharp teeth"&gt;</pre>

<p>The easiest way to test your <code>alt</code> text is to purposely misspell your filename. If for example our image name was spelled <code>dinosooooor.jpg</code>, the browser wouldn't display the image, and would display the alt text instead:</p>
...
<ul>
 <li>The user is visually impaired, and is using a <a href="https://en.wikipedia.org/wiki/Screen_reader">screen reader</a> to read the web out to them. In fact, having alt text available to describe images is useful to most users.</li>
 <li>As described above, the spelling of the file or path name might be wrong.</li>
 <li>The browser doesn't support the image type. Some people still use text-only browsers, such as <a href="https://en.wikipedia.org/wiki/Lynx_%28web_browser%29">Lynx</a>, which displays the alt text of images.</li>
 ...
 <li>Users have turned off images to reduce data transfer volume and distractions. This is especially common on mobile phones, and in countries where bandwidth is limited or expensive.</li>
</ul>

<p>What exactly should you write inside your <code>alt</code> attribute? It depends on <em>why</em> the image is there in the first place. In other words, what you lose if your image doesn't show up:</p>

<ul>
 <li><strong>Decoration.</strong> You should use {{anch("CSS background images")}} for decorative images, but if you must use HTML, add a blank <code>alt=""</code>. If the image isn't part of the content, a screen reader shouldn't waste time reading it.</li>
 <li><strong>Content.</strong> If your image provides significant information, provide the same information in a <em>brief </em><code>alt</code> text – or even better, in the main text which everybody can see. Don't write redundant <code>alt</code> text. How annoying would it be for a sighted user if all paragraphs were written twice in the main content? If the image is described adequately by the main text body, you can just use <code>alt=""</code>.</li>
 <li><strong>Link.</strong> If you put an image inside {{htmlelement("a")}} tags, to turn an image into a link, you still must provide <a href="/en-US/docs/Learn/HTML/Introduction_to_HTML/Creating_hyperlinks#use_clear_link_wording">accessible link text</a>. In such cases you may, either, write it inside the same <code>&lt;a&gt;</code> element, or inside the image's <code>alt</code> attribute – whichever works best in your case.</li>
 <li><strong>Text.</strong> You should not put your text into images. If your main heading needs a drop shadow, for example, <a href="/en-US/docs/Web/CSS/text-shadow">use CSS</a> for that rather than putting the text into an image. However, If you <em>really can't avoid doing this</em>, you should supply the text inside the <code>alt</code> attribute.</li>
</ul>
...
<p>As <a href="/en-US/docs/Learn/HTML/Introduction_to_HTML/Creating_hyperlinks#adding_supporting_information_with_%3ctitle%3e">with links</a>, you can also add <code>title</code> attributes to images, to provide further supporting information if needed. In our example, we could do this:</p>
...
<p>{{ EmbedLiveSample('Active_learning_embedding_an_image', 700, 350) }}</p>
...
<p>{{ EmbedLiveSample('Active_learning_creating_a_figure', 700, 350) }}</p>
...
<div class="note">
<p><strong>Note:</strong> You'll learn a lot more about <a href="/en-US/docs/Learn/CSS/Building_blocks/Backgrounds_and_borders">CSS background images</a> in our <a href="/en-US/docs/Learn/CSS">CSS</a> topic.</p>
</div>

<h2 id="Test_your_skills!">Test your skills!</h2>

<p>You've reached the end of this article, but can you remember the most important information? You can find some further tests to verify that you've retained this information before you move on — see <a href="/en-US/docs/Learn/HTML/Multimedia_and_embedding/Images_in_HTML/Test_your_skills:_HTML_images">Test your skills: HTML images</a>.</p>
