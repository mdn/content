...
![Complex table has a caption above it. The top row cells are column header. There are three columns of headers. The first two columns have merged cells with the third column being individual headers for each row. All the text is centered. The headers and every other row have a slight background color.   ](assessment-table.png)
