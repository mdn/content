...
TCP's role is to ensure the packets are reliably delivered, error free. TCP has [concurrence control](https://en.wikipedia.org/wiki/Concurrency_control), which means the initial requests start small, increasing in size to the levels of bandwidth the computers, servers, and network can support.
