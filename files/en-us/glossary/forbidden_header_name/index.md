...
> **Note:** While {{HTTPHeader("Referer")}} header is listed as a forbidden header [in the spec](https://fetch.spec.whatwg.org/#forbidden-request-header), the user agent does not retain full control over it and the header can be programmatically modified. For example, [the `Referer` header](/en-US/docs/Web/API/Request/Request#referrer) can be programmatically modified when using [`fetch()`](/en-US/docs/Web/API/fetch) via [`referrer` option](/en-US/docs/Web/API/fetch#referrer)
