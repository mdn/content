...
What exactly counts as "content" is somewhat difficult to define precisely and so is heuristic-based to some extent and may not perfectly align to user perception or developer intent. These means some paints drawn to screen in different ways (for example, using different HTML elements), may count as "contentful" or not even if they appear to be the same to the user.
