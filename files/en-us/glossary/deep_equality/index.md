...
Deep equality implementations usually compare two objects recursively like so:
...
- Whether non-enumerable or symbol properties are compared
- Whether property descriptors are compared
- If accessor properties are compared as accessors, or they trigger getters and compare the return values
- Whether prototype chains are compared by identity
...
Comparing the results of {{jsxref("JSON.stringify()")}} is not a reliable deep equality check. Serialization can omit properties or transform values, property order affects the resulting string, and circular references cause an error.
