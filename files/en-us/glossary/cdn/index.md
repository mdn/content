...
- Contrary to popular belief, CDN may actually be _anti-performance_. By establishing connection with a third-party website, the user's browser has to go through more rounds of DNS lookup, content negotiation, etc. On the other hand, modern browsers do not share cache between different origins for the same resource for privacy reasons, so the user has to download the same asset (e.g. jQuery) multiple times on different websites anyway.
