...
**Content headers** are a group of [HTTP headers](/en-US/docs/Web/HTTP/Headers) that describe information related to safe transport and reconstruction of the original resource from one or more messages.
These headers used to be defined as "Payload headers", but "content" is used instead of "payload" because data contained within HTTP/2 and HTTP/3 frame payloads could be header data, body data, or other control information).
