...
Note also that if you wish to allow xml:base, you will need the [xml:base function](/en-US/docs/Archive/Add-ons/Code_snippets/XML/base_function), and the line beginning `let href = ...` should become:

```js
let href = getXmlBaseLink (/* XLink sans xml:base */ xinclude.getAttribute('href'), /* Element to query from */ xinclude);
```

```js
function resolveXIncludes(docu) {
    // http://www.w3.org/TR/xinclude/#xml-included-items
    let xincludes = docu.getElementsByTagNameNS('http://www.w3.org/2001/XInclude', 'include');
    if (xincludes) {
        for (i=0; i < xincludes.length; i++) {
            let xinclude = xincludes[i];
            let href = xinclude.getAttribute('href');
            let parse = xinclude.getAttribute('parse');
            let xpointer = xinclude.getAttribute('xpointer');
            let encoding = xinclude.getAttribute('encoding'); // e.g., UTF-8 // "text/xml or application/xml or matches text/*+xml or application/*+xml" before encoding (then UTF-8)
            let accept = xinclude.getAttribute('accept'); // header "Accept: "+x
            let acceptLanguage = xinclude.getAttribute('accept-language'); // "Accept-Language: "+x
            let xiFallback = xinclude.getElementsByTagNameNS('http://www.w3.org/2001/XInclude', 'fallback')[0]; // Only one such child is allowed
            ...
            let j;
            let xincludeParent = xinclude.parentNode;
            try {
                ...
                if (href !== null) {
                    ...
                    switch (parse) {
                        case 'text':
                            ...
                            let patternXML = /\.(svg|xml|xul|rdf|xhtml)$/;
                            if ((contentType && contentType.match(/[text|application]\/(.*)\+?xml/)) || (href.indexOf('file://') === 0 && href.match(patternXML))) {
                               ...
                               let encName = '([A-Za-z][A-Za-z0-9._-]*)';
                               let pattern = new RegExp('^<\\?xml\\s+.*encoding\\s*=\\s*([\'"])'+encName+'\\1.*\\?>'); // Check document if not?
                    ...
                    if((request.status === 200 || request.status === 0) && request[responseType] !== null) {
                        response = request[responseType];
                         if (responseType === 'responseXML') {
                            ...
                            let responseNodes;
                            if (xpointer) {
                                let xpathResult = response.evaluate(
                                                                 xpointer,
                                                                 response,
                                                                 null,
                            ...
                            xincludeParent.removeChild(xinclude);
                             ...
                             if (encName) {
                                  let encodingType = response.match(pattern);
                                  if (encodingType) {
                             ...
                             let textNode = docu.createTextNode(response);                             xincludeParent.replaceChild(textNode, xinclude);
                         }
                ...
                let xiFallbackChildren = xiFallback.childNodes;
