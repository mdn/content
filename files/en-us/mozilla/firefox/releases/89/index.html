...
<p>This article provides information about the changes in Firefox 89 that will affect developers. Firefox 89 was released on June 1, 2021.</p>

<div class="note">
  <p><strong>Note:</strong> See also <a href="https://hacks.mozilla.org/2021/06/looking-fine-with-firefox-89/">Looking fine with Firefox 89</a> on Mozilla Hacks.</p>
</div>
...
<ul>
  <li>Top-level <a href="/en-US/docs/Web/JavaScript/Reference/Operators/await#top-level-await"><code>await</code></a> is now enabled by default ({{bug(1681046)}}).</li>
...
<ul>
  <li>{{domxref("PerformanceEventTiming")}} is now enabled by default ({{bug(1701029)}}).</li>
  <li>The content of {{htmlelement("input")}} and {{htmlelement("textarea")}} elements can now be manipulated using {{domxref("Document.execCommand()")}} commands by default, preserving edit history and providing parity with other browsers, without <a href="/en-US/docs/Web/HTML/Global_attributes/contenteditable"><code>contentEditable</code></a> or any lengthy workarounds required ({{bug(1220696)}}).</li>
</ul>
...
<ul>
  <li>The following sensor events and their associated handlers have been removed (primarily for better compatibility with other major browser engines, and to address concerns related to privacy leaks):
     <ul>
       ...
       <li><code>DeviceLightEvent</code> and its event handler <code>window.ondevicelight</code> ({{bug(1701824)}}).</li>
     </ul>
  </li>
 </ul>
...
<ul>
  ...
  <li>Firefox's UI has been redesigned, which affects usage of the {{WebExtAPIRef("theme")}} API. The <code>tab_background_separator</code> and <code>toolbar_field_separator</code> properties are no longer supported. The <code>tab_line</code> and <code>toolbar_vertical_separator</code> will behave differently. For more information, see <a href="https://blog.mozilla.org/addons/2021/04/19/changes-to-themeable-areas-of-firefox-in-version-89/">Changes to themeable areas of Firefox in version 89</a>.</li>
  <li>The {{WebExtAPIRef("pageAction")}} button can no longer be pinned or unpinned from the address bar, because the three-dot meatball menu is no longer visible by default ({{bug(1691454)}}). As a result, the <code>pinned</code> property of the <code><a href="/en-US/docs/Mozilla/Add-ons/WebExtensions/manifest.json/page_action">page_action</a></code> manifest key no longer has any effect ({{bug(1703537)}}).</li>
  <li>The "Remove from Address Bar" context menu item has been removed from the {{WebExtAPIRef("pageAction")}} button ({{bug(1704474)}}). For alternatives to this functionality, see {{bug(1712556)}}.</li>
</ul>
