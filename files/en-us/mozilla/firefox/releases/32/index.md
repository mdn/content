...
- Enabled {{cssxref("mix-blend-mode")}} by default ([Webkit bug 952643](https://bugzil.la/952643)).
- Enabled `position:sticky` by default in release builds (only enabled on Nightly and Aurora before) ([Webkit bug 916315](https://bugzil.la/916315)).
- Implemented {{cssxref("box-decoration-break")}} and removed the non-standard `-moz-background-inline-policy` ([Webkit bug 613659](https://bugzil.la/613659)).
- Allowed {{cssxref("flex-grow")}} and {{cssxref("flex-shrink")}} to transition between zero and nonzero values, like 'flex-grow: 0.6'([Webkit bug 996945](https://bugzil.la/996945)).

### HTML

- Experimentally implemented, behind a pref, {{HTMLElement("img")}} [`srcset`](/en-US/docs/Web/HTML/Element/img#srcset) property, To activate it set `dom.image.srcset.enable` to `true` ([Webkit bug 870021](https://bugzil.la/870021)).
...
- Scoped selectors for {{domxref("Document.querySelector()")}} and {{domxref("Document.querySelectorAll()")}}, for example `querySelector(":scope > li")` have been implemented ([Webkit bug 528456](https://bugzil.la/528456)).
...
- To match the spec and the evolution of the CSS syntax, minor changes have been done to {{domxref("CSS.escape_static")}}. The identifier now can begins with `'--'` and the second dash must not be escaped. Also vendor identifier are no more escaped. ([Firefox bug 1008719](https://bugzil.la/1008719))
