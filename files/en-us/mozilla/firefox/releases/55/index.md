...
- The non-standard, Mozilla-specific, WebRTC offer options `mozDontOfferDataChannel` and `mozBundleOnly` have been removed from the [`RTCOfferOptions`](/en-US/docs/Web/API/RTCPeerConnection/createOffer#rtcofferoptions_dictionary) dictionary and are no longer supported by {{domxref("RTCPeerConnection.createOffer()")}} ({{bug(1196974)}}).
