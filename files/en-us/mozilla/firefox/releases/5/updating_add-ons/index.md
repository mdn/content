...
Instantiating certain services, including the `nsICertOverrideService`, at startup can make Firefox unusable ([Firefox bug 650858](https://bugzilla.mozilla.org/show_bug.cgi?id=650858). This happens only if you try to instantiate a service before the `load` event is fired.
