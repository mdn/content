...
<ul>
 ...
 <li>Support of a non-standard Mozilla-only media query to determine the operating system version has been added: <code><a href="/en-US/docs/Web/CSS/Media_Queries/Using_media_queries#-moz-os-version">-moz-os-version</a></code> ({{bug("810399")}}). The property is currently only implemented on Windows.</li>
 <li>The <code>-moz-osx-font-smoothing</code> CSS property has been added ({{bug("857142")}}).</li>
...
<ul>
 <li>The method {{jsxref("Array.of()")}} is now implemented on <code><a href="/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array">Array</a></code> ({{bug("866849")}}).</li>
 ...
 <li>New mathematical methods have been implemented on <a href="/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math"><code>Math</code></a>: <code>Math.log10()</code>, <code>Math.log2()</code>, <code>Math.log1p()</code>, <code>Math.expm1()</code>, <code>Math.cosh()</code>, <code>Math.sinh()</code>, <code>Math.tanh()</code>, <code>Math.acosh()</code>, <code>Math.asinh()</code>, <code>Math.atanh()</code>, <code>Math.trunc()</code>, <code>Math.sign()</code> and <code>Math.cbrt()</code> ({{bug("717379")}}).</li>
 ...
 <li>The machine epsilon constant, that is the smallest representible number that added to 1 will not be 1, is now available as {{jsxref("Global_Objects/Number/EPSILON", "Number.EPSILON")}} ({{bug("885798")}}).</li>
...
<ul>
 <li>The <a href="/en-US/docs/Web/API/Web_Audio_API">Web Audio API</a> is now supported. An incomplete implementation was previously available behind a preference ({{bug("779297")}}).</li>
 <li>Some IME related keys on Windows are supported by <code>KeyboardEvent.key</code> ({{bug("865565")}}), see <a href="/en-US/docs/Web/API/KeyboardEvent#keyname_table_win">the key name table</a> for the detail.</li>
 <li>Firefox for Metro now dispatches key events in the same way as the desktop version ({{bug("843236")}}).</li>
 <li><code>keypress</code> event is no longer dispatched if <code>preventDefault()</code> of preceding <code>keydown</code> event is called ({{bug("501496")}}), see <a href="/en-US/docs/Web/API/Document/keydown_event#preventdefault()_of_keydown_event">the document of <code>keydown</code> event</a> for the detail.</li>
