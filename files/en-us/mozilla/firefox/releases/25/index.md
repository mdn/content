...
- Support for a non-standard Mozilla-only media query to determine the operating system version has been added: `-moz-os-version` ([Firefox bug 810399](https://bugzil.la/810399)). The property is currently only implemented on Windows.
- The `-moz-osx-font-smoothing` CSS property has been added ([Firefox bug 857142](https://bugzil.la/857142)).
- Our experimental support for {{cssxref("filter")}} now supports the `hue-rotate()` functional notation ([Firefox bug 897392](https://bugzil.la/897392)). It is still disabled by default.
...
- The machine epsilon constant, that is the smallest representable number that, when added to 1, does not equal 1, is now available as {{jsxref("Global_Objects/Number/EPSILON", "Number.EPSILON")}} ([Firefox bug 885798](https://bugzil.la/885798)).
...
- Some IME-related keys on Windows are supported by `KeyboardEvent.key` ([Firefox bug 865565](https://bugzil.la/865565)), see [the key name table](/en-US/docs/Web/API/UI_Events/Keyboard_event_key_values) for details.
...
- The `createTBody()` method on the {{domxref("HTMLTableElement")}} interface, allowing retrieval of its {{HTMLElement("tbody")}}, is now supported ([Firefox bug 813034](https://bugzil.la/813034)).
...
- Support for the `ParentNode` mixin on {{domxref("Document")}} and {{domxref("DocumentFragment")}} has been added ([Firefox bug 895974](https://bugzil.la/895974)).
