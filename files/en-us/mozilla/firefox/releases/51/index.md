...
- The {{domxref("HTMLImageElement")}} interface and the corresponding {{HTMLElement("img")}} element now support the [`onerror`](/en-US/docs/Web/HTML/Global_attributes#onerror) event handler, sending {{domxref("Element/error_event", "error")}} events to the element whenever [errors occur attempting to load or interpret images](/en-US/docs/Web/API/HTMLImageElement#errors).
