...
<p>Firefox 36 was released on February 24th, 2015. This article lists key changes that are useful not only for web developers, but also Firefox and Gecko developers as well as add-on developers.</p>
...
<ul>
 ...
 <li>The {{cssxref("@font-face/unicode-range", "unicode-range")}} descriptor is now supported by {{cssxref("@font-face")}} ({{bug(475891)}}), but not enabled by default.</li>
 ...
 <li>In <a href="/en-US/docs/Web/HTML/Quirks_Mode_and_Standards_Mode">Quirks mode</a>, the <a href="/en-US/docs/Mozilla_Quirks_Mode_Behavior#Miscellaneous_.26_Style"><code>:active</code> and <code>:hover</code> quiver quirk</a> has been altered to be applied less often: it is now used only on links, only if there are no pseudo-element or other pseudo-class in the element and if it isn't part of a pseudo-class element ({{bug(783213)}}).</li>
...
<ul>
 <li>The <a href="/en-US/docs/Web/JavaScript/New_in_JavaScript/ECMAScript_6_support_in_Mozilla">ECMAScript 2015</a> Symbol data type has been enabled by default (was available in the Nightly channel since version 33) ({{bug(1066322)}}):
 ...
 <li>The old placeholder string <code>"@@iterator"</code> has been replaced with the real ES2015 well-known symbol {{jsxref("Symbol.iterator")}} for the <a href="/en-US/docs/Web/JavaScript/Reference/Iteration_protocols">iterable</a> interface property key ({{bug(918828)}}).</li>
 ...
 <li>The ES2016 method {{jsxref("Array.prototype.includes")}} has been implemented, but for now, it is only enabled in Nightly builds ({{bug(1069063)}}).</li>
...
<ul>
 ...
 <li>The default values of the options for {{domxref("MutationObserver.observe()")}} have been updated to match the latest specification ({{bug(973638)}}).</li>
 ...
 <li>The experimental implementation of Web Animations make progress: the method {{domxref("Animation/play", "AnimationPlayer.play()")}} and {{domxref("Animation/pause", "AnimationPlayer.pause()")}} are now supported ({{bug(1070745)}}), as well as {{domxref("Animation/playState", "AnimationPlayer.playState")}} ({{bug(1037321)}}).</li>
 ...
 <li>The <a href="/en-US/docs/Mozilla/Tech/XPCOM/Reference/Interface/nsIWebBrowserPersist#saveURI()"><code>nsIWebBrowserPersist.saveURI()</code></a> method now requires 8 arguments, in an order incompatible with previous releases.</li>
...
<ul>
 ...
 <li>The <a href="/en-US/docs/Web/HTTP/Headers/Content-Security-Policy#form-action"><code>form-action</code></a> directive of CSP 1.1 is now supported ({{bug(529697)}}).</li>
 <li>In the preferences of Firefox, The <a href="/en-US/docs/Web/HTTP/Headers/DNT">Do not track </a>selection widget is again an on/off switch ({{bug(1071747)}}).</li>
</ul>
...
<ul>
 <li>The <code><a href="en-US/Add-ons/SDK/Low-Level_APIs/test_httpd">sdk/test/httpd</a></code> module was removed in <a href="/en-US/docs/Mozilla/Firefox/Releases/36">Firefox 36</a>, please use the <a href="https://www.npmjs.com/package/addon-httpd">addon-httpd</a> npm module instead.</li>
...
<ul>
 <li><a href="/en-US/docs/Mozilla/JavaScript_code_modules/PromiseUtils.jsm#resolveOrTimeout()">PromiseUtils.resolveOrTimeout</a> is implemented ({{bug(1080466)}}).</li>
 <li><a href="/en-US/docs/Mozilla/JavaScript_code_modules/PromiseUtils.jsm#defer()">PromiseUtils.defer</a> (a replacement for <a href="/en-US/docs/Mozilla/JavaScript_code_modules/Promise.jsm#defer()">Promise.defer</a>) is implemented ({{bug(1093021)}}).</li>
</ul>
...
<dl>
 ...
 <dd>Indicates a content load request initiated by the {{domxref("fetch()")}} method.</dd>
...
<ul>
 <li>Firefox <code>-remote</code> <a href="/en-US/docs/Mozilla/Command_Line_Options">command line option</a> has been removed ({{bug(1080319)}}).</li>
</ul>
