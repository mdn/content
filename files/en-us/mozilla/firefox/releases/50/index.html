...
<ul>
 <li>The default style of {{HTMLElement("bdo")}} now sets {{cssxref("unicode-bidi")}} with the <code>isolate-override</code> value ({{bug(1249497)}}).</li>
 <li>Setting the {{HTMLElement("track")}} element's {{htmlattrxref("src", "track")}} attribute now works correctly ({{bug(1281418)}}).</li>
 <li>The <code>referrerpolicy</code> attribute on {{HTMLElement("area")}}, {{HTMLElement("a")}}, {{HTMLElement("img")}}, {{HTMLElement("iframe")}} and {{HTMLElement("link")}} elements is now available by default ({{bug(1223838)}}, {{bug(1264165)}}).</li>
</ul>
...
<ul>
 ...
 <li><a href="/en-US/docs/Tools/Storage_Inspector#indexeddb">The Storage Inspector now lets you delete individual items from IndexedDB object stores.</a></li>
...
<ul>
 ...
 <li>The cookie prefixes <code>__Host-</code> and <code>__Secure-</code> have been implemented. See {{HTTPHeader("Set-Cookie")}} and {{bug(1283368)}}.</li>
...
<ul>
 <li>The {{htmlattrxref("ping", "a")}} attribute of {{htmlelement("a")}} element now abides by the <code><a href="/en-US/docs/Web/HTTP/Headers/Content-Security-Policy#connect-src">connect-src</a></code> <a href="/en-US/docs/Web/HTTP/Headers/Content-Security-Policy">CSP 1.1 policy directive</a> ({{bug(1100181)}}).</li>
 <li>Support for the <code><a href="/en-US/docs/Web/HTTP/Headers/Content-Security-Policy#sandbox">sandbox</a></code> <a href="/en-US/docs/Web/HTTP/CSP">CSP</a> directive has been added ({{bug(671389)}}).</li>
 ...
 <li>The {{domxref("Navigator.sendBeacon()")}} method no longer throws an exception if the beacon data couldn't be sent due to a <a href="/en-US/docs/Web/HTTP/CSP">Content Security Policy</a> restriction; instead, it returns <code>false</code> as expected ({{bug(1234813)}}).</li>
 <li>Support for RC4 encryption was deprecated in Firefox 36 and disabled by default in Firefox 44. The one-year grace period has ended, so Firefox 50 removes all support for RC4 (Google Chrome removed support for RC4 in August 2016). From now on, any time Firefox encounters RC4 encryption, it will report an <code>SSL_ERROR_NO_CYPHER_OVERLAP</code> error.</li>
</ul>
...
<ul>
 ...
 <li>The methods {{domxref("Document.createElement()")}} and {{domxref("Document.createElementNS()")}} now have an optional <code>options</code> parameter for creating <a href="/en-US/docs/Web/Web_Components/Using_custom_elements">custom elements</a> ({{bug(1276579)}}).</li>
</ul>
