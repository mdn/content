...
- The `LocalMediaStream` interface and its `stop()` method have been removed ({{bug(1258143)}}). This method is no longer available with the deprecation of `LocalMediaStream`. See [Stopping a video stream](/en-US/docs/Web/API/MediaStreamTrack/stop#stopping_a_video_stream) to learn how to stop an entire stream.
