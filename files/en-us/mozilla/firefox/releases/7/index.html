...
<ul>
 <li>As part of the <a href="https://blog.mozilla.com/joe/2011/04/26/introducing-the-azure-project/">Azure project</a> the Direct2D Azure Backend <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=651858">has been implemented</a> and will significantly improve the performance of the 2D canvas.</li>
 ...
 <li>Calling <a href="/en-US/docs/Web/API/CanvasRenderingContext2D#drawimage()"><code>drawImage()</code></a> with a zero width or height {{ HTMLElement("canvas") }} now throws <code>INVALID_STATE_ERR</code>.</li>
 <li>Calling <a href="/en-US/docs/Web/API/CanvasRenderingContext2D#drawimage()"><code>drawImage()</code></a> with non-finite coordinates no longer throws an exception.</li>
 ...
 <li><a href="/en-US/docs/Canvas_tutorial/Applying_styles_and_colors#Shadows">Shadows</a> are now only drawn for <code>source-over</code> compositing operations.</li>
 <li>You can now configure the fill rule used by canvas by setting the <code>mozFillRule</code> <a href="/en-US/docs/Web/API/CanvasRenderingContext2D#attributes">attribute</a> on the context.</li>
 ...
 <li>Support for the non-standard methods <code>mozDrawText()</code>, <code>mozMeasureText()</code>, <code>mozPathText()</code> and <code>mozTextAlongPath()</code> has been removed.</li>
</ul>
...
<ul>
 ...
 <li>The {{ cssxref("-moz-orient", "-moz-orient") }} property has been fixed so that {{ HTMLElement("progress") }} elements that are vertically oriented have appropriate default dimensions.</li>
</ul>
...
<ul>
 ...
 <li>Support for <a href="https://www.ctan.org/tex-archive/fonts/Asana-Math/">Asana Math</a> fonts has been added.</li>
...
<ul>
 <li>The {{ domxref("File") }} interface's non-standard methods <code>getAsBinary()</code>, <code>getAsDataURL()</code>, and <code>getAsText()</code> have been removed as well as the non-standard properties <code>fileName</code> and <code>fileSize</code> ({{bug("661876")}}).</li>
 <li>The {{ domxref("FormData", "FormData") }} interface no longer reports the filename as an empty string when sending the <code>Content-Disposition</code> HTTP header if the data was set using a {{ domxref("Blob") }}. This fixes errors that were happening with some servers.</li>
 ...
 <li>In a effort to conform to the upcoming <a href="https://www.w3.org/TR/dom/">DOM4</a> specification where {{ domxref("Attr") }} do not inherit from {{ domxref("Node") }} anymore (it did in DOM Core 1, 2 and 3), many {{ domxref("Node") }} properties and methods on the {{ domxref("Attr") }} interface are <a href="/en-US/docs/Web/API/Attr#deprecated_properties_and_methods">now reporting warnings</a> as we work toward removing them in a later version.</li>
...
<ul>
 <li>The <code><a href="/en-US/docs/JavaScript/Reference/Global_Objects/Function/arity">Function.arity</a></code> property has been removed; use <code><a href="/en-US/docs/Web/JavaScript/Reference/Global_Objects/Function/length">Function.length</a></code> instead.</li>
</ul>
...
<ul>
 ...
 <li>The underlying WebSocket protocol version 8 (as specified by <a href="https://datatracker.ietf.org/doc/html/draft-ietf-hybi-thewebsocketprotocol-10">IETF draft 10</a>) is used now instead of the version 7 protocol used by Firefox 6.</li>
...
<ul>
 <li>Message logged with <code>console.log</code> while the <a href="/en-US/docs/Tools/Web_Console">web console</a> isn't open are still logged, although they aren't displayed when the web console is opened.</li>
</ul>

<h3 id="Web_timing">Web timing</h3>

<ul>
 <li>Initial implementation of the <a href="/en-US/docs/Web/API/Navigation_timing_API">Navigation Timing</a> specification which provides data that can be used to measure the performance of a website.</li>
</ul>

<h3 id="XML">XML</h3>

<ul>
 <li>In addition to the previously supported <code>text/xsl</code>, XSLT stylesheets can now use the official internet media (MIME) type <code>application/xslt+xml</code> (in the <a href="https://www.w3.org/TR/xml-stylesheet/">stylesheet processing instruction</a> or the <a href="https://datatracker.ietf.org/doc/html/rfc5988">HTTP Link header field</a>).</li>
</ul>

<h2 id="Changes_for_Mozilla_and_add-on_developers">Changes for Mozilla and add-on developers</h2>

<p>These changes affect add-on developers as well as developers working on or with Mozilla code itself. Add-on developers should see <a href="/en-US/docs/Mozilla/Firefox/Updating_extensions_for_Firefox_7">Updating extensions for Firefox 7</a> for additional information.</p>

<div class="note">
  <p><strong>Note:</strong> Firefox 7 requires that binary components be recompiled, as do all major releases of Firefox. See <a href="/en-US/docs/Mozilla/Developer_guide/Interface_Compatibility#binary_interfaces">Binary Interfaces</a> for details.</p>
</div>
...
<ul>
 <li>The Add-on Manager has new methods for managing lists of add-ons that changed during applications startup: <code><a href="/en-US/docs/Mozilla/Add-ons/Add-on_Manager/AddonManager#addstartupchange()">AddonManager.addStartupChange()</a></code>, <code><a href="/en-US/docs/Mozilla/Add-ons/Add-on_Manager/AddonManager#removestartupchange()">AddonManager.removeStartupChange()</a></code>, and <code><a href="/en-US/docs/Mozilla/Add-ons/Add-on_Manager/AddonManager#getstartupchanges()">AddonManager.getStartupChanges()</a></code>.</li>
</ul>
...
<ul>
 ...
 <li>{{ xulelem("panel") }} elements can now be configured to <a href="/en-US/docs/XUL/PopupGuide/Panels#Letting_panels_be_dragged_by_grabbing_the_background">let the user drag them by clicking anywhere on their background</a> by using the new {{ XULAttr("backdrag") }} attribute.</li>
</ul>

<h3 id="XPCOM">XPCOM</h3>

<ul>
 <li>The new <a href="/en-US/docs/Components.utils.schedulePreciseGC"><code>Components.utils.schedulePreciseGC()</code></a> method lets you schedule a thorough garbage collection cycle to occur at some point in the future when no JavaScript code is executing; a callback is executed once collection is complete.</li>
 <li>The <a href="/en-US/docs/Components.utils.unload"><code>Components.utils.unload()</code></a> method lets you unload JavaScript code modules previously loaded by calling <a href="/en-US/docs/Components.utils.load"><code>Components.utils.load()</code></a>.</li>
</ul>
...
<ul>
 <li>Extension options can now be <a href="/en-US/docs/Extensions/Inline_Options">displayed inside the Add-on Manager</a> for both restartless and traditional extensions.</li>
 <li>The destination of downloads is now remembered on a site-by-site basis. This data can be accessed using <a href="/en-US/docs/JavaScript_code_modules/DownloadLastDir.jsm">DownloadLastDir.jsm</a>.</li>
</ul>
...
<ul>
 ...
 <li>{{ interface("nsITelemetry") }} has a new method, {{ ifmethod("nsITelemetry","getHistogramById") }} which returns a histogram by its ID, and a new attribute, <code>canRecord</code> which when set to <code>false</code> disables recording of telemetry statistics. Telemetry statistics are no longer recorded when in Private Browsing Mode. (see {{ bug("661574") }} and {{ bug("661573") }}) Telemetry histograms defined with {{ ifmethod("nsITelemetry","newHistogram") }} will not be reported in the telemetry ping.</li>
...
<dl>
 ...
 <dd>Describes a single font face.</dd>
 <dt>{{ interface("nsIDOMFontFaceList") }}</dt>
 <dd>Describes a list of font faces, each represented by {{ interface("nsIDOMFontFace") }}.</dd>
</dl>
