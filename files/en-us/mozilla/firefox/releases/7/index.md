...
- The {{ HTMLElement("body") }} element's [`background`](/en-US/docs/Web/HTML/Reference/Element/body#background) attribute is no longer resolved as a URI; this is in compliance with the current HTML specification.
- The {{ HTMLElement("option") }} element's [`label`](/en-US/docs/Web/HTML/Reference/Element/option#label) attribute now reflects the value of the element's text content if the attribute isn't specified.
