...
<p>This article provides information about the changes in Firefox 93 that will affect developers. Firefox 93 was released on October 5, 2021.</p>

<div class="note">
  <p><strong>Note:</strong> See also <a href="https://hacks.mozilla.org/2021/10/lots-to-see-in-firefox-93/">Lots to see in Firefox 93</a> on Mozilla Hacks.</p>
</div>
...
<ul>
  <li>The ARIA <code><a href="/en-US/docs/Web/Accessibility/ARIA/Roles/meter_role">meter</a></code> role has been implemented ({{bug(1727616)}}).</li>
  <li>The UI for <a href="/docs/Web/HTML/Element/input/datetime-local"><code>&lt;input type="datetime-local"&gt;</code></a> has been implemented. ({{bug(1283388)}}).</li>
</ul>

<h3 id="CSS">CSS</h3>

<ul>
  <li>The <code>small-caps</code> keyword is now supported for the {{cssxref("font-synthesis")}} property ({{bug(1706080)}}).</li>
</ul>

<h3 id="JavaScript">JavaScript</h3>

<ul>
  <li><a href="/en-US/docs/Web/JavaScript/Reference/Classes/Class_static_initialization_blocks">Class <code>static</code> initialization blocks</a> are now supported, allowing more flexible initialization of {{jsxref("Classes/static", "static")}} properties ({{bug(1725689)}}).</li>
  <li>The properties <code>imageOrientation</code> and <code>premultiplyAlpha</code> can be passed to the method {{domxref("createImageBitmap()")}} using the <code>options</code> object ({{bug(1367251)}}).</li>
  <li>{{jsxref("Intl.supportedValuesOf()")}} is now supported, allowing code to enumerate values supported by an implementation, This might be used, for example, to download a polyfill for just the missing category of values ({{bug(1670033)}}).</li>
</ul>
...
<ul>
  <li>The {{domxref("GlobalEventHandlers.onsecuritypolicyviolation","onsecuritypolicyviolation")}} global event handler property is now supported.
  This can be used to assign a handler for processing <a href="/en-US/docs/Web/API/Element/securitypolicyviolation_event"><code>securitypolicyviolation</code></a> events fired when there is a <a href="/en-US/docs/Web/HTTP/CSP">Content Security Policy</a> violation ({{bug(1727302)}}).</li>
  <li>The <code>onslotchange</code> event handler property is now supported on {{domxref("GlobalEventHandlers.onslotchange","GlobalEventHandlers")}} and {{domxref("ShadowRoot.onslotchange","ShadowRoot")}}.
  This can be used to assign a handler for processing <a href="/en-US/docs/Web/API/HTMLSlotElement/slotchange_event"><code>slotchange</code></a> events, which are fired on {{HTMLElement("slot")}} elements when the node(s) contained in the slot change ({{bug(1501983)}}).</li>
</ul>
