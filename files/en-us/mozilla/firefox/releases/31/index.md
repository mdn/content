...
- Changed `var-` prefix of CSS Variables to `--` to reflect the final spec change ([Webkit bug 985838](https://bugzilla.mozilla.org/show_bug.cgi?id=985838)).
- The {{cssxref("hyphens")}} property now support Polish hyphenation rules ([Webkit bug 987668](https://bugzilla.mozilla.org/show_bug.cgi?id=987668)).
- Removed an unwanted white space for multiple of 10,000 in Korean counter styles ([Webkit bug 985186](https://bugzilla.mozilla.org/show_bug.cgi?id=985186)).
- CSS opacity transition broken with parent pseudo :before and overflow auto ([Webkit bug 990340](https://bugzilla.mozilla.org/show_bug.cgi?id=990340)).
- The `::-moz-math-stretchy` pseudo-element has been removed ([Webkit bug 1000879](https://bugzilla.mozilla.org/show_bug.cgi?id=1000879)).

### HTML

- {{HTMLElement("track")}} has been implemented ([Webkit bug 629350](https://bugzilla.mozilla.org/show_bug.cgi?id=629350)).

### JavaScript

New ECMAScript 2015 features implemented:

- New `Array` built-in: {{jsxref("Array.prototype.fill()")}} ([Webkit bug 911147](https://bugzilla.mozilla.org/show_bug.cgi?id=911147))
- New `Math` function: {{jsxref("Math.clz32()")}} ([Webkit bug 925123](https://bugzilla.mozilla.org/show_bug.cgi?id=925123))
- New `String` built-in: {{jsxref("String.prototype.normalize()")}} is available in Firefox Desktop ([Webkit bug 918987](https://bugzilla.mozilla.org/show_bug.cgi?id=918987)).
...
- The ES2015 Proxy {{jsxref("Global_Objects/Proxy/Proxy/isExtensible", "isExtensible")}} trap have been implemented ([Firefox bug 978235](https://bugzilla.mozilla.org/show_bug.cgi?id=978235)).

### Interfaces/APIs/DOM

- Constructor of `KeyboardEvent` has been implemented ([Webkit bug 930893](https://bugzilla.mozilla.org/show_bug.cgi?id=930893)).
- The Resource Timing API has been implemented (see [Webkit bug 822480](https://bugzilla.mozilla.org/show_bug.cgi?id=822480)).
- `KeyboardEvent.isComposing` attribute has been implemented ([Webkit bug 993234](https://bugzilla.mozilla.org/show_bug.cgi?id=993234)).
- `InputEvent` interface has been implemented ([Webkit bug 993253](https://bugzilla.mozilla.org/show_bug.cgi?id=993253)).
- `InputEvent.isComposing` attribute has been implemented ([Webkit bug 993253](https://bugzilla.mozilla.org/show_bug.cgi?id=993253)).
- {{domxref("CSS.escape", "CSS.escape()")}} has been implemented ([Webkit bug 955860](https://bugzilla.mozilla.org/show_bug.cgi?id=955860)).
- {{domxref("Element/mousemove_event", "mousemove")}} is now cancelable like in other browsers ([Webkit bug 704423](https://bugzilla.mozilla.org/show_bug.cgi?id=704423)). Calling `preventDefault()` only sets `defaultPrevented` attribute to `true;` any other behaviors are not changed. E.g., it cannot prevent to set `:hover` state.
...
- The `Connection` interface has been renamed to {{domxref("NetworkInformation")}} and has been modified to match the new specification ([Firefox bug 960426](https://bugzilla.mozilla.org/show_bug.cgi?id=960426)).
...
- Partial implementation of the [OpenType MATH table](https://mpeg.chiariglione.org/standards/mpeg-4/open-font-format/text-isoiec-cd-14496-22-3rd-edition), section 6.3.6 ([Webkit bug 407059](https://bugzilla.mozilla.org/show_bug.cgi?id=407059)). For details, try the [MathML torture test](/en-US/docs/Mozilla/MathML_Project/MathML_Torture_Test) .
- The `::-moz-math-stretchy` pseudo-element has been removed ([Webkit bug 1000879](https://bugzilla.mozilla.org/show_bug.cgi?id=1000879)).
- When available, the Unicode Mathematical alphanumeric characters are used for bold, italic and bold-italic math variants ([Webkit bug 930504](https://bugzilla.mozilla.org/show_bug.cgi?id=930504)).
