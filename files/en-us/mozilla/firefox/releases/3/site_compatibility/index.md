...
In Gecko 1.8, it was not possible to set capturing load event listeners on images. In Gecko 1.9, this has been fixed by [Webkit bug 234455](https://bugzilla.mozilla.org/show_bug.cgi?id=234455). But this can cause problems on websites that incorrectly have their event listeners set to capture the load event. See the discussion in [Webkit bug 335251](https://bugzilla.mozilla.org/show_bug.cgi?id=335251). To fix this problem, the problematic page in question should not set a capturing load event listener.
...
In Gecko 1.8, the `preventBubble` method existed on events to prevent events from bubbling upwards. In Gecko 1.9 this method has been removed. Instead, you should use the standard [stopPropagation()](/en-US/docs/Web/API/Event/stopPropagation), which also works fine in Gecko 1.8. The patch in [Webkit bug 330494](https://bugzilla.mozilla.org/show_bug.cgi?id=330494) made this happen. See also [Webkit bug 105280](https://bugzilla.mozilla.org/show_bug.cgi?id=105280).
