...
<ul>
 ...
 <li>In the same fashion as native terminals, you can now use reverse search to find entries in your JavaScript console history (<code>F9</code> on Windows/Linux or <code>Ctrl</code> + <code>R</code> on macOS, then type a search term, followed by <code>Ctrl</code> + <code>R</code>/<code>Ctrl</code> + <code>S</code> to toggle through results).</li>
...
<ul>
 ...
 <li>{{domxref("createImageBitmap()")}} now supports SVG images ({{domxref("SVGImageElement")}}) as an image source ({{bug(1500768)}}).</li>
</ul>
...
<ul>
 ...
 <li>The <code><a href="/en-US/docs/Web/API/Document/keyup_event">keyup</a></code> and <code><a href="/en-US/docs/Web/API/Document/keydown_event">keydown</a></code> events are now fired during IME composition, to improve cross-browser compatibility for CJKT users ({{bug(354358)}}.</li>
</ul>
...
<ul>
 <li>The {{domxref("WebRTC API", "WebRTC", "", "1")}} {{domxref("RTCIceCandidateStats")}} dictionary has been updated according to the latest spec changes ({{bug(1324788)}}, {{bug(1489040)}}; RTCIceCandidateStats has been updated to the latest spec for more details on exactly what has changed).</li>
...
<ul>
 ...
 <li>The {{event("keypress")}} event is no longer fired for <a href="/en-US/docs/Web/API/KeyboardEvent/keyCode#non-printable_keys_(function_keys)">non-printable keys</a> ({{bug(968056)}}), except for the <code>Enter</code> key, and the <code>Shift</code> + <code>Enter</code> and <code>Ctrl</code> + <code>Enter</code> key combinations (these were kept for cross-browser compatibility purposes).</li>
</ul>
