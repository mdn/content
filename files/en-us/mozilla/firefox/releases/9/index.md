...
- Event handlers are now implemented as standard IDL interfaces. For most cases, this won't affect content, but [there are exceptions](/en-US/docs/Web/Guide/Events/Event_handlers#event_handler_changes_in_firefox_9).
...
- The {{ ifmethod("nsIBrowserHistory", "registerOpenPage") }} and {{ ifmethod("nsIBrowserHistory", "unregisterOpenPage") }} methods have been removed as part of a performance overhaul in the Places system. You can use the corresponding methods in `mozIPlacesAutoComplete` instead.
- The {{ ifmethod("nsIDOMWindowUtils", "wrapDOMFile") }} method has been added; this returns a DOM {{ domxref("File") }} object for a given `nsIFile`.
