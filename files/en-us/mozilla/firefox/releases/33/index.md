...
- The {{HTMLElement("label")}}, especially without a [`for`](/en-US/docs/Web/HTML/Reference/Element/label#for) attribute, doesn't apply anymore to a `<input type=hidden>` field ([Firefox bug 597650](https://bugzil.la/597650)). The previous behavior wasn't spec-compliant.
...
- On Android, support for two new values for the [`name`](/en-US/docs/Web/HTML/Reference/Element/meta#name) attribute of {{HTMLElement("meta")}} has been added: `msapplication-TileImage` and `msapplication-TileColor` ([Firefox bug 1014712](https://bugzil.la/1014712)). Example:
