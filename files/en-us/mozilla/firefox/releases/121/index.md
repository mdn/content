...
- [Lazy loading](/en-US/docs/Web/Performance/Lazy_loading) of `<iframe>`s is now supported, allowing developers to hint that particular `<iframe>`s should only be loaded when (and if) they become visible. This can speed up initial load time by reducing the resources that need to be fetched on page load (some `<iframe>`s may not need to be fetched at all).
