...
- The [date](/en-US/docs/Web/HTML/Element/input/date) and [time](/en-US/docs/Web/HTML/Element/input/time) {{htmlelement("input")}} types are now enabled in all builds ([Firefox bug 1399036](https://bugzilla.mozilla.org/show_bug.cgi?id=1399036)).

### CSS

- The `minimal-ui` and `standalone` values of the [`display-mode`](/en-US/docs/Web/CSS/@media/display-mode) media query are now supported ([Firefox bug 1369815](https://bugzilla.mozilla.org/show_bug.cgi?id=1369815)). See also the [Web app manifest `display` field](/en-US/docs/Web/Manifest#display).
- The `grid-row-gap` and `grid-column-gap` properties are no longer reset by the {{CSSxRef("grid")}} shorthand ([Firefox bug 1387410](https://bugzilla.mozilla.org/show_bug.cgi?id=1387410)).
- The `layout.css.clip-path-shapes.enabled` preference has been removed ([Firefox bug 1399767](https://bugzilla.mozilla.org/show_bug.cgi?id=1399767)). This preference allowed disabling the {{CSSxRef("&lt;basic-shape&gt;")}} support in {{CSSxRef("clip-path")}}. This support was shipped in Firefox 54 and can no longer be disabled.

#### Quantum CSS notes

Following bugs have been fixed in Quantum:

- Radial gradient values like `radial-gradient(circle gold,red)` will work in the old Gecko style system, even though they shouldn't because of the missing comma between `circle` and `gold` ([Firefox bug 1383323](https://bugzilla.mozilla.org/show_bug.cgi?id=1383323)).
- When you animate an offscreen element onscreen but specify a delay, Gecko does not repaint on some platforms, e.g. Windows ([Firefox bug 1383239](https://bugzilla.mozilla.org/show_bug.cgi?id=1383239)).
- In Gecko, {{htmlelement("details")}} elements can't be made open by default using the `open` attribute if they have an {{CSSxRef("animation")}} active on them ([Firefox bug 1382124](https://bugzilla.mozilla.org/show_bug.cgi?id=1382124)).
- In Gecko, {{CSSxRef("transition", "transitions")}} will not work when transitioning from a {{CSSxRef("text-shadow")}} with a color specified to a `text-shadow` without a color specified ([Firefox bug 726550](https://bugzilla.mozilla.org/show_bug.cgi?id=726550)).
- In Gecko, cancelling a filling animation (e.g. with `animation-fill-mode: forwards` set) can trigger a transition set on the same element, although only once (see [Firefox bug 1192592](https://bugzilla.mozilla.org/show_bug.cgi?id=1192592) and [these test cases](https://bug1192592.bmoattachments.org/attachment.cgi?id=8843824) for more information). In general declarative animations should not trigger transitions.
- Animations using em units are not affected by changes to the {{CSSxRef("font-size")}} on the animated element's parent in Gecko, whereas they should be ([Firefox bug 1254424](https://bugzilla.mozilla.org/show_bug.cgi?id=1254424)).
- Gecko also deals with `font-size` inheritance differently from Quantum CSS, meaning that for some language settings inherited font sizes end up being smaller than expected (see [Firefox bug 1391341](https://bugzilla.mozilla.org/show_bug.cgi?id=1391341)).
- Gecko reuses the same mechanism used when parsing a url-token when parsing the `domain()` or `url-prefix()` URL matching functions for a {{CSSxRef("@document", "@-moz-document")}} rule. Quantum CSS does not use the same mechanism and it does not consider tokens invalid when they contain brackets or quotes ([Firefox bug 1362333](https://bugzilla.mozilla.org/show_bug.cgi?id=1362333)).
- In Gecko, when you set a system font as the value of a canvas 2D context's {{DOMxRef("CanvasRenderingContext2D.font", "font")}} (e.g. `menu`), getting the font value fails to return the expected font (it returns nothing). This has been fixed in Quantum. ([Firefox bug 1374885](https://bugzilla.mozilla.org/show_bug.cgi?id=1374885)).
- In Gecko, when you create a detached subtree (e.g. a {{htmlelement("div")}} created using {{DOMxRef("Document.createElement","createElement()")}} that is not yet inserted into the DOM), the subtree's root element is set as a block-level element. In Quantum CSS this is set as inline, as per spec ([Firefox bug 1374994](https://bugzilla.mozilla.org/show_bug.cgi?id=1374994)).
- In Gecko, {{CSSxRef("calc", "calc()")}} expressions are rejected — causing the value to be invalid — when used as the radius component of a {{CSSxRef("radial-gradient", "radial-gradient()")}} function ([Firefox bug 1376019](https://bugzilla.mozilla.org/show_bug.cgi?id=1376019)).
- In Gecko, `calc(1*2*3)` is not parsed successfully; Quantum CSS fixes this ([Firefox bug 1379467](https://bugzilla.mozilla.org/show_bug.cgi?id=1379467)).
- In Quantum CSS, [`calc()` is supported everywhere that the spec explains it should be](https://drafts.csswg.org/css-values-3/#calc-notation) ([Firefox bug 1350857](https://bugzilla.mozilla.org/show_bug.cgi?id=1350857)). In Gecko it is not.
- Gecko has a bug whereby the {{CSSxRef("::before")}} and {{CSSxRef("::after")}} pseudo-elements are still generated even if the {{CSSxRef("content")}} property value is set to `normal` or `none`. As per spec, they shouldn't be ([Firefox bug 1387931](https://bugzilla.mozilla.org/show_bug.cgi?id=1387931)).
- Another Gecko bug means that the {{CSSxRef("background-position")}} property can't be transitioned between two values containing different numbers of {{CSSxRef("&lt;position&gt;")}} values, for example `background-position: 10px 10px;` and `background-position: 20px 20px, 30px 30px;` (see [Firefox bug 1390446](https://bugzilla.mozilla.org/show_bug.cgi?id=1390446)).
...
- The non-standard {{JSxRef("Statements/for_each...in", "for each...in")}} (E4X) loop has been removed. Please use {{JSxRef("Statements/for...of", "for...of")}} instead and see [Warning: JavaScript 1.6's for-each-in loops are deprecated](/en-US/docs/Web/JavaScript/Reference/Errors/For-each-in_loops_are_deprecated) for migration help. ([Firefox bug 1083470](https://bugzilla.mozilla.org/show_bug.cgi?id=1083470)).
- The {{JSxRef("Object.prototype.watch()")}} and {{JSxRef("Object.unwatch", "unwatch()")}} methods are deprecated, will now throw a warning when used, and will be removed soon ([Firefox bug 934669](https://bugzilla.mozilla.org/show_bug.cgi?id=934669)).
- The non-standard {{JSxRef("Iterator")}} and {{JSxRef("StopIteration")}} objects as well as the legacy iteration protocol have been removed ([Firefox bug 1098412](https://bugzilla.mozilla.org/show_bug.cgi?id=1098412)).
- Async generator is now enabled ([Firefox bug 1352312](https://bugzilla.mozilla.org/show_bug.cgi?id=1352312)).
- for await (... of ...) syntax is now enabled ([Firefox bug 1352312](https://bugzilla.mozilla.org/show_bug.cgi?id=1352312)).

### APIs

#### New APIs

- The {{DOMxRef("PerformanceObserver")}} API is now enabled by default ([Firefox bug 1386021](https://bugzilla.mozilla.org/show_bug.cgi?id=1386021)).
- The {{DOMxRef("AbortController")}} and {{DOMxRef("AbortSignal")}} interfaces (known as the Abort API) have been added, allowing DOM requests (such as [fetch requests](/en-US/docs/Web/API/fetch)) to be aborted if desired ([Firefox bug 1378342](https://bugzilla.mozilla.org/show_bug.cgi?id=1378342)).
- \[2] The [Storage API](/en-US/docs/Web/API/Storage_API) is implemented and enabled by default ([Firefox bug 1399038](https://bugzilla.mozilla.org/show_bug.cgi?id=1399038)).

#### DOM

- The {{DOMxRef("Selection.type")}} property of the [Selection API](/en-US/docs/Web/API/Selection) is now implemented ([Firefox bug 1359157](https://bugzilla.mozilla.org/show_bug.cgi?id=1359157)).
- {{DOMxRef("Document.createEvent", "Document.createEvent('FocusEvent')")}} is now supported ([Firefox bug 1388069](https://bugzilla.mozilla.org/show_bug.cgi?id=1388069)).
- The `files` property of the {{DOMxRef("HTMLInputElement")}} interface is now settable ([Firefox bug 1384030](https://bugzilla.mozilla.org/show_bug.cgi?id=1384030)).
- The `HTMLDocument.getSelection()` method has been moved to the {{DOMxRef("Document/getSelection","Document")}} interface so it is available to XML documents ([Firefox bug 718711](https://bugzilla.mozilla.org/show_bug.cgi?id=718711)).
- The `messageerror` event is now implemented, and can have code run in response to it firing via event handlers implemented on message targets — see {{DOMxRef("MessagePort.messageerror_event")}}, {{DOMxRef("DedicatedWorkerGlobalScope.messageerror_event")}}, {{DOMxRef("Worker.messageerror_event")}}, {{DOMxRef("BroadcastChannel.messageerror_event")}}, and {{DOMxRef("Window.messageerror_event")}} ([Firefox bug 1359017](https://bugzilla.mozilla.org/show_bug.cgi?id=1359017)).
- When {{DOMxRef("Headers")}} values are iterated over, they are automatically sorted in lexicographical order, and values from duplicate header names are combined ([Firefox bug 1396848](https://bugzilla.mozilla.org/show_bug.cgi?id=1396848)).
...
- Updated the documentation around {{DOMxRef("OfflineAudioContext")}} since its constructor's inputs can now be specified in an object rather than as a list of parameters ([Firefox bug 1388591](https://bugzilla.mozilla.org/show_bug.cgi?id=1388591)).
- The Web Audio API now properly supports multi-channel output ([Firefox bug 1378070](https://bugzilla.mozilla.org/show_bug.cgi?id=1378070)).

### Security

- `resource://` URLs no longer leak information ([Firefox bug 863246](https://bugzilla.mozilla.org/show_bug.cgi?id=863246))
- Data URLs are now treated as unique opaque origins, rather than inheriting the origin of the settings object responsible for the navigation ([Firefox bug 1324406](https://bugzilla.mozilla.org/show_bug.cgi?id=1324406)).
...
- Firefox [headless mode](/en-US/docs/Mozilla/Firefox/Headless_mode) now includes a `-screenshot` flag that allows you to take website screenshots directly from the command line ([Firefox bug 1378010](https://bugzilla.mozilla.org/show_bug.cgi?id=1378010)).

## Removals from the web platform

### HTML

- `<link rel="preload">` (see [Preloading content with rel="preload"](/en-US/docs/Web/HTML/Attributes/rel/preload)) has been disabled in Firefox 57 because of various web compatibility issues (e.g. [Firefox bug 1405761](https://bugzilla.mozilla.org/show_bug.cgi?id=1405761)). An improved version that works for non-cacheable resources is expected to land in Firefox 58.

### APIs

- Mozilla's proprietary [Social API](/en-US/docs/Archive/Social_API) has been completely removed ([Firefox bug 1388902](https://bugzilla.mozilla.org/show_bug.cgi?id=1388902)).
