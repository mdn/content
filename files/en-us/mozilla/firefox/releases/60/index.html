...
<p>This article provides information about the changes in Firefox 60 that will affect developers. Firefox 60 was released on May 9, 2018.</p>

<h2 id="Stylo_comes_to_Firefox_for_Android_in_60">Stylo comes to Firefox for Android in 60</h2>

<p><a href="https://hacks.mozilla.org/2017/08/inside-a-super-fast-css-engine-quantum-css-aka-stylo/">Firefox's new parallel CSS engine</a> — also known as <strong>Quantum CSS</strong> or <strong>Stylo</strong> — which was <a href="/en-US/docs/Mozilla/Firefox/Releases/57#firefox_57_firefox_quantum">first enabled by default in Firefox 57 for desktop</a>, has now been enabled in Firefox for Android.</p>
...
<ul>
 <li>In the CSS Pane rules view (see <a href="/en-US/docs/Tools/Page_Inspector/How_to/Examine_and_edit_CSS">Examine and edit CSS</a>), the keyboard shortcuts for precise value increments (increase/decrease by 0.1) have changed from <code>Alt</code> + <code>Up</code>/<code>Down</code> to <code>Ctrl</code> + <code>Up</code>/<code>Down</code> on Linux and Windows, to avoid clashes with default OS-level shortcuts (see {{bug("1413314")}}).</li>
 <li>Also in the CSS Pane rules view, <a href="/en-US/docs/Web/CSS/Using_CSS_custom_properties">CSS variable names</a> will now auto-complete ({{bug(1422635)}}). If you enter <code>var(</code> into a property value and then type a dash (<code>-</code>), any variables you have declared in your CSS will then appear in an autocomplete list.</li>
...
<ul>
 <li>ECMAScript 2015 modules have been enabled by default in ({{bug(1438139)}}). See <a href="https://hacks.mozilla.org/2015/08/es6-in-depth-modules/">ES6 In Depth: Modules</a> and <a href="https://hacks.mozilla.org/2018/03/es-modules-a-cartoon-deep-dive/">ES modules: A cartoon deep dive</a> for more information, or consult MDN reference docs:

  <ul>
   <li><a href="/en-US/docs/Web/HTML/Element/script#attr-type"><code>&lt;script src="main.js" type="module"&gt;</code></a> and <a href="/en-US/docs/Web/HTML/Element/script#attr-nomodule"><code>&lt;script nomodule src="fallback.js"&gt;</code> </a></li>
 ...
 <li>The {{jsxref("Array.prototype.values()")}} method has been added again ({{bug(1420101)}}). Make sure your code doesn't have any custom implementation of this method.</li>
</ul>
...
<ul>
 <li>The <a href="/en-US/docs/Web/API/Web_Authentication_API">Web Authentication API</a> has been enabled in ({{bug(1432542)}}).</li>
</ul>
...
<ul>
 ...
 <li>The <code>dom.workers.enabled</code> pref has been removed, meaning workers can no longer be disabled since ({{bug(1434934)}}).</li>
 ...
 <li><a href="/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise">Promises</a> can now be used within <a href="/en-US/docs/Web/API/IndexedDB_API">IndexedDB</a> code ({{bug(1193394)}}).</li>
</ul>
...
<ul>
 ...
 <li>As per spec, the {{domxref("ConvolverNode.ConvolverNode","ConvolverNode()")}} constructor now throws a <code>NotSupportedError</code> {{domxref("DOMException")}} if the referenced {{domxref("AudioBuffer")}} does not have 1, 2, or 4 channels ({{bug(1443228)}}).</li>
...
<ul>
 <li>If the <code>privacy.resistFingerprinting</code> preference is set to <code>true</code>, the {{domxref("WEBGL_debug_renderer_info")}} WebGL extension will be disabled from now on ({{bug(1337157)}}).</li>
</ul>
...
<ul>
 <li><code>SameSite</code> cookies are now supported ({{bug(795346)}}). See {{HTTPHeader("Set-Cookie")}} for more information.</li>
</ul>
