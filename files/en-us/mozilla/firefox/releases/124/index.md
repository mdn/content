...
- Implemented the [storage.getCookies](https://w3c.github.io/webdriver-bidi/#command-storage-getCookies) command, which allows users to retrieve cookies. Users can optionally provide `partition` information to retrieve partitioned cookies, as well as `filter` options to only return cookies matching specific criterias. ([Firefox bug 1854580](https://bugzil.la/1854580))
- Implemented the [storage.setCookie](https://w3c.github.io/webdriver-bidi/#command-storage-setCookie) command, which creates new cookies. Users can provide information about the cookie in the `cookie` parameter, and optionally a `partition` parameter to build the partition key of the partition which should own the cookie. ([Firefox bug 1854582](https://bugzil.la/1854582))
...
- Added support for the "contexts" argument to the [script.addPreloadScript](https://w3c.github.io/webdriver-bidi/#command-script-addPreloadScript) command, this allows clients to add preload scripts only to specific browsing contexts. ([Firefox bug 1858458](https://bugzil.la/1858458))
...
- Fixed an issue with [Get Element Text](https://www.w3.org/TR/webdriver2/#get-element-text), which ignored the slot value for web component without custom text. ([Firefox bug 1865381](https://bugzil.la/1865381))
