...
- The implicit CSS filter applied to [`pageAction`](/en-US/docs/Mozilla/Add-ons/WebExtensions/API/pageAction) SVG icons on dark themes (`about:config` preferenca `extensions.webextensions.pageActionIconDarkModeFilter.enabled` is set to `true` or not defined) that reduce the contrast in icons using multiple colors is disabled in Firefox Desktop Nightly. ([Firefox bug 2001318](https://bugzil.la/2001318)) It will be disabled on the release channel as part of [Firefox bug 2016509](https://bugzil.la/2016509), which is planned for Firefox 152.
