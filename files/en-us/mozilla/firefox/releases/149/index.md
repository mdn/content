...
- Up to Firefox 148, `structuredClone.call(iframe.contentWindow)` incorrectly created objects in the caller's [realm](/en-US/docs/Web/JavaScript/Reference/Execution_model#realms) instead of the iframe's realm. The implementation now instantiatet objects in the `this` realm, so the method's behavior more closely matched the specification.
...
- The implementation of {{domxref("structuredClone")}} has changed to instantiate objects in the `this` realm, so the method's behavior more closely matches the specification. This change would mean that calls in [WebExtensions' content scripts](/en-US/docs/Mozilla/Add-ons/WebExtensions/Content_scripts) would always clone into the content script's realm. To enable content scripts to determine where values are cloned, they can call `window.structuredClone(value)` to clone values in the page's realm and `globalThis.structuredClone(value)` to clone into the content script's realm.
