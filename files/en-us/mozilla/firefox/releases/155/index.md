...
- Fixed the `Perform Actions` command so that a <kbd>Ctrl</kbd> + double click fires a `dblclick` event, rather than two `click` events. ([Firefox bug 2058556](https://bugzil.la/2058556)).
- Native asynchronous touch events are now dispatched at the widget level instead of being synthesized as DOM events, which makes it possible to automate touch scrolling. ([Firefox bug 1797215](https://bugzil.la/1797215)).

#### WebDriver BiDi

- Removed support for the `contexts` argument of the `session.unsubscribe` command, following [w3c/webdriver-bidi#998](https://github.com/w3c/webdriver-bidi/pull/998). Note that this is a breaking change: clients that still pass `contexts` need to unsubscribe using subscription ids instead. ([Firefox bug 1988723](https://bugzil.la/1988723)).
- Subscriptions are now removed from the session's subscription map when the browsing context they belong to is destroyed. ([Firefox bug 1943359](https://bugzil.la/1943359)).
- Fixed a bug where the `browsingContext.reload` command failed with a "Browsing context got discarded" error when used on an iframe, regardless of the `wait` argument. ([Firefox bug 2030909](https://bugzil.la/2030909)).
- Updated the `moz:debugging` capability to set breakpoints recursively on child scripts, so that breakpoints inside functions compiled as child scripts still apply after a reload or when scripts are loaded dynamically. ([Firefox bug 2058426](https://bugzil.la/2058426)).
- Fixed a bug where triggering a download moved the focus away from the browsing context. ([Firefox bug 2035439](https://bugzil.la/2035439)).
- WebDriver BiDi debugging no longer shares the nested event loop API with the developer tools, so both can be driven in parallel without conflicting. ([Firefox bug 2041335](https://bugzil.la/2041335)).
