...
<p>This article provides information about the changes in Firefox 69 that will affect developers. Firefox 69 was released on September 3, 2019.</p>
...
<ul>
 <li><a href="/en-US/docs/Tools/Debugger/Set_event_listener_breakpoints">Event Listener Breakpoints</a> let you diagnose which code a page executes in response to browser events. You can pick specific types, such as <code>click</code> or <code>keydown</code>, or whole categories of events, like all mouse input events. ({{bug(1526082)}}).</li>
 <li>Scripts shown in the debugger's  <a href="/en-US/docs/Tools/Debugger/UI_Tour#source_list_pane">source list pane</a> can now be saved via the <em>Download file</em> context menu option ({{bug(888161)}}).</li>
 <li>In the debugger's source list pane, loaded extensions are listed with their name, rather than just their UUID ({{bug(1486416)}}), making it much easier to find the extension code you want to debug.</li>
 <li>The debugger now loads significantly faster via lazy-loading scripts ({{bug(1527488)}}).</li>
</ul>

<h4 id="Console">Console</h4>

<ul>
 <li><a href="/en-US/docs/Tools/Browser_Console">Browser Console</a> messages from <a href="/en-US/docs/Mozilla/Firefox/Privacy/Tracking_Protection">tracking protection errors</a>, <a href="/en-US/docs/Web/HTTP/CSP">CSP errors</a>, and <a href="/en-US/docs/Web/HTTP/CORS/Errors">CORS errors</a> are grouped automatically to reduce noise from repeated blocked resources and storage access ({{bug(1522396)}}).</li>
 <li>All visible logs in the console can shared by saving to a file or copying to clipboard via a new <em>Export visible messages to</em> context menu item ({{bug(1517728)}}).</li>
 <li>The console's toolbar now responsively reduces its height into a single row to save vertical space ({{bug(972530)}}).</li>
 <li>Messages from content can now be hidden in the console to focus on logs from the Firefox UI ({{bug(1523842)}}).</li>
</ul>

<h4 id="Network">Network</h4>

<ul>
 <li>Resources that got blocked because of <a href="/en-US/docs/Web/HTTP/CSP">CSP</a> or <a href="/en-US/docs/Web/Security/Mixed_content">Mixed Content</a> are now shown in the Network panel, with details of the reason ({{bug(1556451)}}).</li>
 <li>A new optional <em>URL</em> column in the Network panel can be enabled to show the full URL for resources ({{bug(1341155)}}).</li>
</ul>

<h4 id="Inspector">Inspector</h4>

<ul>
 <li>When you hover over an element in the <a href="/en-US/docs/Tools/Page_Inspector">Page Inspector</a>, the infobar that appears now includes the fact that an element is a flex container, or flex item ({{bug(1521188)}}).</li>
 <li>When inspecting a page containing a grid with a subgrid, the parent grid's overlay lines are displayed whenever the subgrid's lines are displayed; if the parent grid's overlay checkbox is unselected, then its lines are translucent ({{bug(1550519)}}).</li>
</ul>

<h4 id="Remote_debugging">Remote debugging</h4>

<ul>
 <li>For our mobile web developers, we have migrated remote debugging from the old WebIDE into a re-designed <a href="/en-US/docs/Tools/about:debugging">about:debugging</a>, making the experience of debugging <a href="https://hacks.mozilla.org/2019/06/geckoview-in-2019/">GeckoView</a> on remote devices via USB much better ({{bug(1462208)}}).</li>
</ul>

<h4 id="General">General</h4>

<ul>
 <li>The DevTools panel order has been changed to reflect popularity ({{bug(1558630)}}).</li>
</ul>
...
<ul>
 <li><a href="/en-US/docs/Web/JavaScript/Reference/Classes#field_declarations">Public class fields</a> are enabled by default ({{bug(1555464)}}). See also <a href="/en-US/docs/Web/JavaScript/Reference/Classes/Public_class_fields">Class fields</a> for more information.</li>
...
<ul>
 ...
 <li>The {{domxref("NavigatorID/userAgent", "navigator.userAgent")}}, {{domxref("NavigatorID/platform", "navigator.platform")}}, and {{domxref("navigator.oscpu")}} properties no longer reveal whether a user is running 32-bit Firefox on a 64-bit OS ({{bug(1559747)}}). They now say <code>Linux x86_64</code> instead of <code>Linux i686 on x86_64</code>, and <code>Win64</code> instead of <code>WOW64</code>.</li>
...
<ul>
 <li>Hacks release post: <a href="https://hacks.mozilla.org/2019/09/firefox-69-a-tale-of-resize-observer-microtasks-css-and-devtools/">Firefox 69 — a tale of Resize Observer, microtasks, CSS, and DevTools</a></li>
</ul>
