...
<ul>
 ...
 <li><a href="/en-US/docs/Tools/Debugger/How_to/Set_Watch_Expressions">See optimized-out variables in the Debugger</a></li>
...
<ul>
 ...
 <li>The implementation of CSS Logical Properties made big progress. The following properties are available behind the {{pref("layout.css.vertical-text.enabled")}} flag (<code>false</code> by default):
  <ul>
   ...
   <li>Direction-independent equivalents of {{cssxref("top")}}, {{cssxref("right")}}, {{cssxref("bottom")}} and {{cssxref("left")}}: {{cssxref("inset-block-start")}}, <code>offset-block-end</code>, <code>offset-inline-start</code> and <code>offset-inline-end</code> ({{bug(1120283)}}).</li>
  </ul>
 </li>
 <li>How <a href="/en-US/docs/Web/CSS/CSS_Transitions/Using_CSS_transitions">CSS transitions</a> start has been modified to match a recent change of the specifications, aiming at having an interoperable behavior between browsers ({{bug(960465)}}).</li>
</ul>
...
<ul>
 ...
 <li>The prefixed version of {{domxref("indexedDB")}}, <code>mozIndexedDB</code> has been removed ({{bug(975699)}}).</li>
 <li>The {{event("DOMContentLoaded")}} event is no longer cancelable ({{bug(1134559)}}).</li>
 <li>You can now use <a href="/en-US/docs/Web/API/WebSockets_API">WebSockets</a> in <a href="/en-US/docs/Web/API/Web_Workers_API">Workers</a> ({{bug(504553)}}).</li>
