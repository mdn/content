...
<p>This article provides information about the changes in Firefox 68 that will affect developers. Firefox 68 was released on July 9, 2019.</p>
...
<ul>
 <li>You can now <a href="/en-US/docs/Tools/Debugger/How_to/Search#searching_in_all_files">Search</a> in all files in the current project from the debugger by pressing <code>Shift</code> + <code>Ctrl</code> + <code>F</code> (Windows or Linux) or <code>Shift</code> + <code>Cmd</code> + <code>F</code> (macOS) ({{bug(1320325)}}).</li>
</ul>
...
<ul>
 ...
 <li>We changed {{CSSxRef("color_value#currentcolor_keyword", "currentColor")}} to be a computed value (except for the {{cssxref("color")}} property)  ({{bug(760345)}}).</li>
...
<ul>
 ...
 <li>The {{domxref("Window")}} feature <code><a href="/en-US/docs/Web/API/Window/open">noreferrer</a></code> is now supported; if specified, the new window's content is loaded without sharing the hostname, IP address, URL, or other identifying information about the host device ({{bug(1527287)}}).</li>
...
<ul>
 ...
 <li>Disabled privileged content processes that caused HTTP authentication dialogs not to appear when navigating to a website after opening a new tab ({{bug(1558763)}}).</li>
</ul>
...
<ul>
 <li>The <code><a href="/en-US/docs/Mozilla/Add-ons/WebExtensions/API/proxy/register">proxy.register()</a></code> and <code><a href="/en-US/docs/Mozilla/Add-ons/WebExtensions/API/proxy/unregister">proxy.unregister()</a></code> functions have been deprecated and will be removed from Firefox 71 ({{bug(1545811)}}).</li>
...
<ul>
 <li>Hacks release post: <a href="https://hacks.mozilla.org/2019/07/firefox-68-bigints-contrast-checks-and-the-quantumbar/">Firefox 68: BigInts, Contrast Checks, and the QuantumBar</a></li>
</ul>
