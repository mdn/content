...
- The CSS {{cssxref("@import")}} [at-rule](/en-US/docs/Web/CSS/At-rule) `supports(…)` conditions are now supported in (CSS) import rules (`@import supports(...)`). Which allows stylesheets to be imported if the included feature is supported in the users browser. ([Firefox bug 1830779](https://bugzil.la/1830779))
