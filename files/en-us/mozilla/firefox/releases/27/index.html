...
tags:
  - Firefox
---
<div>{{FirefoxSidebar}}</div><p>Firefox 27 was released on February 4, 2014. This article lists key changes that are useful not only for web developers, but also Firefox and Gecko developers as well as add-on developers.</p>
...
<ul>
 <li>Breakpoints can now be set on DOM events.</li>
 <li>JavaScript in the debugger panel can be unminified, using the { } button.</li>
 <li>The inspector now has an "edit-element-html" feature, without needing an add-on.</li>
 <li>Background-URLs and colors have preview in inspector. Even hovering over canvas elements will give a pop-up with an image preview.</li>
 ...
 <li>The {{HTTPHeader("SourceMap", "X-SourceMap")}} header is now supported ({{bug(765993)}}).</li>
</ul>
...
<ul>
 ...
 <li>The {{cssxref("all")}} shorthand property has been added ({{bug(842329)}}).</li>
 <li>The {{cssxref("unset")}} global value has been added; it allows to reset any CSS property ({{bug(921731)}}).</li>
 <li>Curly brackets are no longer allowed in HTML <code>style</code> attributes: doing <code>&lt;div style="{ display: none }"&gt;</code> was working in quirks mode, but won't anymore {{bug(915053)}}.</li>
...
<ul>
 ...
 <li>The {{htmlattrxref("typemustmatch", "object")}} attribute on the {{HTMLElement("object")}} element is now supported ({{bug(827160)}}).</li>
</ul>
...
<ul>
 <li>The <a href="/en-US/docs/Web/JavaScript/Reference/Operators/Spread_syntax">spread operator</a> is now supported in Function calls ({{bug("762363")}}).</li>
 <li>The mathematical function {{jsxref("Global_Objects/Math/hypot", "Math.hypot()")}} has been implemented ({{bug("896264")}}).</li>
 <li>The {{jsxref("Operators/yield*", "yield*")}} expression is now implemented ({{bug(666396)}}).</li>
 ...
 <li><a href="/en-US/docs/Web/JavaScript/Reference/Statements/for...of">for..of</a> loops now expect the ES2015 standard <a href="/en-US/docs/Web/JavaScript/Reference/Iteration_protocols">iterator protocol</a> moving away from SpiderMonkey old iterator protocol using <code>StopIteration</code>.</li>
...
<ul>
 ...
 <li>The methods <code>getAllKeys()</code> and <code>openKeyCursor()</code> have been added to {{domxref("IDBObjectStore")}} ({{bug(920633)}} and {{bug(920800)}}).</li>
 <li>The {{domxref("HTMLFormControlsCollection")}} interface has been implemented ({{bug(913920)}}).</li>
 <li>The {{domxref("CanvasRenderingContext2D")}} interface now supports the two methods {{domxref("CanvasRenderingContext2D.getLineDash()", "getLineDash()")}} and {{domxref("CanvasRenderingContext2D.setLineDash()", "setLineDash()")}} and the {{domxref("CanvasRenderingContext2D.lineDashOffset", "lineDashOffset")}} property  ({{bug(768067)}}).</li>
 ...
 <li>The WebRTC API's {{domxref("RTCIceCandidate")}} object now includes a {{domxref("RTCIceCandidate.toJSON", "toJSON()")}} method to help with signaling and debugging ({{bug(928304)}}).</li>
 ...
 <li><a href="/en-US/docs/Web/API/WebGL_API">WebGL</a>: <code>MOZ_</code> prefixed extension strings are deprecated. Support for them will be removed in the future. Use unprefixed extension string only. To get draft extensions, set the <code>webgl.enable-draft-extensions</code> preferences ({{bug(924176)}}).</li>
</ul>
...
<ul>
 <li><a href="https://bugzilla.mozilla.org/buglist.cgi?resolution=FIXED&amp;component=Marionette&amp;product=Testing&amp;target_milestone=mozilla27">List of changes</a> in <a href="https://firefox-source-docs.mozilla.org/testing/marionette/marionette/index.html">Marionette</a> for Firefox 27.</li>
</ul>
