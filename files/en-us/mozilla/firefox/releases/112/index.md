...
- The updated syntax for the `ray()` function value of the [`offset-path`](/en-US/docs/Web/CSS/offset-path) property is now supported. The `<ray_size>` parameter in the `ray()` function is now optional and has a default value of `closest-side` ([Firefox bug 1820071](https://bugzil.la/1820071)).
