...
Strings in JavaScript are represented by unsigned 16-bit integers, and it's possible to have valid string types that are not well-formed UTF-16 Unicode text.
The {{jsxref("String.prototype.isWellFormed()")}} and {{jsxref("String.prototype.toWellFormed()")}} methods are used to check if a string is well-formed UTF-16 Unicode text and to convert a string to well-formed UTF-16 Unicode text, respectively.
