...
You may face limitations of the `activeTab` permission trying to inject [content scripts](/en-US/docs/Mozilla/Add-ons/WebExtensions/Content_scripts) into web pages with {{HTMLElement("iframe", "frames")}}. When the {{webextAPIref("menus/onClicked", "menus.onClicked")}} event is fired in response to a [context menu](/en-US/docs/Mozilla/Add-ons/WebExtensions/user_interface/Context_menu_items) opened inside a frame, the extension receives access to the **top-level frame** of the tab. As a result, the frame specified by the `frameId` field of {{webextAPIref("menus/OnClickData", "menus.OnClickData")}} may be inaccessible for injection of scripts and styles by {{webextAPIref("scripting/executeScript", "scripting.executeScript")}} and {{webextAPIref("scripting/insertCSS", "scripting.insertCSS")}} or by their {{webextAPIref("tabs", "tabs")}} counterparts. Running script in a [cross-origin](/en-US/docs/Web/Security/Same-origin_policy#cross-origin_network_access) frame may require [host permissions](#host_permissions), unless it is a Firefox mv2 add-on. Support of special cases like [`data:` URI](/en-US/docs/Web/HTTP/Basics_of_HTTP/Data_URLs) or {{DOMxRef("URL.createObjectURL", "Blob")}} frames in respect to content scripts is not uniform across browsers, see the Firefox [bug #1411641 comment 41](https://bugzil.la/1411641#c41) for some details.
