...
<p>If you supply a popup, then the popup is opened when the user clicks the button, and your JavaScript running in the popup can handle the user's interaction with it. If you don't supply a popup, then a click event is dispatched to your extension's <a href="/en-US/docs/Mozilla/Add-ons/WebExtensions/Anatomy_of_a_WebExtension#background_scripts">background scripts</a> when the user clicks the button.</p>

<p>You can also create and manipulate browser actions programmatically using the <a href="/en-US/docs/Mozilla/Add-ons/WebExtensions/API/browserAction">browserAction API</a>.</p>
 ...
 <tbody>
  <tr>
   ...
   <td>
    ...
    <p>Use this to include a stylesheet in your popup that will make its look consistent with the browser's UI and with other extensions that use the <code>browser_style</code> property. Although this key defaults to <code>false</code>, it's recommended that you include it and set it to <code>true</code> in order to make your popups consistent with the look of the rest of the browser user interface.</p>
    ...
    <p><a href="/en-US/docs/Mozilla/Add-ons/WebExtensions/user_interface/Browser_styles">Browser styles</a> describes the classes you can apply to elements in the popup in order to get particular styles.</p>
  ...
  <tr>
   ...
   <td>
    ...
    <p>Firefox remembers the <code>default_area</code> setting for an extension, even if that extension is uninstalled and subsequently reinstalled. To force the browser to acknowlege a new value for <code>default_area</code>, the id of the extension must be changed.</p>
...
"default_icon": "path/to/geo.svg"</pre>
    ...
    <p>You cannot specify multiple icons of the same sizes.<br>
     <br>
     See <a href="/en-US/docs/Mozilla/Add-ons/WebExtensions/manifest.json/browser_action#choosing_icon_sizes">Choosing icon sizes</a> for more guidance on this.</p>
   </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>
    ...
    <p>If this property is present, it's an array containing at least one <code>ThemeIcons</code> object. A <code>ThemeIcons</code> object contains three mandatory properties:</p>

    <dl>
     <dt><code>"dark"</code></dt>
     <dd>A URL pointing to an icon. This icon displays when a theme using dark text is active (such as the Firefox Light theme, and the Default theme if no default_icon is specified).</dd>
     <dt><code>"light"</code></dt>
     <dd>A URL pointing to an icon. This icon displays when a theme using light text is active (such as the Firefox Dark theme).</dd>
    ...
    <p>You should supply 16x16 and 32x32 (for retina display) <code>ThemeIcons</code>.</p>
   </td>
  </tr>
 </tbody>
</table>

<h2 id="Choosing_icon_sizes">Choosing icon sizes</h2>

<p>The browser action's icon may need to be displayed in different sizes in different contexts:</p>

<ul>
 <li>The icon is displayed in the browser toolbar. Older versions of Firefox supported the option of placing the icon in the browser's menu panel (the panel that opens when the user clicks the "hamburger" icon). In those versions of Firefox the icon in the menu panel was larger than the icon in the toolbar.</li>
...
<p>For a simple, but complete, extension that uses a browser action, see the <a href="/en-US/docs/Mozilla/Add-ons/WebExtensions/Your_second_WebExtension">walkthrough tutorial</a>.</p>
