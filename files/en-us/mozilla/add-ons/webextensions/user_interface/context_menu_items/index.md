...
You can also override the context menus displayed in extension pages such as custom sidebars and popups to use either the tab or bookmark context menus with {{WebExtAPIRef("menus.overrideContext")}} instead of the default context menu. This is a helpful method when your extension provides a custom presentation of tabs or bookmarks. The menu automatically includes menu items for any other extensions that have defined tab or bookmark context menu items. You can choose whether to include the default context menu items. Hiding the default items enables you to add the appropriate option for a sidebar or popup, as shown in the image below for the Tree Style Tab extension.
