...
- `color`
  - : `string`. The color for the identity. This color is shown in tabs belonging to the identity. For the list of supported values, see {{WebExtAPIRef("contextualIdentities.getSupportedColors()")}}. Use the method in Firefox 153 or later to retrieve the supported list.
- `colorCode`
  - : `string`. A hex code representing the color used for the identity. For example: `"#37adff"`.
- `icon`
  - : `string`. The name of an icon for the identity. This icon is shown in the URL bar for tabs belonging to this identity. For the list of supported values, see {{WebExtAPIRef("contextualIdentities.getSupportedIcons()")}}. Use the method in Firefox 153 or later to retrieve the supported list.
- `iconUrl`
