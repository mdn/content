...
You can use any of the [WebExtensions APIs](/en-US/docs/Mozilla/Add-ons/WebExtensions/API) in the script, if you've requested the necessary [permissions](/en-US/docs/Mozilla/Add-ons/WebExtensions/manifest.json/permissions).
...
All of these are [Extension pages](/en-US/docs/Mozilla/Add-ons/WebExtensions/user_interface/Extension_pages). Unlike a normal web page, your extension's JavaScript can use the same privileged WebExtensions APIs as your background script.

## Extension pages

You can also include HTML documents in your extension that are not attached to some predefined user interface component. Unlike the documents you might provide for sidebars, popups, or options pages, these don't have an entry in `manifest.json`. However, they do also get access to all the same privileged WebExtensions APIs as your background script.
...
- Use a small subset of the [WebExtensions APIs](/en-US/docs/Mozilla/Add-ons/WebExtensions/API).
- [Exchange messages with their background scripts](/en-US/docs/Mozilla/Add-ons/WebExtensions/Content_scripts#communicating_with_background_scripts), which a
  content script can use to request the background script to do something using a WebExtensions API.
...
WebExtensions support internationalization through the `_locales/` directory, which is part of the extension's top-level file structure alongside files such as `manifest.json`, background scripts, and extension pages.

The `_locales/` directory contains one subdirectory per supported locale (for example, `en`, `fr`, or `de`). Each locale directory includes a `messages.json` file that defines translated strings used by the extension.

Internationalization is tightly integrated with other parts of the extension anatomy:

- The `manifest.json` file can reference localized strings from `_locales/` for fields such as the extension name and description.
- Background scripts, extension pages (such as popups, options pages, and sidebars), and content scripts can retrieve localized strings at runtime through the internationalization APIs.
- Localized strings can also be used in extension stylesheets, allowing user interface text to adapt to the user's language.

By placing localization resources in a dedicated `_locales/` directory, extensions can keep translated content separate from logic and layout while still making it accessible throughout the extension.
