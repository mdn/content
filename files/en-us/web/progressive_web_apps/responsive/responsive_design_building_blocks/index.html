...
tags:
  ...
  - Responsive Design
---
<p>In this article we will discuss the main essential components of responsive design, with some links to further information where necessary.</p>
...
<p>There are disadvantages to this approach as well. If the content, layout, and functionality need to change greatly for different devices, it may not be such a good approach. Also, taking an existing site and adding responsiveness to it, to make it mobile/tablet friendly, can be a lot more effort than just creating a separate mobile site or app, especially if it is a sprawling enterprise site. Read more about <a href="/en-US/docs/Web/Progressive_web_apps">responsive design advantages and disadvantages</a>.</p>

<div class="note">
<p><strong>Note:</strong> You can also read our discussion on the basics of <a href="/en-US/docs/Web/Progressive_web_apps">responsive design</a>, if you need some more background information and basics.</p>
</div>
...
<p><img alt="" src="desktop-layout.png"></p>
...
<div class="note">
<p><strong>Note:</strong> These weird x- elements may be unfamiliar; they are part of <a href="https://mozbrick.github.io/">Brick</a>, Mozilla's UI element library for mobile web apps. We have used Brick to create the mobile layout for Snapshot, which you will read more about below.</p>
</div>
...
<p><img alt="" src="broken-images.png"></p>
...
<p><img alt="" src="broken-max-width-layout.png"></p>
...
<div class="note">
<p><strong>Note:</strong> Media queries are a CSS3 feature that allow you to selectively apply CSS depending on the results of media feature tests — for more on the basics, read <a href="/en-US/docs/Web/CSS/Media_Queries/Using_media_queries">Media queries</a>.</p>
</div>
...
<p>In our example, we have a desktop layout, as we've already seen. This is created using the CSS rules included at the top of the stylesheet, before any media queries are encountered.<br>
 <br>
 <img alt="" src="desktop-layout.png"></p>
...
<p><img alt="" src="middle-layout.png"></p>
...
<p>This rule changes the width of the gallery images so now there are two per line.</p>
...
<p><img alt="single column layout for mobile app view, with three buttons to navigate between cards, an image viewer, and a Save Picture button at the button." src="mobile-layout.png">But there are more tricks up our sleeves for this mobile app layout! As mentioned above, we used <a href="https://mozilla.github.io/brick/">Mozilla Brick</a>, a collection of ready-rolled mobile UI components, in the making of the mobile app layout. In particular, we used the <a href="https://mozilla.github.io/brick/docs.html#deck">deck</a> component for the nice transition effect between cards when the buttons are pressed. For more on using Brick, read <a href="/en-US/docs/Web/Apps/app_layout/Mozilla_Brick_ready_made_UI_components">Mozilla Brick: ready made UI components</a>.</p>
...
<p>Another challenge is dealing with high resolution screens — raster graphics designed for low resolutions are in danger of appearing tiny when displayed on a high resolution screen, so devices often apply a default zoom factor to rendered pages to avoid this. The trouble with this, then, is that raster images are zoomed in and as a result can start to look pixelated.</p>

<h3 id="CSS_background_images">CSS background images</h3>

<p>For CSS background images this is a fairly easy problem to solve. If you use the <a href="/en-US/docs/Web/Progressive_web_apps/Responsive/Mobile_first">mobile first</a> methodology, you will be creating your mobile layout inside your default CSS, before any media queries have been applied. The media queries then supply CSS that is only applied to the markup when the viewport is <strong>above</strong> a certain width. Let's look at a quick example:</p>
...
<p>HTML images are a more difficult proposition. There is no mechanism inherent in HTML images for serving different image files dependent on viewport size, and, due to a number of irksome browser behavior realities, solutions are more difficult to hack together than you would imagine. There are currently some standards proposals in the works that would provide this — the W3C <a href="https://www.w3.org/community/respimg/">responsive images community group</a> discussed this problem for ages and arrived at the <a href="https://www.w3.org/TR/html-picture-element/">&lt;picture&gt;</a> element, which provides a similar markup structure to {{HTMLElement("video")}}<a href="/en-US/docs/Web/HTML/Element/video">,</a> with {{HTMLElement("source")}} alternatives selectable via media query results. Another proposal, <a href="https://www.w3.org/html/wg/drafts/srcset/w3c-srcset/">srcset</a>, was put forward by Apple and takes a slightly different approach, instead providing a new <code>srcset</code> attribute for {{HTMLElement("img")}} inside which image references are placed along with "hints" that the browser can use to work out which image is most suitable to display given its viewport size, resolution, etc. These are not intended to be mutually exclusive.</p>
...
<ul>
 <li>You should try to use <a href="/en-US/docs/Web/CSS/CSS3">CSS3</a> features to programmatically generate graphical effects where possible, rather than relying on image files. these include rounded corners, gradients, and drop shadows. These scale as the resolution changes or the browser zooms. Although they are not supported very well on older browsers such as Internet Explorer 6-8, this is not too much of a concern when you are creating an interface aimed at modern devices, and they also tend to gracefully degrade.</li>
 <li>You could also try using <a href="/en-US/docs/Web/SVG">SVG</a> to create interface elements. SVG produces vector graphics and is supported well across modern browsers, with polyfills available for older browser support.</li>
 <li>Using <a href="/en-US/docs/Web/CSS/@font-face">Web fonts</a> for displaying icons is an effective technique for keeping file size and HTTP requests down, and this is supported well across modern and older browsers.</li>
</ul>
...
<ul>
 ...
 <li><a href="/en-US/docs/Web/Guide/Mobile">Mobile Web development</a></li>
</ul>
