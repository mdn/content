...
This ensures we have only the files we need in the cache, so we don't leave any garbage behind; the [available cache space in the browser is limited](/en-US/docs/learn/javascript/client-side_web_APIs/browser_storage_limits_and_eviction_criteria), so it is a good idea to clean up after ourselves.
