...
- A {{HTMLElement("meta")}} element with a [name](/en-US/docs/Web/HTML/Reference/Element/meta#name) of `referrer` and the content set to `no-referrer` to disable the Referer header for the whole document. See [Referrer-Policy Integration with HTML](/en-US/docs/Web/HTTP/Reference/Headers/Referrer-Policy#integration_with_html).
