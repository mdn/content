...
- **Attestation preferences**: Whether the RP is interested in authenticator [attestation](#attestation) (a mechanism that helps the RP decide if it should trust the authenticator), and if so, what form the attestation should take.
...
Depending on the authenticator capabilities and the preferences of the RP, the authenticator may ask the user to authorize passkey creation via some [user verification](#user_verification) method: for example, using a biometric such as a fingerprint.
