...
- **Authenticator preferences**: What type of authenticator to use, and whether the authenticator should perform [user verification](#user_verification) before creating the passkey.
...
Next, the browser finds the passkey associated with the current site. The authenticator which stores this passkey will typically ask the user to authorize the use of this passkey, including [user verification](#user_verification) if this is requested by the website and supported by the authenticator.
...
The security of a passkey depends on the reliability of the authenticator used. For example, if an authenticator does not protect the private keys it stores, then an attacker could steal the keys and impersonate users. In _attestation_, an authenticator provides verifiable evidence to the relying website about its origin, and about the data it produces (such as key pairs or signed assertions). This can help a website decide whether it wants to rely on the authenticator to authenticate its users.
...
Additionally, depending on the capabilities of the authenticator and the options provided by the relying website, the authenticator may ask the user to authorize the operation, for example by entering a PIN or a biometric such as a fingerprint. This is called "user verification".
...
The scope of a passkey determines which sites will be able to use the passkey.

By default:

- When a page creates a passkey by calling {{domxref("CredentialsContainer.create()")}}, the browser sets its website ID to the domain component of the caller's {{glossary("origin")}}, and the authenticator stores this value alongside the passkey.

- When a page uses a passkey by calling {{domxref("CredentialsContainer.get()")}}, the browser passes the domain component of the caller's {{glossary("origin")}} to the authenticator, and the authenticator will only allow the passkey to be used if this value matches the stored ID.
...
- When a website creates a passkey, it can pass a website ID into {{domxref("CredentialsContainer.create()")}}, and the browser will pass this to the authenticator. The value must be a {{glossary("registrable domain")}} that is a _domain suffix_ of the domain of the caller's origin.

- Similarly, when a website tries to use a passkey, it can pass an ID into {domxref("CredentialsContainer.get()")}}, and this value follows the same rules.

This means that, for example, a page at `https://register.example.com` may create a passkey with a website ID of `example.com`, and a page at `https://login.example.com` will then be allowed to use that passkey.

These rules help defend against [phishing](/en-US/docs/Web/Security/Attacks/Phishing) attacks. In a phishing attack, the user is presented with a malicious page that looks like the target site, and that asks the user to enter their credentials for the target site. Typically, the URL of the malicious site appears similar to that of the target site, helping to confuse the user. For example, if the target site is `https://example.com`, then the phishing site might be served from `https://examp1e.com`.

With the scope rules for passkeys, though, a site served from `https://examp1e.com` is not able to use passkeys that were created for `https://examp1e.com`.
