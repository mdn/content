...
- [Leaking redirects with a CSP](#leaking_redirects_with_a_csp): in this attack, the attacker's page has a [Content Security Policy](/en-US/docs/Web/HTTP/Guides/CSP) which effectively blocks redirects, then tries to load a page in the target site. If the page is blocked, the attacker knows that the target redirected the request, and again, this may reveal that the user is signed in.
