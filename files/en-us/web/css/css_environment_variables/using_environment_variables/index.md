...
- `safe-area-max-inset-*`
  - : The four safe area maximum inset environment variables - `safe-area-max-inset-top`, `safe-area-max-inset-right`, `safe-area-max-inset-bottom`, and `safe-area-max-inset-left` - each represent the static maximum value of their dynamic `safe-area-inset-*` variable counterpart. On some platforms, the `safe-area-inset-*` values can change dynamically while scrolling. For example, there may be a button bar displayed when scrolling up or down. While the `safe-area-inset-*` values change as the currently visible content area changes, the `safe-area-max-inset-*` values are static. They represent the maximum value of their dynamic counterpart when all dynamic user interface features are retracted, making the layout viewport size the large viewport size.
