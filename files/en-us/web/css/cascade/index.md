...
If any of the layered styles in A, B, or C, have higher specificity selectors targeting a same element, similar to `:root body p { color: black;}`, those declarations are removed from consideration because of _origin_ as normal layered styles have less precedence than normal unlayered styles . If, however, the more specific selector `:root body p { color: black;}` was found in `unlayeredStyles.css`, as both origin and importance are the same, _specificity_ would mean the black declaration had precedence.
