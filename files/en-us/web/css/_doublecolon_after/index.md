...
> **Note:** The pseudo-elements generated by `::before` and `::after` are [contained by the element's formatting box](https://www.w3.org/TR/CSS2/generate.html#before-after-content), and thus don't apply to _[replaced elements](/en-US/docs/Web/CSS/Replaced_element)_ such as {{HTMLElement("img")}}, or to {{HTMLElement("br")}} elements, but you can change default behavior by `display` rule (for example: `img {display: inline-block}`).
