...
The **`:read-only`** [CSS](/en-US/docs/Web/CSS) [pseudo-class](/en-US/docs/Web/CSS/Pseudo-classes) pseudo-class selects elements (such as text `input` or `textarea`) that are not editable by the user. Elements on which the HTML attribute [`readonly`](/en-US/docs/Web/HTML/Attributes/readonly) doesn't have an effect(such as radio, checkboxes, and all other non-form elements) are also selected by the `:read-only` pseudo-class.
