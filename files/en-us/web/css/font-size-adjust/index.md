...
For a given font, the same content in two side-by-side [`<span>`](/en-US/docs/Web/HTML/Reference/Element/span) elements can be used to determine the font's aspect value. If the same font size is used for content in both spans, the spans will match when the `font-size-adjust` value in one span is accurate for the given font.
