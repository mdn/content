...
- `manual` {{Experimental_Inline}}
  - : Has the same effect as `word-break: normal` except that breaks are not automatically inserted in Southeast Asian languages. In such cases, user agents frequently place them in suboptimal positions. This property value allows you to insert line breaks in optimal positions manually. This is not currently supported in any browsers.
