...
<p>The <strong><code>@font-face</code></strong> <a href="/en-US/docs/Web/CSS">CSS</a> <a href="/en-US/docs/Web/CSS/At-rule">at-rule</a> specifies a custom font with which to display text; the font can be loaded from either a remote server or a locally-installed font on the user's own computer.</p>
...
<ul>
 <li>Web fonts are subject to the same domain restriction (font files must be on the same domain as the page using them), unless <a href="/en-US/docs/Web/HTTP/CORS">HTTP access controls</a> are used to relax this restriction.</li>
...
<p>This example specifies a downloadable font to use, applying it to the entire body of the document:</p>
...
<p>The output of this example code looks like so:</p>

<p>{{EmbedGHLiveSample("css-examples/web-fonts/basic-web-font.html", '100%', 50)}}</p>

<h3 id="Specifying_local_font_alternatives">Specifying local font alternatives</h3>
