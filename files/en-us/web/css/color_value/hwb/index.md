...
> **Note:** The computed values of colors expressed using `hwb()` syntax are {{CSSXref("color_value/rgb", "rgb()")}} values in the case of absolute colors, and {{CSSXref("color_value/color", "color(srgb ... )")}} values in the case of relative colors.
...
- `W`, `B`
  ...
  - : Each one is a {{CSSXref("&lt;percentage&gt;")}} or the keyword `none` (equivalent to `0%` in this case). These values represent the color's whiteness and blackness, respectively. They specify the amount of white and black to mix in, from `0%` (no whiteness or blackness) to `100%` (full whiteness or blackness).
    ...
    If `W + B = 100%`, it defines a shade of gray. If `W + B > 100%`, `W` and `B` are effectively normalized as `W / (W + B)` and `B / (W + B)`, respectively.
...
Let's start with an origin color of `hsl(0 100% 50%)` (equivalent to `hwb(0 0% 0%)`). The following function outputs the same color as the origin color — it uses the origin color's `h`, `w`, and `b` channel values as the output channel values:
...
- Sets the output color's `W` channel value to a new value not based on the origin color: `30%`. The output `hwb()` color is `hwb(0 30% 0%)`.

The final output color is the equivalent color in the sRGB color space — `color(srgb 1 0.3 0.3)`.
...
In the following example, the `hsl()` origin color is again converted into an `hwb()` representation — `hwb(0 0% 0%)`. {{cssxref("calc")}} calculations are applied to the `H`, `W`, `B`, and `A` values, and the final output color is `color(srgb 0.25 0.9 0.25 / 0.9)`:
