...
- {{cssxref("&lt;integer&gt;")}}
  - : A value of `1` means that the broken image icon is shown even if the image has an [`alt`](/en-US/docs/Web/HTML/Reference/Element/img#alt) attribute. When the value `0` is used, the image will act as usual and only display the `alt` attribute.
