...
- `safe-area-max-inset-top`, `safe-area-max-inset-right`, `safe-area-max-inset-bottom`, `safe-area-max-inset-left`
  - : The `safe-area-max-inset-*` variables represent values of the above safe area insets when maximum area is available after dynamic UA interfaces are retracted. The max safe area insets are fixed values.
