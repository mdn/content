...
The predefined fallback option values of the `position-try-fallbacks` property (defined as {{cssxref("position-try-fallbacks", "&lt;try-tactic&gt;", "#try-tactic")}}s in the spec) will "flip" the position of the anchor-positioned element across one or both axes if the element would otherwise overflow.
