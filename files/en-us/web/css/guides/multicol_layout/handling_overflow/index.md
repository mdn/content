...
The {{cssxref("column-height")}} and {{cssxref("column-wrap")}} properties can be used to set a fixed height for generated columns and force excess columns to overflow into additional rows in the block direction. In horizontal writing mode content, this results in vertically-scrolling rows of columns, rather than a horizontally scrolling single row. Let's look at an example.
