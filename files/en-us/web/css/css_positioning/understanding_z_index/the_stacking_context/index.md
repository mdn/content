...
Within a stacking context, child elements are stacked according to the same rules explained in above scenarios. Importantly, the `z-index` values of its child stacking contexts only have meaning in this parent. Stacking contexts are treated atomically as a single unit in the parent stacking context.
