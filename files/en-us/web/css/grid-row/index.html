...
<p>The <strong><code>grid-row</code></strong> CSS <a href="/en-US/docs/Web/CSS/Shorthand_properties">shorthand property</a> specifies a grid item’s size and location within the grid row by contributing a line, a span, or nothing (automatic) to its grid placement, thereby specifying the inline-start and inline-end edge of its {{glossary("grid areas", "grid area")}}.</p>
...
<dl>
 ...
 <div class="note">
   <p><strong>Note:</strong> Named grid areas automatically generate implicit named lines of this form, so specifying <code>grid-row: foo;</code> will choose the start/end edge of that named grid area (unless another line named <code>foo-start</code>/<code>foo-end</code> was explicitly specified before it).</p>
 </div>
...
<h3 id="Setting_grid_row_size_and_location">Setting grid row size and location</h3>
...
<p>{{EmbedLiveSample("Setting_grid_row_size_and_location", "200px", "200px")}}</p>
