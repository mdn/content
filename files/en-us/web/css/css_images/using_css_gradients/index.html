...
<p><strong>CSS gradients</strong> are represented by the {{cssxref("&lt;gradient&gt;")}} data type, a special type of {{cssxref("&lt;image&gt;")}} made of a progressive transition between two or more colors. You can choose between three types of gradients: <em>linear</em> (created with the {{cssxref("linear-gradient()")}} function), <em>radial</em> (created with {{cssxref("radial-gradient()")}}), and <em>conic</em> (created with the {{cssxref("conic-gradient()")}} function). You can also create repeating gradients with the {{cssxref("repeating-linear-gradient()")}}, {{cssxref("repeating-radial-gradient()")}}, and {{cssxref("repeating-conic-gradient()")}} functions.</p>

<p>Gradients can be used anywhere you would use an <code>&lt;image&gt;</code>, such as in backgrounds. Because gradients are dynamically generated, they can negate the need for the raster image files that traditionally were used to achieve similar effects. In addition, because gradients are generated by the browser, they look better than raster images when zoomed in, and can be resized on the fly.</p>

<p>We'll start by introducing linear gradients, then introduce features that are supported in all gradient types using linear gradients as the example, then move on to radial, conic and repeating gradients</p>

<h2 id="Using_linear_gradients">Using linear gradients</h2>

<p>A linear gradient creates a band of colors that progress in a straight line.</p>

<div>
<h3 id="A_basic_linear_gradient">A basic linear gradient</h3>
...
<div>
<h3 id="Changing_the_direction">Changing the direction</h3>
...
<div>
<h3 id="Diagonal_gradients">Diagonal gradients</h3>
...
<div>
<h3 id="Using_angles">Using angles</h3>
...
<p>When using an angle, <code>0deg</code> creates a vertical gradient running bottom to top, <code>90deg</code> creates a horizontal gradient running left to right, and so on in a clockwise direction. Negative angles run in the counterclockwise direction.</p>

<p><img src="linear_red_angles.png"></p>
</div>

<h2 id="Declaring_colors_creating_effects">Declaring colors &amp; creating effects</h2>

<p>All CSS gradient types are a range of position-dependent colors. The colors produced by CSS gradients can vary continuously with position, producing smooth color transitions. It is also possible to create bands of solid colors, and hard transitions between two colors. The following are valid for all gradient functions:</p>

<div>
<h3 id="Using_more_than_two_colors">Using more than two colors</h3>
...
<p>Unlike linear gradients, you can specify the size of radial gradients. Possible values include <code>closest-corner</code>, <code>closest-side</code>, <code>farthest-corner</code>, and <code>farthest-side</code>, with <code>farthest-corner</code> being the default. Circles can also be sized with a length, and ellipses a length or percentage.</p>
...
<h4 id="Example_length_or_percentage_for_ellipses">Example: length or percentage for ellipses</h4>
...
<p>{{ EmbedLiveSample('Example_length_or_percentage_for_ellipses', 240, 120) }}</p>

<h4 id="Example_length_for_circles">Example: length for circles</h4>
...
<p>{{ EmbedLiveSample('Example_length_for_circles', 240, 120) }}</p>
...
<p>In a radial gradient, the colors transition from the center of an ellipse, outward, in all directions. With conic gradients, the colors transition as if spun around the center of a circle, starting at the top and going clockwise. Similar to radial gradients, you can position the center of the gradient. Similar to linear gradients, you can change the gradient angle.</p>
