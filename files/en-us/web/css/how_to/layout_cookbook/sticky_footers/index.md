...
The flexbox example starts out in the same way, but we use `display:flex` rather than `display:grid` on the `.wrapper`; we also set {{cssxref("flex-direction")}} to `column`. Then we set our main content to {{cssxref("flex-grow", "flex-grow: 1")}} and the other two elements to {{cssxref("flex-shrink", "flex-shrink: 0")}} — this prevents them from shrinking smaller when content fills the main area.
