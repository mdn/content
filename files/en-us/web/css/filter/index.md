...
When a single `filter` property has two or more functions its results will be different from when two or more `filter` properties are separately applied with the same functions.
