...
Some functional notations, such as [`linear-gradient()`](/en-US/docs/Web/CSS/gradient/linear-gradient) use commas, but commas are generally only used to separate items in a list. If a comma is used to separate arguments, white space is optional before and after the comma.
