...
Some legacy functional notations such as legacy syntax for `rgb()`, `rgba()`, `hsl()`, and `hsla()` used commas, but generally commas are only used to separate items in a list. If a comma is used to separate arguments, white space is optional before and after the comma.
