...
A better way to perform the above tasks would be to use the Flexbox `gap` property.It is specified on the container and creates a equal space among all flex items.Try to implement gaps in the  both using negative margins and the `gap` property in the editor below.
