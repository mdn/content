...
One of the reasons that flexbox is so useful is that enables proper alignment, including providing a quick method of vertically centering boxes. In this guide, we will take a thorough look at how the alignment and justification properties work in flexbox.

To center a box, we use the {{cssxref("align-items")}} property to align our item on the {{glossary("cross axis")}}, which in this case is the [block axis](/en-US/docs/Glossary/Flow_relative_values) running vertically. We use {{cssxref("justify-content")}} to align the item on the main axis, which in this case is the inline axis running horizontally.
...
- {{cssxref("align-content")}} — controls space between flex lines on the cross axis.
...
The {{cssxref("align-items")}} property, set on the flex container, and {{cssxref("align-self")}} property, set on flex items, control alignment of our flex items on the cross axis, down the columns if {{cssxref("flex-direction")}} is `row` and along the row if `flex-direction` is `column`.

We are making use of cross-axis alignment in this basic flex example. When we add `display: flex` to a container, the child items all become flex items arranged in a row. By default, they will all stretch to be as tall as the tallest item, as that item is defining the height of the items on the cross axis. If your flex container has a height set, then the items will stretch to that height, regardless of how much content is in the item.
...
Thus far, we have looked at alignment behavior when the `flex-direction` has defaulted to `row` while working in a language written top to bottom, with a horizontal main axis and vertical cross axis.

![Three items, the first aligned to flex-start, second to center, third to flex-end. Aligning on the vertical axis.](align4.png)

Keeping the same writing mode, when the `flex-direction` is changed to `column`, the `align-items` and `align-self` will align the items to the left and right instead of top and bottom; these properties are still aligning items along the cross axis, but the cross axis is now horizontal!
...
So far we have been aligning the items, or an individual item inside the area defined by a {{glossary("flex-container")}} containing line of flex items. When the flex items are allowed to wrap across multiple flex lines, the {{cssxref("align-content")}} property can be used to control the distribution of space between the lines, also known as **packing flex lines**.

For `align-content` to have an effect, the cross axis dimension (the height in this case) of your flex container must be greate than needed to display the items. It then works on all the items as a set. The `align-content` values dictates what happens with the extra available space, and the alignment of the entire set of items within it.
...
Remember that with all of these alignment methods, the values of `start` and `end` are writing mode-aware. If the value of `justify-content` is `start` and the writing mode is left-to-right as in English, the items will line up starting at the left side of the container.
