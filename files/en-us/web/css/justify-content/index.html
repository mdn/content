...
<p>The <a href="/en-US/docs/Web/CSS">CSS</a> <strong><code>justify-content</code></strong> property defines how the browser distributes space between and around content items along the {{Glossary("Main Axis", "main-axis")}} of a flex container, and the inline axis of a grid container.</p>

<p>The interactive example below demonstrates some of the values using Grid Layout.</p>

<div>{{EmbedInteractiveExample("pages/css/justify-content.html")}}</div>

<p>The alignment is done after the lengths and auto margins are applied, meaning that, if in a <a href="/en-US/docs/Web/CSS/CSS_Flexible_Box_Layout">Flexbox layout</a> there is at least one flexible element, with {{cssxref("flex-grow")}} different from <code>0</code>, it will have no effect as there won't be any available space.</p>
...
<dl>
 ...
 <dd>The items are packed flush to each other toward the edge of the alignment container depending on the flex container's main-start side.<br>
 ...
 <dd>The items are packed flush to each other toward the edge of the alignment container depending on the flex container's main-end side.<br>
 ...
 <dd>The items are packed in their default position as if no <code>justify-content</code> value was set. This value behaves as <code>stretch</code> in grid and flex containers.</dd>
 <dt><code>baseline<br>
 ...
 <dd>
 <p>If the combined size of the items along the main axis is less than the size of the alignment container, any <code>auto</code>-sized items have their size increased equally (not proportionally), while still respecting the constraints imposed by {{cssxref("max-height")}}/{{cssxref("max-width")}} (or equivalent functionality), so that the combined size exactly fills the alignment container along the main axis.</p>

 <div class="notecard note">
 <p><strong>Note:</strong> <code>stretch</code> is not supported by flexible boxes (flexbox).</p>
 </div>
 </dd>
...
<h3 id="Setting_flex_item_distribution">Setting flex item distribution</h3>
...
<p>{{EmbedLiveSample("Setting_flex_item_distribution", "100%", 140)}}</p>
