...
Content designed for large viewports may exhibit a variety of bugs when viewed in smaller viewports, including unintended wrapping, clipped content, wrong-sized {{glossary("scroll container", "scroll containers")}}. HTML provides a [viewport meta tag](/en-US/docs/Web/HTML/Reference/Elements/meta/name/viewport), `<meta name="viewport">`, to provide hints about the initial size of the viewport. If the site isn't designed to work well on small viewports and this tag is omitted, some mobile browsers use a fixed initial containing block width (typically `980px`) then scale the content down making making the CSS pixel size smaller than an actual pixel. The result is a scaled down site that fits in the available screen space but is illegible for the user, requiring the user to zoom and pan to view the content.

The {{glossary("viewport")}} initial containing block for continuous media that has the dimensions of the viewport. Since the viewport is generally no larger than the display, devices with smaller displays such as phones or tablets typically present a smaller viewport than larger devices like desktop or laptops.
