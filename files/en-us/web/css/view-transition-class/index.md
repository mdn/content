...
The `view-transition-class` is used only to apply styles using the view transition pseudo-elements ({{cssxref("::view-transition-group()")}}, {{cssxref("::view-transition-image-pair()")}}, {{cssxref("::view-transition-old()")}}, {{cssxref("::view-transition-new()")}}).
