...
Using a `visibility` value of `hidden` on an element will remove it from the {{Glossary("accessibility tree")}}. This will cause the element and all its descendant elements to no longer be announced by screen reading technology.
