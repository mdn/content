...
- `enabled` {{Deprecated_Inline}} {{Non-standard_Inline}}
  - : The element accepts user input. For textboxes, this is the default behavior. **Please note that this value is no longer supported in Firefox 60 onwards ([Firefox bug 1405087](https://bugzilla.mozilla.org/show_bug.cgi?id=1405087)).**
- `disabled` {{Deprecated_Inline}} {{Non-standard_Inline}}
  - : The element does not accept user input. However, this is not the same as setting `disabled` to true, in that the element is drawn normally. **Please note that this value is no longer supported in Firefox 60 onwards ([Firefox bug 1405087](https://bugzilla.mozilla.org/show_bug.cgi?id=1405087)).**
