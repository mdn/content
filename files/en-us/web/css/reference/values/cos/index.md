...
The **`cos()`** [CSS](/en-US/docs/Web/CSS) [function](/en-US/docs/Web/CSS/Reference/Values/Functions) returns the cosine of an angle as a number between `-1` and `1`.
...
In practical use, `cos()` is used for circular positioning, rotational geometry, and smooth periodic motion in CSS. It is commonly paired with {{CSSxRef("sin")}} to place or animate elements along circular paths such as spinners, radial menus, clock faces, orbiting icons, and data visualizations, without requiring JavaScript-based calculations.
