...
There are some limitations to using the `path()` function. The path has to be defined as a single string, so a custom path can't be created using variables ({{cssxref("var()")}} functions). Also, all the lengths in the path are implicitly defined in [pixel](/en-US/docs/Web/CSS/Guides/Values_and_units/Numeric_data_types#absolute_length_units) (`px`) units; other units can't be used. The {{cssxref("shape()")}} function offers more flexibility than the `path()` function.
