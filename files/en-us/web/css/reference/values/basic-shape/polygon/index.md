...
Note: The SVG {{svgelement("polygon")}} element has separate attributes for {{svgattr("fill-rule")}} and {{svgattr("points")}}, and `points` is flexible about the use of space and comma separators. CSS `polygon()` rules for separators are strictly enforced.
