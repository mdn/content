...
The {{cssxref("basic-shape/polygon","polygon()")}} function defines a polygon using an optional first parameter containing values that modify the polygon's behavior, plus one or more parameters containing coordinate pairs. Each parameter is separated by a comma.
...
- An SVG {{SVGAttr("fill-rule")}} value that specifies the algorithm used to fill the polygon shape.
- The keyword [`round`](/en-US/docs/Web/CSS/Reference/Values/basic-shape/polygon#round) followed by a {{cssxref("length")}} value, with the two separated by a space. This specifies that the polygon should have rounded corners, with the `<length>` specifying the corner radius.
