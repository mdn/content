...
When animating `content-visibility` with [CSS transitions](/en-US/docs/Web/CSS/Guides/Transitions), {{cssxref("transition-behavior", "transition-behavior: allow-discrete", "#allow-discrete")}} needs to be set on `content-visibility`. This effectively enables `content-visibility` transitions.

> [!NOTE]
> When transitioning an element's `content-visibility` value, you don't need to provide a set of starting values for transitioned properties using a [`@starting-style`](/en-US/docs/Web/CSS/Reference/At-rules/@starting-style) block, like you do when transitioning {{cssxref("display", "display", "#animating_display")}}. This is because `content-visibility` doesn't hide an element from the DOM like `display` does: it just skips rendering the element's content.
