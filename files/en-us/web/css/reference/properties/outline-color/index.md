...
An outline is a user interface component rather than text, so the text contrast requirements of [Success Criterion 1.4.3](https://www.w3.org/WAI/WCAG22/Understanding/contrast-minimum) do not apply to it. [Success Criterion 1.4.11 Non-text Contrast](https://www.w3.org/WAI/WCAG22/Understanding/non-text-contrast) requires a contrast ratio of at least 3:1 between the outline and the adjacent colors it sits against. Where the outline is used as a focus indicator, note that the adjacent colors include both the background outside the element and the element's own background, because the outline may border both.
