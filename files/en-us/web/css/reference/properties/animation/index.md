...
> [!NOTE]
> Other animation-related properties such as {{cssxref("animation-range-start")}}, {{cssxref("animation-range-end")}} and {{cssxref("animation-trigger")}} cannot be set in the `animation` shorthand, but may be reset by it, depending on the browser implementation. For this reason, it is advisable to declare these longhand properties after declaring any `animation` shorthand to ensure that they take effect.
