...
- {{cssxref("transition-behavior", "transition-behavior: allow-discrete", "#allow-discrete")}} needs to be set on the {{cssxref("transition-property")}} declaration (or the {{cssxref("transition")}} shorthand) to enable `display` transitions.

For examples of transitioning the `display` property, see the [`@starting-style`](/en-US/docs/Web/CSS/Reference/At-rules/@starting-style#examples) and {{cssxref("transition-behavior", "transition-behavior", "#examples")}} pages.
