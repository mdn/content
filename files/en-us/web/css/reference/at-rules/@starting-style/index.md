...
- Specify the exit animation inside the `.fade-out` rule — this is the class that the JavaScript assigns to the `<div>` elements when their close buttons are pressed. Besides setting the `opacity` and `scale` ending states, we also set {{cssxref("display", "display: none")}} on the `<div>`s — we want them to become immediately unavailable when removed from the UI.
