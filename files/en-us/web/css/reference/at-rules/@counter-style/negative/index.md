...
This example [extends](/en-US/docs/Web/CSS/Reference/At-rules/@counter-style/system#extends) the {{cssxref("list-style-type", "decimal", "#decimal")}} list style. The `negative` descriptor is used to add `(-` and `)` before and after negative counter values.
