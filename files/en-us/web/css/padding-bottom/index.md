---
title: padding-bottom
slug: Web/CSS/padding-bottom
tags:
  - CSS
  - CSS Padding
  - CSS Property
  - Reference
  - recipe:css-property
browser-compat: css.properties.padding-bottom
---
<div>{{CSSRef}}</div>

<p>The <strong><code>padding-bottom</code></strong> <a href="/en-US/docs/Web/CSS">CSS</a> property sets the height of the <a href="/en-US/docs/Web/CSS/CSS_Box_Model/Introduction_to_the_CSS_box_model#padding_area">padding area</a> on the bottom of an element.</p>

<div>{{EmbedInteractiveExample("pages/css/padding-bottom.html")}}</div>


<p>An element's padding area is the space between its content and its border.</p>

<p><img alt="The effect of the CSS padding-bottom property on the element box" src="padding-bottom.svg"></p>

<div class="note">
<p><strong>Note:</strong> The {{cssxref("padding")}} property can be used to set paddings on all four sides of an element with a single declaration.</p>
</div>

<h2 id="Syntax">Syntax</h2>

<pre class="brush:css no-line-numbers">/* &lt;length&gt; values */
padding-bottom: 0.5em;
padding-bottom: 0;
padding-bottom: 2cm;

/* &lt;percentage&gt; value */
padding-bottom: 10%;

/* Global values */
padding-bottom: inherit;
padding-bottom: initial;
padding-botom: revert;
padding-bottom: unset;
</pre>

<p>The <code>padding-bottom</code> property is specified as a single value chosen from the list below. Unlike margins, negative values are not allowed for padding.</p>

<h3 id="Values">Values</h3>

<dl>
 <dt>{{cssxref("&lt;length&gt;")}}</dt>
 <dd>The size of the padding as a fixed value. Must be nonnegative.</dd>
 <dt>{{cssxref("&lt;percentage&gt;")}}</dt>
 <dd>The size of the padding as a percentage, relative to the <em>width</em> of the containing block. Must be nonnegative.</dd>
</dl>

<h2 id="Formal_definition">Formal definition</h2>

<p>{{cssinfo}}</p>

<h2 id="Formal_syntax">Formal syntax</h2>

{{csssyntax}}

<h2 id="Examples">Examples</h2>

<h3 id="Setting_padding_bottom_with_pixels_and_percentages">Setting padding bottom with pixels and percentages</h3>

<pre class="brush: css">.content { padding-bottom: 5%; }
.sidebox { padding-bottom: 10px; }
</pre>

<h2 id="Specifications">Specifications</h2>

{{Specifications}}

<h2 id="Browser_compatibility">Browser compatibility</h2>

<p>{{Compat}}</p>

<h2 id="See_also">See also</h2>

<ul>
 <li><a href="/en-US/docs/Web/CSS/CSS_Box_Model/Introduction_to_the_CSS_box_model">Introduction to the CSS basic box model</a></li>
 <li>{{cssxref("padding-top")}}, {{cssxref("padding-right")}}, {{cssxref("padding-left")}} and the {{cssxref("padding")}} shorthand</li>
 <li>The mapped logical properties: {{cssxref("padding-block-start")}}, {{cssxref("padding-block-end")}}, {{cssxref("padding-inline-start")}}, and {{cssxref("padding-inline-end")}} and the shorthands {{cssxref("padding-block")}} and {{cssxref("padding-inline")}}</li>
</ul>
