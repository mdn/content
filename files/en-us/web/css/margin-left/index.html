...
<p>The <strong><code>margin-left</code></strong> <a href="/en-US/docs/Web/CSS">CSS</a> property sets the <a href="/en-US/docs/Web/CSS/CSS_Box_Model/Introduction_to_the_CSS_box_model">margin area</a> on the left side of an element. A positive value places it farther from its neighbors, while a negative value places it closer.</p>
...
<p>The <code>margin-left</code> property is specified as the keyword <code>auto</code>, or a <code>&lt;length&gt;</code>, or a <code>&lt;percentage&gt;</code>. Its value can be positive, zero, or negative.</p>
...
<dl>
 ...
 <dd>The left margin receives a share of the unused horizontal space, as determined mainly by the layout mode that is used. If the values of <code>margin-left</code> and <code>margin-right</code> are both <code>auto</code>, the calculated space is evenly distributed. This table summarizes the different cases:
...
<h3 id="Setting_left_margin_using_pixels_and_percentages">Setting left margin using pixels and percentages</h3>
...
<ul>
 <li>{{cssxref("margin-top")}}, {{cssxref("margin-right")}}, and {{cssxref("margin-bottom")}} and the {{cssxref("margin")}} shorthand</li>
