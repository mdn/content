...
> **Note:** The pseudo-elements generated by `::before` and `::after` are generate boxes as if they were immediate children of the element on which they are applied, or "originating element", and thus can not apply to _[replaced elements](/en-US/docs/Web/CSS/Replaced_element)_ such as {{htmlelement("img")}} whose content is outside the scope of the CSS formatting model.
