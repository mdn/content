...
- When `position` is set to `absolute` or `fixed`, the `bottom` property specifies the distance between the element's `outer margin` of bottom edge and the `inner border/outer padding` of the bottom edge of its containing block.
