...
In the first above example, the width will be at least 400px, but will be wider if the viewport is more than 2000px wide (in which case 1vw would be 20px, so 20vw would be 400px). Think this technique of using `max(<relative unit>, <absolute unit>)` as providing the _minimum_ value a property can have, because the value in absolute unit fixes the minimum.
