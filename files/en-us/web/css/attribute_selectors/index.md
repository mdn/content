...
The case sensitivity of attribute names and values depends on the document language. In HTML, attribute names are case insensitive, as are spec-defined {{glossary("enumerated")}} values. The [list of case-insensitive CSS attribute values](https://html.spec.whatwg.org/multipage/semantics-other.html#case-sensitivity-of-selectors) is enumerated in the HTML spec. For these attributes, the attribute value in the selector is case-insensitive, even if it's an invalid value or if the attribute is not a valid value for the element on which it is set.
