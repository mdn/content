...
The result is kept in a per-document in-memory cache. If you preload something your current page doesn't navigate to, it is generally a waste of resources, although the result may populate the HTTP cache if headers allow.
...
- The browser also does this same for module dependencies automatically.
...
| [`<link rel="prefetch">`](/en-US/docs/Web/HTML/Attributes/rel/prefetch)           | Pre-populating the HTTP cache                            | Use to prefetch same-site future navigation resources or subresources used on those pages. Uses HTTP cache there has a number of issues, such as being potentially blocked by [Cache-Control](/en-US/docs/Web/HTTP/Headers/Cache-Control) settings. Use [Speculation Rules API](/en-US/docs/Web/API/Speculation_Rules_API) prefetch instead, where it is supported. |
...
| [Speculation Rules API](/en-US/docs/Web/API/Speculation_Rules_API) prefetch       | Preparing for the next navigation                        | Use to prefetch same or cross-site future navigation resources. Broad adoption is recommended, where it is supported; check to make sure the pages are [safe to prefetch](/en-US/docs/Web/API/Speculation_Rules_API#unsafe_prefetching).                                                                                                                            |
| [Speculation Rules API](/en-US/docs/Web/API/Speculation_Rules_API) prerender      | Preparing for the next navigation                        | Use to preload same-site future navigation resources, for instant navigations. Use on high-priority pages, where it is supported; check to make sure the pages are [safe to prerender](/en-US/docs/Web/API/Speculation_Rules_API#unsafe_prerendering).                                                                                                              |
