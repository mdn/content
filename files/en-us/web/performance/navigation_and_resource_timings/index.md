...
- {{domxref("PerformanceTiming.domContentLoadedEventStart","domContentLoadedEventStart")}}
  - : Right before the parser sent the `[DOMContentLoaded](/en-US/docs/Web/API/Window/DOMContentLoaded_event)` event, that is right after all the scripts that need to be executed right after parsing have been executed. |
