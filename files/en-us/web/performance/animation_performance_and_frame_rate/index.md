...
- Properties that affect an element's **geometry** or **position** trigger:
...
- Properties that do _not_ affect geometry or position, but are _not rendered_ in their own layer, do _not_ trigger a layout.
  For example: {{cssxref("color")}}

- Properties that _are rendered_ in their **own layer** don't even trigger a repaint, because the update is handled in **composition**.
