...
<p>The <strong><code>&lt;ellipse&gt;</code></strong> element is an SVG basic shape, used to create ellipses based on a center coordinate, and both their x and y radius.</p>

<div class="note">
<p><strong>Note:</strong> Ellipses are unable to specify the exact orientation of the ellipse (if, for example, you wanted to draw an ellipse tilted at a 45 degree angle), but it can be rotated by using the {{SVGAttr("transform")}} attribute.</p>
</div>
...
<div class="note">
<p><strong>Note:</strong> Starting with SVG2 <code>cx</code>, <code>cy</code>, <code>rx</code> and <code>ry</code> are <em>Geometry Properties</em>, meaning those attributes can also be used as CSS properties for that element.</p>
</div>
