...
In this example, we define three shapes using {{SVGElement('path')}} elements, each with a different [`stroke`](/en-US/docs/Web/SVG/Reference/Attribute/stroke) and `fill` color set on them. We also define a {{SVGElement('circle')}} element as a marker via the {{SVGElement('marker')}} element. Each shape has the marker applied via the `marker` CSS property.
