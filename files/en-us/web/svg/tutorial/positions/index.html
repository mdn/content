...
<p>For all elements, SVG uses a coordinate system or <strong>grid</strong> system similar to the one used by <a href="/en-US/docs/Web/API/Canvas_API">canvas</a> (and by a whole lot of other computer drawing routines). That is, the top left corner of the document is considered to be the point (0,0), or point of origin. Positions are then measured in pixels from the top left corner, with the positive x direction being to the right, and the positive y direction being to the bottom.</p>

<img alt="" src="canvas_default_grid.png">

<p>Note that this is slightly different than the way you're taught to graph as a kid (y axis is flipped). However, this is the same way elements in HTML are positioned (By default, LTR documents are considered not the RTL documents which position X from right-to-left).</p>
...
<h3 id="What_are_pixels">What are "pixels"?</h3>
...
<p>Without further specification, one user unit equals one screen unit. To explicitly change this behavior, there are several possibilities in SVG. We start with the <code>svg</code> root element:</p>
