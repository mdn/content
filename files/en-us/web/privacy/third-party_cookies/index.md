...
- If the cookie domain and scheme match the current page the user is looking at, the cookie is considered to be from the same site as the page, and is referred to as a _first-party cookie_.
...
A first-party cookie is set when a user first visits a page or follows an internal link to another page on the same site, and when a resource residing on the same site is requested (for example an embedded image, web font, or JavaScript file).

Third-party cookies are sent in a couple of common situations:

- When a link is clicked on one site to navigate to another site. You might for example have an affiliate link to a partner site, and set a cookie when the user follows it that displays a reward banner and provides a discount if a certain product is purchased.
- When a page embeds components from another sites, such as images, or other documents embedded in {{htmlelement("iframe")}}s (often referred to as _third party content_). As well as the original request for the component, these components may generate further requests that set more third-party cookies. For example, you might have a sign-in widget embedded on multiple different-but-related sites, which shares a cookie across all sites stating that the user is signed in, so they don't have to sign in again on each site.
...
Legislation such as the [General Data Privacy Regulation](https://en.wikipedia.org/wiki/General_Data_Protection_Regulation) (GDPR) in the European Union and the [California Consumer Privacy Act](https://www.oag.ca.gov/privacy/ccpa) (CCPA) have helped by making it a legal requirement for companies to be transparent about the cookies they set and the information they collect. Examples include asking customers to opt in to such data collection, allowing them to see what data a company holds on them, and allowing them to delete it if they wish. However, it is still not always crystal clear to customers how their data is being used.
