...
## Deprecated Privacy sandbox features
...
- Protected Audience API {{deprecated_inline}}
  - : Since Chrome 152, all calls to Protected Audience API throw errors or return placeholder information. Chrome will remove API stubs at a later date. Protected Audience API enabled on-device ad auctions to serve remarketing and custom audiences, without cross-site third-party tracking.
- Private Aggregation API {{deprecated_inline}}
  - : Since Chrome 152, Private Aggregation API is unreachable. Prior to removal, Private Aggregation API provided aggregating and reporting on cross-site data in a privacy-preserving manner. With Private Aggregation, developers could generate aggregate data reports using data from Protected Audience and cross-site data from Shared Storage API.
- [Shared Storage API](/en-US/docs/Web/API/Shared_Storage_API) {{deprecated_inline}}
  - : Since Chrome 152, all calls to Shared Storage API throw errors or return placeholder information. Chrome will remove API stubs at a later date. A client-side storage mechanism that enables unpartitioned, cross-site data access while preserving privacy (i.e., without relying on tracking cookies). Shared Storage API could be used in conjunction with Private Aggregation API to generate aggregate data reports from cross-site data.
- [Fenced Frame API](/en-US/docs/Web/API/Fenced_frame_API) {{deprecated_inline}}
