...
To do this, you create a `<video>` (or `<audio>`) element with no [`src`](/en-US/docs/Web/HTML/Reference/Element/video#src) attribute.
...
The video is offered first in WebM format (with the [`type`](/en-US/docs/Web/HTML/Reference/Element/source#type) attribute set to `video/webm`).
