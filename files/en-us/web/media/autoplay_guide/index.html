...
<p>Otherwise, the playback will likely be blocked. The exact situations that result in blocking, and the specifics of how sites become allowlisted vary from browser to browser, but the above are good guidelines to go by.</p>
...
<p>If you want to start playing the video after the first interaction with the page, <a href="/en-US/docs/Web/API/setInterval"><code>setInterval()</code></a> might be used to achieve this:</p>
