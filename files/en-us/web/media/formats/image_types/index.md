...
APNG is a file format first introduced by Mozilla which extends the {{anch("PNG (Portable Network Graphics)", "PNG")}} standard to add support for animated images.
...
As support is not yet comprehensive (and has little historical depth) you should provide a fallback in {{anch("WebP image", "WebP")}}, {{anch("JPEG (Joint Photographic Experts Group image)", "JPEG")}} or {{anch("PNG (Portable Network Graphics)", "PNG")}} format using [the `<picture>` element](/en-US/docs/Web/HTML/Element/picture) (or some other approach).
...
Theoretically, several compression algorithms are supported, and the image data can also be stored in {{anch("JPEG (Joint Photographic Experts Group image)", "JPEG")}} or {{anch("PNG (Portable Network Graphics)", "PNG")}} format within the BMP file.
...
GIF was one of the first two graphics formats supported by {{Glossary("HTML")}}, along with {{anch("XBM (X Window System Bitmap file)", "XBM")}}.
...
Another popular feature of GIF is support for {{interwiki("wikipedia", "Interlacing_(bitmaps)", "interlacing")}}, where rows of pixels are stored out of order so that partially-received files can be displayed in lower quality.
...
Typically, modern content should use {{anch("PNG (Portable Network Graphics)", "PNG")}} for lossless _and_ indexed still images, and should consider using {{anch("APNG (Animated Portable Network Graphics)", "APNG")}} for lossless animation sequences.
...
Each icon's data can be either a {{anch("BMP (Bitmap file)", "BMP")}} image without the file header, or a complete {{anch("PNG (Portable Network Graphics)", "PNG")}} image (including the file header).
...
The {{Glossary("PNG")}} (pronounced "**ping**") image format uses lossless or lossy compression to provide more efficient compression, and supports higher color depths than {{anch("GIF (Graphics Interchange Format)", "GIF")}}, as well as full alpha transparency support.
...
{{interwiki("wikipedia", "TIFF")}} is a raster graphics file format which was created to store scanned photos, although it can be any kind of image.
...
Provide a fallback in either {{anch("JPEG (Joint Photographic Experts Group image)", "JPEG")}} or {{anch("PNG (Portable Network Graphics)", "PNG")}} format, such as with [the `<picture>` element](/en-US/docs/Web/HTML/Element/picture).
...
This makes {{anch("JPEG (Joint Photographic Experts Group image)", "JPEG")}} and {{anch("WebP image", "WebP")}} good choices for photographs, with JPEG being more compatible but WebP perhaps offering better compression.
To maximize quality and minimize download time, consider providing both {{anch("Providing image fallbacks", "using a fallback")}} with WebP as the first choice and JPEG as the second.
...
While lossless WebP is ideal for this purpose, support is not widespread yet, so PNG is a better choice unless you offer a {{anch("Providing image fallbacks", "fallback")}}.
...
If the icon can be represented using vector graphics, consider {{anch("SVG (Scalable Vector Graphics)", "SVG")}}, since it scales across various resolutions and sizes, so it's perfect for responsive design.
...
<table class="standard-table" style="max-width: 42rem">
  <thead>
    ...
    <tr>
      <td>{{anch("SVG (Scalable Vector Graphics)", "SVG")}}</td>
      <td>{{anch("PNG (Portable Network Graphics)", "PNG")}}</td>
    </tr>
  </thead>
</table>
