...
<h2 id="General_codec_requirements">General codec requirements</h2>

<p>Before looking at codec-specific capabilities and requirements, there are a few overall requirements that must be met by <em>any</em> codec configuration used with WebRTC.</p>

<p>Unless the {{Glossary("SDP")}} specifically signals otherwise, the web browser receiving a WebRTC video stream must be able to handle video at 20 FPS at a minimum resolution of 320 pixels wide by 240 pixels tall. It's encouraged that video be encoded at a frame rate and size no lower than that, since that's essentially the lower bound of what WebRTC generally is expected to handle.</p>

<p>SDP supports a codec-independent way to specify preferred video resolutions ({{RFC(6236)}}. This is done by sending an <code>a=imageattr</code> SDP attribute to indicate the maximum resolution that is acceptable. The sender is not required to support this mechanism, however, so you have to be prepared to receive media at a different resolution than you requested. Beyond this simple maximum resolution request, specific codecs may offer further ways to ask for specific media configurations.</p>

<h2 id="Supported_video_codecs">Supported video codecs</h2>

<p>WebRTC establishes a baseline set of codecs which all compliant browsers are required to support. Some browsers may choose to allow other codecs as well.</p>
...
<p>Complete details of what video codecs and configurations WebRTC is required to support can be found in {{RFC(7742, "WebRTC Video Processing and Codec Requirements")}}. It's worth noting that the RFC covers a variety of video-related requirements, including color spaces (sRGB is the preferred, but not required, default color space), recommendations for webcam processing features (automatic focus, automatic white balance, automatic light level), and so on.</p>
...
<dl>
 ...
 <dd>If specified and supported by the software, the <code>max-br</code> parameter specifies the maximum video bit rate in units of 1,000 bps for VCL and 1,200 bps for NAL. You'll find details about this on <a href="https://datatracker.ietf.org/doc/html/rfc6184#page-47">page 47 of RFC 6184</a>.</dd>
 <dt><code>max-cpb</code></dt>
 <dd>If specified and supported by the software, <code>max-cpb</code> specifies the maximum coded picture buffer size. This is a fairly complicated parameter whose unit size can vary. See <a href="https://datatracker.ietf.org/doc/html/rfc6184#page-45">page 45 of RFC 6184</a> for details.</dd>
 <dt><code>max-dpb</code></dt>
 <dd>If specified and supported, <code>max-dpb</code> indicates the maximum decoded picture buffer size, given in units of 8/3 macroblocks. See <a href="https://datatracker.ietf.org/doc/html/rfc6184#page-46">RFC 6184, page 46</a> for further details.</dd>
 <dt><code>max-fs</code></dt>
 <dd>If specified and supported by the software, <code>max-fs</code> specifies the maximum size of a single video frame, given as a number of macroblocks.</dd>
 <dt><code>max-mbps</code></dt>
 <dd>If specified and supported by the software, this value is an integer specifying the maximum rate at which macroblocks should be processed per second (in macroblocks per second).</dd>
 <dt><code>max-smbps</code></dt>
 <dd>If specified and supported by the software, this specifies an integer stating the maximum static macroblock processing rate in static macroblocks per second (using the hypothetical assumption that all macroblocks are static macroblocks).</dd>
</dl>
...
<dl>
 ...
 <dd>All endpoints are required to support mode 1 (non-interleaved mode). Support for other packetization modes is optional, and the parameter itself is not required to be specified.</dd>
 <dt><code>sprop-parameter-sets</code></dt>
 <dd>Sequence and picture information for AVC can be sent either in-band or out-of-band. When AVC is used with WebRTC, this information <em>must</em> be signaled in-band; the <code>sprop-parameter-sets</code> parameter must therefore <em>not</em> be included in the SDP.</dd>
</dl>
...
<dl>
 ...
 <dd>All WebRTC implementations are <em>required</em> to specify and interpret this parameter in their SDP, identifying the sub-profile used by the codec. The specific value that is set is not defined; what matters is that the parameter be used at all. This is useful to note, since in {{RFC(6184)}} ("RTP Payload Format for H.264 Video"), <code>profile-level-id</code> is entirely optional.</dd>
</dl>
...
<p>The payload format used for AVC in WebRTC is described in {{RFC(6184, "RTP Payload Format for H.264 Video")}}. AVC implementations for WebRTC are required to support the special "filler payload" and "full frame freeze" SEI messages; these are used to support switching among multiple input streams seamlessly.</p>

<h2 id="Supported_audio_codecs">Supported audio codecs</h2>

<p>The audio codecs which {{RFC(7874)}} mandates that all WebRTC-compatible browsers must support are shown in the table below.</p>
...
<p>It's useful to note that {{RFC(7874)}} defines more than a list of audio codecs that a WebRTC-compliant browser must support; it also provides recommendations and requirements for special audio features such as echo cancelation, noise reduction, and audio leveling.</p>
...
<table class="standard-table">
 <thead>
  <tr>
   ...
   <th scope="col">Recommended bit rate range</th>
  </tr>
 </thead>
 <tbody>
  <tr>
...
<p>The bit rate may be adjusted at any time. In order to avoid network congestion, the average audio bit rate should not exceed the available network bandwidth (minus any other known or anticipated added bandwidth requirements).</p>

<h3 id="G.711">G.711</h3>

<p>G.711 defines the format for <strong>Pulse Code Modulation</strong> (<strong>PCM</strong>) audio as a series of 8-bit integer samples taken at a sample rate of 8,000 Hz, yielding a bit rate of 64 kbps. Both {{interwiki("wikipedia", "M-law", "µ-law")}} and {{interwiki("wikipedia", "A-law")}} encodings are allowed.</p>

<p>G.711 is <a href="https://www.itu.int/rec/T-REC-G.711-198811-I/en">defined by the ITU</a> and its payload format is defined in {{RFC(3551, "4.5.14")}}.</p>
...
<p>Due to its low sample rate and sample size, G.711 audio quality is generally considered poor by modern standards, even though it's roughly equivalent to what a landline telephone sounds like. It is generally used as a least common denominator to ensure that browsers can achieve an audio connection regardless of platforms and browsers, or as a fallback option in general.</p>

<h2 id="Specifying_and_configuring_codecs">Specifying and configuring codecs</h2>

<h3 id="Getting_the_supported_codecs">Getting the supported codecs</h3>

<p>Because a given browser and platform may have different availability among the potential codecs—and may have multiple profiles or levels supported for a given codec—the first step when configuring codecs for an {{domxref("RTCPeerConnection")}} is to get the list of available codecs. To do this, you first have to establish a connection on which to get the list.</p>

<p>There are a couple of ways you can do this. The most efficient way is to use the static method {{domxref("RTCRtpSender.getCapabilities()")}} (or the equivalent {{domxref("RTCRtpReceiver.getCapabilities()")}} for a receiver), specifying the type of media as the input parameter. For example, to determine the supported codecs for video, you can do this:</p>

<pre class="brush: js">codecList = RTCRtpSender.getCapabilities("video").codecs;</pre>

<p>Now <code>codecList</code> is an array of {{domxref("RTCRtpCodecCapability")}} objects, each describing one codec configuration. Also present in the list will be entries for retransmission (RTX), redundant coding (RED), and forward error correction (FEC).</p>

<p>If the connection is in the process of starting up, you can use the {{domxref("RTCPeerConnection.icegatheringstatechange_event", "icegatheringstatechange")}} event to watch for the completion of {{Glossary("ICE")}} candidate gathering, then fetch the list.</p>
...
<p>The event handler for <code>icegatheringstatechange</code> is established; in it, we look to see if the ICE gathering state is <code>complete</code>, indicating that no further candidates will be collected. The method {{domxref("RTCPeerConnection.getSenders()")}} is called to get a list of all the {{domxref("RTCRtpSender")}} objects used by the connection.</p>

<p>With that in hand, we walk through the list of senders, looking for the first one whose {{domxref("MediaStreamTrack")}} indicates that it's {{domxref("MediaStreamTrack.track", "track")}}'s {{domxref("MediaStreamTrack.kind", "kind")}} is <code>video</code>, indicating that the track's data is video media. We then call that sender's {{domxref("RTCRtpSender.getParameters", "getParameters()")}} method and, from the returned {{domxref("RTCRtpSendParameters")}} object, we set <code>codecList</code> to the {{domxref("RTCRtpParameters.codecs", "codecs")}} property and return to the caller.</p>

<p>If no video track is found, we set <code>codecList</code> to <code>null</code>.</p>

<p>On return, then, <code>codecList</code> is either <code>null</code> to indicate that no video tracks were found or it's an array of {{domxref("RTCRtpCodecParameters")}} objects, each describing one permitted codec configuration. Of special importance in these objects: the {{domxref("RTCRtpCodecParameters.payloadType", "payloadType")}} property, which is a one-byte value which uniquely identifies the described configuration.</p>
...
<h3 id="Customizing_the_codec_list">Customizing the codec list</h3>

<p>Once you have a list of the available codecs, you can alter it and then send the revised list to {{domxref("RTCRtpTransceiver.setCodecPreferences()")}} to rearrange the codec list. This changes the order of preference of the codecs, letting you tell WebRTC to prefer a different codec over all others.</p>
...
<p>In this sample, the function <code>changeVideoCodec()</code> takes as input the MIME type of the codec you wish to use. The code starts by getting a list of all of the {{domxref("RTCPeerConnection")}}'s transceivers.</p>

<p>Then, for each transceiver, we get the kind of media represented by the transceiver from the {{domxref("RTCRtpSender")}}'s track's {{domxref("MediaStreamTrack.kind", "kind")}}. We also get the lists of all codecs supported by the browser for both sending and receiving video, using the <code>getCapabilities()</code> static method of both {{domxref("RTCRtpSender")}} and {{domxref("RTCRtpReceiver")}}.</p>
...
<p>Finally, we call the {{domxref("RTCRtpTransceiver")}}'s {{domxref("RTCRtpTransceiver.setCodecPreferences", "setCodecPreferences()")}} method to specify that the given send and receive codecs are allowed, in the newly rearranged order.</p>

<p>That's done for each transceiver on the <code>RTCPeerConnection</code>; once all of the transceivers have been updated, we call the {{domxref("RTCPeerConnection.onnegotiationneeded", "onnegotiationneeded")}} event handler, which will create a new offer, update the local description, send the offer along to the remote peer, and so on, thereby triggering the renegotiation of the connection.</p>
...
<p>This code is just splitting the codec list into two arrays: one containing codecs whose MIME type matches the one specified by the <code>mimeType</code> parameter, and the other with all the other codecs. Once the list has been split up, they're concatenated back together with the entries matching the given <code>mimeType</code> first, followed by all of the other codecs. The rearranged list is then returned to the caller.</p>

<h2 id="Default_codecs">Default codecs</h2>

<p>Unless otherwise specified, the default—or, more accurately, preferred—codecs requested by each browser's implementation of WebRTC are shown in the table below.</p>
...
<h2 id="Choosing_the_right_codec">Choosing the right codec</h2>

<p>Before choosing a codec that isn't one of the mandatory codecs (VP8 or AVC for video and Opus or PCM for audio), you should seriously consider the potential drawbacks: in particular, only these codecs can be generally assumed to be available on essentially all devices that support WebRTC.</p>

<p>If you choose to prefer a codec other than the mandatory ones, you should at least allow for fallback to one of the mandatory codecs if support is unavailable for the codec you prefer.</p>

<h3 id="Audio">Audio</h3>

<p>In general, if it's available and the audio you wish to send has a sample rate greater than 8 kHz, you should strongly consider using Opus as your primary codec. For voice-only connections in a constrained environment, using G.711 at an 8 kHz sample rate can provide an acceptable experience for conversation, but typically you'll use G.711 as a fallback option, since there are other options which are more efficient and sound better, such as Opus in its narrowband mode.</p>

<h3 id="Video">Video</h3>

<p>There are a number of factors that come into play when deciding upon a video codec (or set of codecs) to support.</p>
