...
Template literals are enclosed by back-tick characters (`` ` ``) ([grave accent](https://en.wikipedia.org/wiki/Grave_accent)) instead of double or single quotes. Template literals can contain place holders. These are indicated by the Dollar sign and curly braces (`${expression}`).
