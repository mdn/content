...
3. Because `B`'s closure includes `A`, `C`'s closure includes `B`, so `C` can access _both_ `B` _and_ `A`'s arguments and variables. In other words, `C` _chains_ the scopes of `B` and `A`, _in that order_.
