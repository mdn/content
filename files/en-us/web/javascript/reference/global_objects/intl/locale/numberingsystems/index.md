...
The **`Intl.Locale.prototype.numberingSystems`** property is an accessor property that returns of 1 or more unique numbering system identifiers according to the [numeral system](https://en.wikipedia.org/wiki/Numeral_system) used by the `Locale`.
...
console.log(arEG.numberingSystems); // Prints ["arab"]
```


```js
let ja = new Intl.Locale("ja");
console.log(ja.numberingSystems); // Prints ["latn"]
```
