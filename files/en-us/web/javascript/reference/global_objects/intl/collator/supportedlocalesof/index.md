...
<p>The <strong><code>Intl.Collator.supportedLocalesOf()</code></strong> method returns an
...
<p>An array of strings representing a subset of the given locale tags that are supported
	in collation without having to fall back to the runtime's default locale.</p>
...
<p>Assuming a runtime that supports Indonesian and German but not Balinese in collation,
	<code>supportedLocalesOf</code> returns the Indonesian and German language tags
	unchanged, even though <code>pinyin</code> collation is not used with Indonesian and a
	specialized German for Indonesia is unlikely to be supported. Note the specification
	of the "<code>lookup</code>" algorithm here — a "<code>best fit</code>" matcher might
	decide that Indonesian is an adequate match for Balinese since most Balinese speakers
