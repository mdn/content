...
<p>The <strong><code>Intl.DateTimeFormat.supportedLocalesOf()</code></strong> method
	returns an array containing those of the provided locales that are supported in date
	and time formatting without having to fall back to the runtime's default locale.</p>
...
<p>Returns an array with a subset of the language tags provided in <code>locales</code>.
	...
	it wouldn't have to fall back to the default locale.</p>
...
<p>Assuming a runtime that supports Indonesian and German but not Balinese in date and
	time formatting, <code>supportedLocalesOf</code> returns the Indonesian and German
	language tags unchanged, even though <code>pinyin</code> collation is neither relevant
	...
	"<code>lookup</code>" algorithm here — a "<code>best fit</code>" matcher might decide
	that Indonesian is an adequate match for Balinese since most Balinese speakers also
	understand Indonesian, and therefore return the Balinese language tag as well.</p>
