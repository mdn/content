...
Because `fibonacci()` is an infinite iterator, using a `for` loop to iterate it without any logic to exit early (e.g. using a {{jsxref("Statements/break", "break")}} statement) would result in an infinite loop.
