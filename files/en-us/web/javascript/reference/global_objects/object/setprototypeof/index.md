...
If the `obj` parameter is not an object (e.g. number, string, etc.), this method does nothing without even attempting to set the prototype of `obj`, and directly returns `obj` as a primitive value without coercion. If `prototype` is the same value as the prototype of `obj`, then `obj` is directly returned, without causing a `TypeError` even when `obj` has immutable prototype.
