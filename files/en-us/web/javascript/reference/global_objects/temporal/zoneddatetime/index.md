...
If you know in advance how you want to handle offset ambiguity, consider using the `offset` option to avoid exceptions that are thrown by default. For example, a calendar application will likely want the time zone identifier to "win" so that recurring meetings are shown in the most up-to-date local time for that time zone, and so using `ignore` is appropriate. On the other hand, a task scheduler application that's programmed to execute a task exactly 3 hours from now should probably pick `use` because changes time zone rules shouldn't change the meaning of "3 hours from now".
