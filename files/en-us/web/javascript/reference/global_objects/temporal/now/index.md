...
For example, if the computer is set to the time zone "America/New_York", then `Temporal.Now.zonedDateTimeISO()` may return a zoned date-time like: `2021-08-01T10:40:12.345-04:00[America/New_York]`. Then, `Temporal.Now.plainTimeISO()` may return the time part of this zoned date-time: `10:40:12.345`. However, if you call `Temporal.Now.plainTimeISO("UTC")`, it will return the time part of the zoned date-time in the UTC time zone: `14:40:12.345`. This is especially useful for cross-system communication where the other end may be expecting the time in a different time zone.
