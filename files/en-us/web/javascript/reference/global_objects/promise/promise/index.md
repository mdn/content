...
- `executor`
  - : A {{jsxref("function")}} to be executed by the constructor. It receives two functions as parameters: `resolutionFunc` and `rejectionFunc`. Any errors thrown in the `executor` will cause the promise to reject, and the return value is neglected. The semantics of `executor` are explained in detail below. 
