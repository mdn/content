...
`then()` returns a new promise object but the handler is retained by the promise object that `then()` is called on, and its lifetime is at least the (called on, not returned) promise's lifetime. If you call the `then()` method twice on the same promise object (instead of chaining), then this promise object will have two pairs of settlement handlers. All handlers attached to the same promise object are always called in the order they were added. Moreover, the two promises returned by each call of `then()` start separate chains and do not wait for each other's settlement.
