...
With the help of the {{jsxref("Reflect")}} class we can give some
...
In this simple example, the number `37` gets returned as the default value when
the property name is not in the object. It is using the
{{jsxref("Global_Objects/Proxy/Proxy/get", "get()")}}
handler.
...
In this example, we are using a native JavaScript object to which our proxy will
...
Note that while this "no-op" works for JavaScript objects, it does not work for
native browser objects like DOM Elements.

### Validation

With a `Proxy`, you can easily validate the passed value for an object. This
example uses the
...
A function proxy could easily extend a constructor with a new constructor. This
example uses the
{{jsxref("Global_Objects/Proxy/Proxy/construct", "construct()")}}
and {{jsxref("Global_Objects/Proxy/Proxy/apply", "apply()")}}
handlers.
...
The `products` proxy object evaluates the passed value and converts it to an
...
flexibly "define" properties without using
...
target will be
{{domxref("HTMLTableElement.rows", "table.rows")}}.
...
[a simple cookie framework](https://reference.codeproject.com/Book/dom/document/cookie/simple_document.cookie_framework).
...
- ["Proxies are awesome" Brendan Eich presentation at JSConf](https://www.youtube.com/watch?v=sClk6aB_CPk)
  ([slides](https://www.slideshare.net/BrendanEich/metaprog-5303821))
