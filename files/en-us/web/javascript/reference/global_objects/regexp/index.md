...
- The _literal notation_ takes a pattern between two slashes, followed by optional {{jsxref("RegExp.prototype.flags", "flags")}}, after the second slash.
- The _constructor function_ takes either a string or a `RegExp` object as its first parameter and a string of optional {{jsxref("RegExp.prototype.flags", "flags")}} as its second parameter.
...
The expression `new RegExp(/ab+c/, flags)` will create a new `RegExp` using the source of the first parameter and the {{jsxref("RegExp.prototype.flags", "flags")}} provided by the second.
