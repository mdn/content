...
Import attributes tell the host how a particular module should be loaded.
...
The attributes syntax is designed to be extensible — although only `type` is specified by the language, the host can extend it with other attributes. An attribute can change the host's behavior at every stage of the module loading process:
...
- Parsing and evaluation: the host may use the attribute to determine how to parse and evaluate the module.

However, you cannot use unknown attributes — the host throws an error if it encounters an unknown attribute.
