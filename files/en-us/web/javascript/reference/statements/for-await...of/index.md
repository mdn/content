...
If a sync generator yields a rejected promise, `for await...of` calls the generator's [`return()`](/en-US/docs/Web/JavaScript/Reference/Global_Objects/Generator/return) method before throwing the rejection reason, allowing `finally` blocks within that generator to run.
