...
Other microtasks can execute before the async function resumes. This example uses [`queueMicrotask()`](/en-US/docs/Web/API/Window/queueMicrotask) to demonstrate how the microtask queue is processed when each `await` expression is encountered.
