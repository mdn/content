...
The **right shift operator (`>>`)** returns the signed number represented by the result of performing a sign-extending shift of the binary representation of the first operand (evaluated as a [two’s complement](https://en.wikipedia.org/wiki/Two's_complement) bit string) to the right by the number of bits, modulo 32, specified in the second operand — with excess bits shifted off to the right being discarded, and copies of the leftmost bit shifted in from the left.
...
And remember that the reason the binary representation 11111111111111111111111111111101 is equal to the negative decimal (base 1) number `-3` is that it’s calculated by inverting all the bits of the binary representation of the positive decimal (base 10) number `3`, which is 00000000000000000000000000000011, and then adding one (that is, the [two’s complement](https://en.wikipedia.org/wiki/Two's_complement)).
