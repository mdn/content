...
<p>The deprecated <strong><code>KeyboardEvent.keyCode</code></strong> read-only property represents a system and implementation dependent numerical code identifying the unmodified value of the pressed key.</p>

<p>This is usually the decimal ASCII ({{RFC(20)}}) or Windows 1252 code corresponding to the key. If the key can't be identified, this value is <code>0</code>.</p>

<p>You should avoid using this if possible; it's been deprecated for some time. Instead, you should use {{domxref("KeyboardEvent.code")}}, if it's implemented. Unfortunately, some browsers still don't have it, so you'll have to be careful to make sure you use one which is supported on all target browsers.</p>

<div class="note">
<p><strong>Note:</strong> Web developers shouldn't use the <code>keyCode</code> attribute for printable characters when handling <code>keydown</code> and <code>keyup</code> events. As described above, the <code>keyCode</code> attribute is not useful for printable characters, especially those input with the <kbd>Shift</kbd> or <kbd>Alt</kbd> key pressed. When implementing a shortcut key handler, the {{event("keypress")}} event is usually better (at least when Gecko is the runtime in use). See <a href="/en-US/docs/Gecko_Keypress_Event">Gecko Keypress Event</a> for details.</p>
</div>
...
<h3 id="Printable_keys_in_standard_position">Printable keys in standard position</h3>
...
<p>See "<a href="https://msdn.microsoft.com/en-us/library/aa452679.aspx">Manufacturer-specific Virtual-Key Codes (Windows CE 5.0)</a>" in MSDN for the detail.</p>
