...
If a new {{domxref("AudioContext")}} is created with a `sinkId` value provided of `{ type: 'none' }`, calling {{domxref("AudioContext.sinkId")}} later in the code will return a `AudioSinkInfo` object containing `type: 'none'`. This is currently the only value available.
