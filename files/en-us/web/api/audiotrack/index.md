...
- {{domxref("AudioTrack.label", "label")}} {{ReadOnlyInline}}
  - : A string providing a human-readable label for the track. For example, an audio commentary track for a movie might have a `label` of `"Commentary with director Elaina A. Madura and actors Voytek Kablowski and Shilpa Seetharanan."` This string is empty if no label is provided.
