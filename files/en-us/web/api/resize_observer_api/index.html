...
<p>Usage is simple, and pretty much the same as other observers, such as <a href="/en-US/docs/Web/API/PerformanceObserver">Performance Observer</a> or <a href="/en-US/docs/Web/API/Intersection_Observer_API">Intersection Observer</a> — you create a new {{domxref("ResizeObserver")}} object using the <code><a href="/en-US/docs/Web/API/ResizeObserver/ResizeObserver">ResizeObserver()</a></code> constructor, then use {{domxref("ResizeObserver.observe()")}} to make it look for changes to a specific element's size. A callback function set up inside the constructor then runs every time the size changes, providing access to the new dimensions and allowing you to do anything you like in response to those changes.</p>

<h2 id="Interfaces">Interfaces</h2>

<dl>
 <dt>{{domxref("ResizeObserver")}}</dt>
 <dd>Provides the ability to register new observers and to start and stop observing elements.</dd>
 <dt>{{domxref("ResizeObserverEntry")}}</dt>
 <dd>Describes a single element which has been resized, identifying the element and its new size.</dd>
</dl>
...
<table>
  ...
  <tr>
   <td><a href="https://drafts.csswg.org/resize-observer/">Resize Observer</a></td>
  </tr>
</table>
