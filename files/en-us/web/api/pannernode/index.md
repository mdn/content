...
The `PannerNode` interface is an audio-processing module that represents the location, direction, and behavior of an audio source signal in simulated physical space. This {{domxref("AudioNode")}} uses right-hand Cartesian coordinates to describe the source's _position_ as a vector and its _orientation_ as a 3D cone.
