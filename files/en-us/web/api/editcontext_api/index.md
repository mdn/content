...
On the web, editable regions are often [`<textarea>`](/en-US/docs/Web/HTML/Reference/Element/textarea) elements, [`<input>`](/en-US/docs/Web/HTML/Reference/Element/input) elements, or elements with the [`contenteditable`](/en-US/docs/Web/HTML/Global_attributes/contenteditable) attribute set to `true`. For these elements, the browser automatically provides the text edit context, and web authors are not required to write any code to support text input.

### Creating custom editable regions

Web authors can also create custom editable regions using the EditContext API. For example, a web author could create a rich text editor using a [`<canvas>`](/en-US/docs/Web/HTML/Reference/Element/canvas) element to render the text. In this case, the web author needs to write code to support text input.
