...
To use the live demo, open [Edit Context API: HTML editor demo](https://mdn.github.io/dom-examples/edit-context/html-editor/).

## Creating the editor UI

The first step is to create the UI for the editor. The editor is a simple {{HTMLElement("div")}} element with the [`spellcheck`](/en-US/docs/Web/HTML/Global_attributes/spellcheck) attribute set to `false` to disable spell checking:
...
To make the editor editable, the demo app creates an {{domxref("EditContext")}} instance, and then sets the {{DOMxRef("HTMLElement.editContext", "editContext")}} property of the editor element to the `EditContext` instance:
...
To render the syntax-highlighted HTML code in the editor when the user enters text, the demo app uses a function, `render()`, that's called when new text is entered, when characters are deleted, or when the selection is changed.
...
The demo app uses the `tokenizeHTML` function to achieve this, which iterates over the string character by character while maintaining a state machine.

You can see the source code for the `tokenizeHTML` function in [tokenizer.js](https://github.com/mdn/dom-examples/blob/main/editcontext-api/html-editor/tokenizer.js), on GitHub.
...
The EditContext API gives us the ability to control the way the edited text is rendered. The above function renders it by using HTML elements, but it could render it in any other way, including by rendering it into a `<canvas>` element.
...
To handle them, the demo app uses an event listener for the {{domxref("keydown_event", "keydown")}} event on the editor element, and uses this listener to update the `EditContext` instance's text content and selection, as shown below:
...
We could improve the demo app by handling other key combinations, such as <kbd>Ctrl</kbd>+<kbd>C</kbd> and <kbd>Ctrl</kbd>+<kbd>V</kbd> to copy and paste text, or <kbd>Ctrl</kbd>+<kbd>Z</kbd> and <kbd>Ctrl</kbd>+<kbd>Y</kbd> to undo and redo text changes.

## Updating the selection state and selection bounds

As we saw earlier, the `render()` function handled rendering the current user selection in the editor element. But the demo app also needs to _update_ the selection state and bounds when the user changes the selection. The EditContext API doesn't do this for automatically, again because the editor UI might be implemented in a different way, such as by using a `<canvas>` element.
...
However, because the EditContext API uses character offsets to represent the selection, the demo app also uses a function, `convertFromSelectionToOffsets()` that converts DOM selection objects to character offsets.
...
As shown in the following code snippet, the demo app uses the `textformatupdate` event and the `convertFromOffsetsToSelection()` function again to find the text range that the IME composition wants to format.
...
With this in place, the demo app creates {{domxref("Range")}} objects for the ranges that need to be styled, and adds them to the `Highlight` object:
