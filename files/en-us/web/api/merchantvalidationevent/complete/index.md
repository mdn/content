...
All you have to do is call `complete()` from your handler for the {{domxref("PaymentRequestk.merchantvalidation_event", "merchantvalidation")}} event, passing in the data fetched from the `validationURL`.
...
This code sets up a handler for the {{domxref("PaymentRequestk.merchantvalidation_event", "merchantvalidation")}} event. The event handler calls a function, `getValidationData()`, which retrieves the data from the validation URL, then passes that data (or a promise to deliver the data) into `complete()`.
