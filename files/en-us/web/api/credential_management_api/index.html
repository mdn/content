...
<div class="note">
<p><strong>Note:</strong> This API is restricted to top-level contexts. Calls to <code>get()</code> and <code>store()</code> within an <code>&lt;iframe&gt;</code> element will resolve without effect.</p>
</div>

<h3 id="Subdomain-shared_credentials">Subdomain-shared credentials</h3>

<p>Later version of the spec allow credentials to be retrieved from a different subdomain. For example, a password stored in <code>login.example.com</code> may be used to log in to <code>www.example.com</code>. To take advantage of this, a password must be explicitly stored by calling {{domxref("CredentialsContainer.store()")}}. This is sometimes referred to as public suffix list (PSL) matching; however the spec only <em>recommends</em> using PSL to determine the effective scope of a credential. It does not require it. Hence browsers may vary in their implementation.</p>
...
<dl>
 ...
 <dd>Provides a credential for logging in using a more secure system based on asymmetric cryptography instead of a password.</dd>
</dl>
