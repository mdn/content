...
<div>
  <p>The <strong><code>get()</code></strong> method of the {{domxref("IDBIndex")}}
    interface returns an {{domxref("IDBRequest")}} object, and, in a separate thread,
    finds either the value in the referenced object store that corresponds to the given
    key or the first corresponding value, if <code>key</code> is set to an
    {{domxref("IDBKeyRange")}}.</p>
</div>

<p>If a value is found, then a structured clone of it is created and set as the
  <code>result</code> of the request object: this returns the record the key is associated
  with.</p>
...
<pre
  class="brush: js">var <em>request</em> = <em>myIndex</em>.get(<em>key</em>);</pre>

<h3 id="Parameters">Parameters</h3>

<dl>
  <dt>key {{optional_inline}}</dt>
  <dd>A key or {{domxref("IDBKeyRange")}} that identifies the record to be retrieved. If
...
<p><code>myIndex.get('Bungle')</code> is then used to retrieve the record with an
  <code>lName</code> of <code>Bungle</code>, and the result of that request is logged to
  the console when its success callback returns.</p>
