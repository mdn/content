...
<p><strong>Warning:</strong> This property is IE specific. Although it is well supported by IE, most other browsers no longer support this property. This property should only be used as one of the solutions when you need to be compatible with lower versions of IE, rather than relying on it completely in cross browser scripts.</p></div>
...
<dl>
 ...
 <dd>Returns a boolean value indicating whether the specified command can be executed successfully with the <code>execCommand</code> method in the current state of the given document. You can also see {{domxref("Document.queryCommandEnabled()")}}.</dd>
 <dt>{{domxref("TextRange.queryCommandState()")}}</dt>
 <dd>Returns the boolean value indicating the current state of the specified command. You can also see {{domxref("Document.queryCommandState()")}}.</dd>
...
<div class="notecard warning">
<p><strong>Warning:</strong> Valid only under <strong>IE9</strong>. The {{domxref("selection")}} interface should be used first, if the browser allows it.</p>
</div>
...
<ul>
 ...
 <li><a href="/en-US/docs/Web/API/Selection">Selection API</a> to replace this non-standard interface</li>
</ul>
