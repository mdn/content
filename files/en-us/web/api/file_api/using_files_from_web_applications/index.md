...
The DOM {{DOMxRef("URL.createObjectURL_static")}} and {{DOMxRef("URL.revokeObjectURL_static")}} methods let you create simple URL strings that can be used to reference any data that can be referred to using a DOM {{DOMxRef("File")}} object, including local files on the user's computer.
...
The object URL is a string identifying the {{DOMxRef("File")}} object. Each time you call {{DOMxRef("URL.createObjectURL_static")}}, a unique object URL is created even if you've created an object URL for that file already. Each of these must be released. While they are released automatically when the document is unloaded, if your page uses them dynamically you should release them explicitly by calling {{DOMxRef("URL.revokeObjectURL_static")}}:
...
3. For each {{DOMxRef("File")}} in the {{DOMxRef("FileList")}} represented by `files`:
   ...
   3. Set the image's source to a new object URL representing the file, using {{DOMxRef("URL.createObjectURL_static")}} to create the blob URL.
   4. Set the image's height to 60 pixels.
   5. Set up the image's load event handler to release the object URL since it's no longer needed once the image has been loaded. This is done by calling the {{DOMxRef("URL.revokeObjectURL_static")}} method and passing in the object URL string as specified by `img.src`.
