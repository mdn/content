...
<p>The <code>readAsDataURL()</code> method of the {{DOMxRef("FileReaderSync")}} interface allows to read {{DOMxRef("File")}} or {{DOMxRef("Blob")}} objects in a synchronous way into an {{DOMxRef("DOMString")}} representing a data URL. This interface is <a href="/en-US/docs/Web/API/Web_Workers_API/Functions_and_classes_available_to_workers">only available</a> in <a href="/en-US/docs/Web/API/Worker">workers</a> as it enables synchronous I/O that could potentially block.</p>

<h2 id="Syntax">Syntax</h2>

<pre class="eval">readAsDataURL(<em>File</em>);
...
<p>An {{DOMxRef("DOMString")}} representing the input data as a data URL.</p>
