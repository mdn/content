---
title: MIDIInputMap
slug: Web/API/MIDIInputMap
page-type: web-api-interface
browser-compat: api.MIDIInputMap
---

{{APIRef("Web MIDI API")}}

The **`MIDIInputMap`** read-only interface of the [Web MIDI API](/en-US/docs/Web/API/Web_MIDI_API) provides the set of MIDI input ports that are currently available.

`MIDIInputMap` is a read-only [`Map`-like object](/en-US/docs/Glossary/Maplike_object), in which each key is the ID string for MIDI input, and the associated value is the corresponding {{domxref("MIDIInput")}} object.

## Specifications

{{Specifications}}

## Browser compatibility

{{Compat}}
