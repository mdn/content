...
> [!WARNING]
> While the `open` property is technically not read only and can be set programmatically, doing so is strongly discouraged by [the HTML specification](https://html.spec.whatwg.org/multipage/interactive-elements.html#attr-dialog-closedby:~:text=Removing%20the%20open%20attribute%20will%20usually%20hide%20the%20dialog.%20However%2C%20doing%20so%20has%20a%20number%20of%20strange%20additional%20consequences%3A), as it can break normal dialog interactions in unexpected ways. For example, the [`close` event](/en-US/docs/Web/API/HTMLDialogElement/close_event) won't fire when programmatically setting `open` to `false`, and subsequent calls to the [`close()`](/en-US/docs/Web/API/HTMLDialogElement/close) and [`requestClose()`](/en-US/docs/Web/API/HTMLDialogElement/requestClose) methods will have no effect. Instead, it's better to use the [`show()`](/en-US/docs/Web/API/HTMLDialogElement/show), [`showModal()`](/en-US/docs/Web/API/HTMLDialogElement/showModal), `close()`, and `requestClose()` methods to change the value of the `open` attribute.
