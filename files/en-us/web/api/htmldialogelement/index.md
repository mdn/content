...
The following example shows a button that, when clicked, opens a modal {{htmlelement("dialog")}} containing a form via the {{domxref("HTMLDialogElement.showModal()")}} function.
...
The events that are fired as the dialog is shown/opened and closed are logged.
...
It then adds a listener to call the {{domxref("HTMLDialogElement.showModal()")}} function when the Update button is clicked.
...
Next we add listeners to the _Confirm_ and _Cancel_ buttons.
...
```js
// Form Confirm button closes dialog if there is a selection.
// Selection value passed to close() to set dialog.returnValue.
...
cancelButton.addEventListener("click", () => {
  dialog.close(); // Clear any selection on cancel
});
```

Calling `close()` also fires the {{domxref("HTMLDialogElement/close_event", "close")}} event, which we implement below by simply logging the return value of the dialog.
...
Next we implement the {{domxref("HTMLDialogElement/cancel_event", "cancel")}} event.
This is a cancelable event that is fired when "platform specific methods" are used to close the dialog, such as the <kbd>Esc</kbd> key.
...
Next we add a listener for the `HTMLElement` [`toggle`](/en-US/docs/Web/API/HTMLElement/toggle_event) event, which is fired just after the dialog has opened and also just after it has closed (but before the `closed` event).
We use this to log when the Dialog opens and closes.
...
dialog.addEventListener("toggle", (event) => {
  ...
  //log(`oldstate: ${event.oldState}, newState: ${event.newState}`);
});
```

The final event that is fired when a dialog is opened and closed is `HTMLElement` [`beforetoggle`](/en-US/docs/Web/API/HTMLElement/beforetoggle_event).
