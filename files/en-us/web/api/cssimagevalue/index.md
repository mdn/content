...
The CSSImageValue object represents an [`<image>`](/en-US/docs/Web/CSS/Reference/Values/image) that involves a URL, such as {{cssxref("url_function", "url()")}} or {{cssxref("image()")}}, but not {{cssxref("gradient/linear-gradient", "linear-gradient()")}} or {{cssxref("element()")}}.
