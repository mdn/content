...
{{domxref("GPUCanvasContext")}} interface configures the context for use with a given {{domxref("GPUDevice")}}. When called the canvas will initially be cleared to transparent black.
  ...
  - : An object containing the following properties:
    ...
    - `usage` {{optional_inline}}
      ...
      - : The allowed usage for textures returned by `getCurrentTexture()`. Possible values are:
        ...
        - `GPUTextureUsage.RENDER_ATTACHMENT`: The texture can be used as a color attachment in a render pass, for example in a color attachment view in a {{domxref("GPUCommandEncoder.beginRenderPass()")}} call. This is the default, but note that it is not automatically included if a different value is explicitly set.
