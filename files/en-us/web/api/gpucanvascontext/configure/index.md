...
{{domxref("GPUCanvasContext")}} interface configures the context for this canvas, and clears the drawing buffer to transparent black.
...
- `configuration`
  - : An object containing the following properties:
    ...
    - `format`
      - : The format that textures returned by `getCurrentTexture()` will have. This can be `bgra8unorm`, `rgba8unorm`, or `rgba16float`. The optimal canvas texture format for the current system can be returned by {{domxref("GPU.getPreferredCanvasFormat()")}}.
    - `usage` {{optional_inline}}
      - : The usage that textures returned by `getCurrentTexture()` will have. Possible values are:
        ...
        - `GPUTextureUsage.COPY_DST`: The texture can be used as the source of a copy/write operation, for example the destination argument of a {{domxref("GPUCommandEncoder.copyTextureToTexture()")}} call.
        - `GPUTextureUsage.RENDER_ATTACHMENT`: The texture can be used as a color or depth/stencil attachment in a render pass. for example in a color attachment view in a {{domxref("GPUCommandEncoder.beginRenderPass()")}} call. This is the default, but note that it is not automatically included if a different value is explicitly set.
