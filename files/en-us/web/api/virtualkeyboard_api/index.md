...
Web browsers usually deal with virtual keyboards on their own, by adjusting the viewport height and by scrolling to the focused input field.

The figure below illustrates the difference between the viewport height and scroll position of a web page when the device has its on-screen virtual keyboard hidden and then shown.

![Two devices, on without a virtual keyboard, showing that the webpage can use most of the device's vertical space, and one with a virtual keyboard, showing that the webpage can only be displayed in the remaining space](viewport-height.png)

However, more complex applications or specific devices such as multi-screen mobile phones may require more control of the layout when the virtual keyboard appears.
...
The VirtualKeyboard API can be used to opt-out of the way the browser automatically handles the virtual keyboard, and take full control of it instead. With the Virtual Keyboard API, the keyboard still appears and disappears as necessary when form controls are focused, but the viewport does not change, and you can use JavaScript and CSS to adapt your layout.

## Concept

The VirtualKeyboard API consists of three parts:

- The VirtualKeyboard interface, accessed through {{domxref('navigator.virtualKeyboard')}}, is used to opt-out of the automatic virtual keyboard behavior, show or hide the virtual keyboard programmatically, as well as get the current position and size of the virtual keyboard.
- The `keyboard-inset-*` CSS environment variables provide information about the virtual keyboard's position and size.
- The `virtualkeyboardpolicy` attribute to choose whether the virtual keyboard should appear on `contenteditable` elements.

### Opt-out of the automatic virtual keyboard behavior

To opt-out of the automatic virtual keyboard behavior of the browser, use `navigator.virtualKeyboard.overlaysContent = true`. The browser will no longer automatically resize the viewport to make space for the virtual keyboard which will, now, overlay your content.

You can then detect the geometry of the virtual keyboard by using JavaScript or CSS to adapt your layout.

### Detect the virtual keyboard geometry using JavaScript

You can get the current geometry of the virtual keyboard by using `navigator.virtualKeyboard.boundingRect` as well as listen to geometry changes, such as when the keyboard is shown or hidden, by using the `geometrychange` event.

This is useful to position important elements of your layouts in the area that's not covered by the virtual keyboard.

The code snippet below uses the `geometrychange` event to know when the virtual keyboard geometry changes, and then accesses the `boundingRect` property to know the size and position of the virtual keyboard:
...
The VirtualKeyboard API also exposes the following {{cssxref("env", "CSS environment variables")}}: `keyboard-inset-top`, `keyboard-inset-right`, `keyboard-inset-bottom`, `keyboard-inset-left`, `keyboard-inset-width`, and `keyboard-inset-height`.
