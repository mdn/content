...
- {{domxref("SVGSVGElement.currentTranslate")}} {{ReadOnlyInline}}
  - : A {{domxref("DOMPoint")}} representing the translation factor that takes into account user "magnification" corresponding to an outermost {{SVGElement("svg")}} element. The behavior is undefined for `<svg>` elements that are not at the outermost level.
