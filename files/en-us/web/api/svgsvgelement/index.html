...
<dl>
	...
	<ul>
		...
		<li>the values for {{SVGAttr("transform")}} and {{SVGAttr("viewTarget")}} within {{SVGAttr("currentView")}} will be <code>null</code></li>
	</ul>
	...
	<ul>
		...
		<li>the values for {{SVGAttr("transform")}} within {{SVGAttr("currentView")}} will be <code>null</code></li>
		<li>the {{SVGAttr("viewTarget")}} within {{SVGAttr("currentView")}} will represent the target of the link</li>
	</ul>
	If the initial view was a link into the SVG document fragment using an SVG view specification fragment identifier (i.e., <code>#svgView(…)</code>), then:
	...
	<dd>On an outermost {{SVGElement("svg")}} element, this float attribute indicates the current scale factor relative to the initial view to take into account user magnification and panning operations. DOM attributes <code>currentScale</code> and <code>currentTranslate</code> are equivalent to the 2×3 matrix <code>[a b c d e f] = [<var>currentScale</var> 0 0 <var>currentScale</var> <var>currentTranslate</var>.x <var>currentTranslate</var>.y]</code>. If "magnification" is enabled (i.e., <code>zoomAndPan="magnify"</code>), then the effect is as if an extra transformation were placed at the outermost level on the SVG document fragment (i.e., outside the outermost {{SVGElement("svg")}} element).</dd>
	<dt>{{domxref("SVGSVGElement.currentTranslate")}} {{ReadOnlyInline}}</dt>
	<dd>An {{domxref("SVGPoint")}} representing the translation factor that takes into account user "magnification" corresponding to an outermost {{SVGElement("svg")}} element. The behavior is undefined for <code>&lt;svg&gt;</code> elements that are not at the outermost level.</dd>
</dl>
...
<dl>
	...
	the corresponding <code>unsuspendRedraw()</code> call has been made, an <code>unsuspendRedrawAll()</code> call has been made, or its timer has timed out.
	...
	<pre class="brush: js">const suspendHandleID = suspendRedraw(<var>maxWaitMilliseconds</var>)</pre>

	<p>and follow the changes with a method call similar to:</p>

	<pre class="brush: js">unsuspendRedraw(<var>suspendHandleID</var>)</pre>

	<p>Note that multiple <code>suspendRedraw()</code> calls can be used at once, and that each such method call is treated independently of the other <code>suspendRedraw()</code> method calls.</p>
	</dd>
	...
	<dd>Returns <code>true</code> if this SVG document fragment is in a paused state.</dd>
	<dt>{{domxref("SVGSVGElement.getCurrentTime()")}}</dt>
	<dd>Returns the current time in seconds relative to the start time for the current SVG document fragment. If <code>getCurrentTime()</code> is called before the document timeline has begun (for example, by script running in a {{SVGElement("script")}} element before the document's <code>SVGLoad</code> event is dispatched), then <code>0</code> is returned.</dd>
	<dt>{{domxref("SVGSVGElement.setCurrentTime()")}}</dt>
	<dd>Adjusts the clock for this SVG document fragment, establishing a new current time. If <code>setCurrentTime()</code> is called before the document timeline has begun (for example, by script running in a {{SVGElement("script")}} element before the document's <code>SVGLoad</code> event is dispatched), then the value of seconds in the last invocation of the method gives the time that the document will seek to once the document timeline has begun.</dd>
	...
	<dd>Creates an {{domxref("SVGNumber")}} object outside of any document trees. The object is initialized to <code>0</code>.</dd>
	<dt>{{domxref("SVGSVGElement.createSVGLength()")}}</dt>
	<dd>Creates an {{domxref("SVGLength")}} object outside of any document trees. The object is initialized to <code>0</code> user units.</dd>
	<dt>{{domxref("SVGSVGElement.createSVGAngle()")}}</dt>
	<dd>Creates an {{domxref("SVGAngle")}} object outside of any document trees. The object is initialized to a value of <code>0</code> degrees (unitless).</dd>
	<dt>{{domxref("SVGSVGElement.createSVGPoint()")}}</dt>
	<dd>Creates an {{domxref("SVGPoint")}} object outside of any document trees. The object is initialized to the point <code>(0,0)</code> in the user coordinate system.</dd>
	...
	<dd>Creates an {{domxref("SVGRect")}} object outside of any document trees. The object is initialized such that all values are set to <code>0</code> user units.</dd>
	...
	<dd>Searches this SVG document fragment (i.e., the search is restricted to a subset of the document tree) for an Element whose <code>id</code> is given by <code><var>elementId</var></code>. If an Element is found, that Element is returned. If no such element exists, returns <code>null</code>. Behavior is not defined if more than one element has this id.</dd>
</dl>
