...
<p>The <strong><code>NodeIterator.whatToShow</code></strong> read-only property represents
	an <code>unsigned integer</code> representing a bitmask signifying what types of nodes
	should be returned by the {{domxref("NodeIterator")}}.</p>

<h2 id="Syntax">Syntax</h2>

<pre class="brush: js">var nodeTypes = <em>nodeIterator</em>.whatToShow;
</pre>
	...
	<tbody>
		...
		<tr>
			...
			<td>Shows attribute {{ domxref("Attr") }} nodes. This is meaningful only when
				creating a {{ domxref("NodeIterator") }} or {{ domxref("TreeWalker") }}
				with an {{ domxref("Attr") }} node as its root; in this case, it means
				that the attribute node will appear in the first position of the iteration
				or traversal. Since attributes are never children of other nodes, they do
				not appear when traversing over the document tree.</td>
		</tr>
		<tr>
