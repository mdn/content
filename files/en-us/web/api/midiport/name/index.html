---
title: MIDIPort.name
slug: Web/API/MIDIPort/name
tags:
  - API
  - Property
  - Reference
  - name
  - MIDIPort
browser-compat: api.MIDIPort.name
---
<div>{{securecontext_header}}{{DefaultAPISidebar("Web MIDI API")}}</div>

<p>The <strong><code>name</code></strong> read-only property of the {{domxref("MIDIPort")}} interface returns the system name of the port.</p>

<h2 id="Syntax">Syntax</h2>

<pre class="brush: js">let aname = MIDIPort.name;
</pre>

<h3>Value</h3>
<p>A {{domxref("DOMString","string")}} containing the system name of the port.</p>

<h2 id="Examples">Examples</h2>

<p>The following example loops through all input ports and prints the name of each to the console.</p>

<pre class="brush: js">for (let entry of midiAccess.inputs) {
  let input = entry[1];
  console.log(input.name);
}</pre>

<h2 id="Specifications">Specifications</h2>

<p>{{Specifications}}</p>

<h2 id="Browser_compatibility">Browser compatibility</h2>

<p>{{Compat}}</p>
