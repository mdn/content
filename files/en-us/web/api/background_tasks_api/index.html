...
<p>Because the Background Tasks API is fairly new, your code may need to be able to work on browsers that don't yet support it. You can do so with a simple shim that uses {{domxref("setTimeout()")}} as a fallback option. This isn't a {{Glossary("polyfill")}}, since it's not functionally identical; <code>setTimeout()</code> doesn't let you make use of idle periods, but instead runs your code when possible, leaving us to do the best we can to avoid causing the user to experience performance lag.</p>
...
<p>If <code>cancelIdleCallback()</code> isn't defined, this creates one which passes the specified callback ID through to {{domxref("clearTimeout()")}}.</p>
...
<pre class="brush: js hidden">window.requestIdleCallback = window.requestIdleCallback || function(handler) {
  let startTime = Date.now();
...
<pre class="brush: js hidden">function getRandomIntInclusive(min, max) {
  min = Math.ceil(min);
  max = Math.floor(max);
