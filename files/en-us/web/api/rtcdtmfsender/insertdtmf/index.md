...
The **`insertDTMF()`** method on the {{domxref("RTCDTMFSender")}} interface starts sending {{Glossary("DTMF")}} tones to the remote peer over the {{domxref("RTCPeerConnection")}}.

Sending of the tones is performed asynchronously, with [`tonechange`](/en-US/docs/Web/API/RTCDTMFSender/tonechange_event) events sent to the `RTCDTMFSender` every time a tone starts or ends.

As long as the connection is active, you can send tones at any time. Calling `insertDTMF()` replaces any already-pending tones from the `toneBuffer`.
...
- `InvalidStateError` {{domxref("DOMException")}}
  - : Thrown if the DTMF tones couldn't be sent because the track has been stopped or is in a read-only or inactive state.
