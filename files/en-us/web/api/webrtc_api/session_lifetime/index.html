...
<p>This is where signaling comes in.</p>
...
<p>In order to exchange signaling information, you can choose to send JSON objects back and forth over a WebSocket connection, or you could use XMPP or SIP over an appropriate channel, or you could use {{domxref("XMLHttpRequest")}} over {{Glossary("HTTPS")}} with polling, or any other combination of technologies you can come up with. You could even use email as the signaling channel.</p>
...
<ol>
 <li>Each peer creates an {{domxref("RTCPeerConnection")}} object representing their end of the WebRTC session.</li>
 <li>Each peer establishes a handler for {{event("icecandidate")}} events, which handles sending those candidates to the other peer over the signaling channel.</li>
 <li>Each peer establishes a handler for {{event("track")}} event, which is received when the remote peer adds a track to the stream. This code should connect the tracks to its consumer, such as a {{HTMLElement("video")}} element.</li>
...
<p>If you need to change the configuration of the connection in some way (such as changing to a different set of ICE servers), you can do so before restarting ICE by calling {{domxref("RTCPeerConnection.setConfiguration()")}} with an updated {{domxref("RTCConfiguration")}} dictionary before restarting ICE.</p>

<p>To explicitly trigger ICE restart, start the renegotiation process by calling {{domxref("RTCPeerConnection.createOffer()")}}, specifying the <code>iceRestart</code> option with a value of <code>true</code>. Then handle the connection process from then on just like you normally would. This generates new values for the ICE username fragment (ufrag) and password, which will be used by the renegotiation process and the resulting connection.</p>

<p>The answerer side of the connection will automatically begin ICE restart when new values are detected for the ICE ufrag and ICE password.</p>
