...
<p>In this guide, we'll examine how to add a data channel to a peer connection, which can then be used to securely exchange arbitrary data; that is, any kind of data we wish, in any format we choose.</p>

<div class="note">
<p><strong>Note:</strong> Since all WebRTC components are required to use encryption, any data transmitted on an <code>RTCDataChannel</code> is automatically secured using Datagram Transport Layer Security (<strong>DTLS</strong>). See {{anch("Security")}} below for more information.</p>
</div>
...
<p><strong>Note:</strong> Much of the information in this section is based in part on the blog post <a href="https://lgrahl.de/articles/demystifying-webrtc-dc-size-limit.html">Demystifying WebRTC's Data Channel Message Size Limitations</a>, written by Lennart Grahl. He goes into a bit more detail there, but as browsers have been updated since then some of it may be out-of-date. In addition, as time goes by, it will become more so, especially once EOR and ndata support are fully integrated in the major browsers.</p>
</div>
