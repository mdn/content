...
In Firefox 60 onwards, Mozilla resolves the relative URL against the global that owns the `fetch()` function being used (see [Firefox bug 1432272](https://bugzilla.mozilla.org/show_bug.cgi?id=1432272)). So in the case described above, it is resolved against the iframe's location:
