...
<p>The {{domxref("GeolocationCoordinates")}} interface's read-only
  <code><strong>longitude</strong></code> property is a double-precision floating point
  ...
  the <code>GeolocationCoordinates</code> object is part of the
  {{domxref("GeolocationPosition")}} interface, which is the object type returned by
  Geolocation API functions that obtain and return a geographical position.</p>

<h2 id="Syntax">Syntax</h2>

<pre class="brush: js"><var>let longitude</var> = <var>geolocationCoordinatesInstance</var>.longitude
</pre>

<h3 id="Value">Value</h3>

<p>The value in <code>longitude</code> is the geographical longitude of the location on
  Earth described by the <code>Coordinates</code> object, in decimal degrees. The value is
  defined by the World Geodetic System 1984 specification (WGS 84).</p>

<div class="note">
  <p><strong>Note:</strong> The zero meridian (also known as the prime meridian or the
...
<p>In this simple example, we fetch the user's location and display the resulting
  coordinates once they're returned.</p>

<h3 id="JavaScript">JavaScript</h3>

<p>The JavaScript code below creates an event listener so that when the user clicks on a
  button, the location information is retrieved and displayed.</p>

<pre class="brush: js">let button = document.getElementById("get-location");
...
<p>After setting up variables to more conveniently reference the button element and the
  ...
  and display the location information.</p>

<p>Upon receiving a {{event("click")}} event, we call
  ...
  determined position.</p>

<p>From the <code>GeolocationPosition</code> object, we obtain the user's latitude and
  longitude using {{domxref("GeolocationCoordinates/latitude", "position.coords.latitude")}} and
  <code>position.coords.longitude</code> so we can update the displayed coordinates. The
  two {{HTMLElement("span")}} elements are updated to display the corresponding values
  after being converted to a value with two decimal places.</p>
...
<ul>
  ...
  <li>The {{domxref("GeolocationPosition")}} interface, which is the top-level interface
    used to return geolocation data from the Geolocation API functions
    {{domxref("Geolocation.getCurrentPosition()")}} and
    {{domxref("Geolocation.watchPosition", "watchPosition()")}}.</li>
</ul>
