...
---
<div>{{securecontext_header}}{{APIRef("Gamepad API")}}</div>

<p>The <strong><code>Gamepad</code></strong> interface of the <a href="/en-US/docs/Web/API/Gamepad_API">Gamepad API</a> defines an individual gamepad or other controller, allowing access to information such as button presses, axis positions, and id.</p>

<p>A Gamepad object can be returned in one of two ways: via the <code>gamepad</code> property of the {{event("gamepadconnected")}} and {{event("gamepaddisconnected")}} events, or by grabbing any position in the array returned by the {{domxref("Navigator.getGamepads()")}} method.</p>

<h2 id="Properties">Properties</h2>

<dl>
 <dt>{{domxref("Gamepad.axes")}} {{readonlyInline}}</dt>
 <dd>An array representing the controls with axes present on the device (e.g. analog thumb sticks).</dd>
 <dt>{{domxref("Gamepad.buttons")}} {{readonlyInline}}</dt>
 <dd>An array of {{domxref("gamepadButton")}} objects representing the buttons present on the device.</dd>
