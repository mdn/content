...
<p>We present our main interface in two columns. On the left is a start button and a {{HTMLElement("video")}} element which displays the video preview; this is the video the user's camera sees. Note that the {{htmlattrxref("autoplay", "video")}} attribute is used so that as soon as the stream starts to arrive from the camera, it immediately gets displayed, and the {{htmlattrxref("muted", "video")}} attribute is specified to ensure that the sound from the user's microphone isn't output to their speakers, causing an ugly feedback loop.</p>
...
<p>On the right we see a stop button and the <code>&lt;video&gt;</code> element which will be used to play back the recorded video. Notice that the playback panel doesn't have autoplay set (so the playback doesn't start as soon as media arrives), and it has {{htmlattrxref("controls", "video")}} set, which tells it to show the user controls to play, pause, and so forth.</p>

<p>Below the playback element is a button for downloading the recorded video.</p>

<pre class="brush: html hidden">&lt;div class="bottom"&gt;
  &lt;pre id="log"&gt;&lt;/pre&gt;
&lt;/div&gt;</pre>
...
<p>The <code>log()</code> function is used to output text strings to a {{HTMLElement("div")}} so we can share information with the user. Not very pretty but it gets the job done for our purposes.</p>
...
<p>The <code>wait()</code> function returns a new {{jsxref("Promise")}} which resolves once the specified number of milliseconds have elapsed. It works by using an <a href="/en-US/docs/Web/JavaScript/Reference/Functions/Arrow_functions">arrow function</a> which calls {{domxref("setTimeout()")}}, specifying the promise's resolution handler as the timeout handler function. That lets us use promise syntax when using timeouts, which can be very handy when chaining promises, as we'll see later.</p>
...
<dl>
 ...
 <dt>Line 5</dt>
 <dd>Sets up the handler for the {{event("dataavailable")}} event. The received event's <code>data</code> property is a {{domxref("Blob")}} that contains the media data. The event handler pushes the <code>Blob</code> onto the <code>data</code> array.</dd>
 <dt>Lines 6-7</dt>
...
<p>The <code>stop()</code> function stops the input media:</p>
...
<p>This calls the <code>{{anch("Stopping the input stream", "stop()")}}</code> function we covered earlier.</p>

<h3 id="Result">Result</h3>

<p>When put all together with the rest of the HTML and the CSS not shown above, it looks and works like this:</p>

<p>{{ EmbedLiveSample('Example_of_recording_a_media_element', 600, 440, "", "", "", "camera;microphone") }}</p>

<p>You can {{LiveSampleLink("Example_of_recording_a_media_element", "view the full demo here")}}, and use your browsers developer tools to inspect the page and look at all the code, including the parts hidden above because they aren't critical to the explanation of how the APIs are being used.</p>
