...
It first uses the information found in each manifest to present the payment apps to the user in the browser-provided Payment Request UI in the merchant app ([`name`](/en-US/docs/Web/Manifest/name), [`icons`](/en-US/docs/Web/Manifest/icons)).

Next, the {{domxref("PaymentRequest.show()")}} method should be invoked by the merchant app in response to a user gesture:

- If there are multiple payment app options, a list of options is presented to the user from them to choose from.
- If there is only one payment app option, the user gesture is still required to continue — along the lines of clicking a "proceed to payment" button.

This starts the payment flow, which causes the browser to Just-In-Time (JIT) install the web app, registering the service worker specified in [`serviceworker`](/en-US/docs/Web/Manifest/serviceworker) so it can handle the payment.
