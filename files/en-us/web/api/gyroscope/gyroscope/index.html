...
<p>The <strong><code>Gyroscope()</code></strong> constructor
    creates a new {{domxref("Gyroscope")}} object which provides on each reading the
    angular velocity of the device along all three axes.</p>
...
<pre
  class="brush: js">var <var>gyroscope</var> = new Gyroscope([<em>options</em>])</pre>
  ...
  <dd>Options are as follows:
    <ul>
      <li><code>frequency</code>: The desired number of times per second a sample should
        ...
        depends device hardware and consequently may be less than requested.</li>
