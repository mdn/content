...
> **Note:** This won't work on the same browse context that is making the changes — it is really a way for other browse contexts on the domain using the storage to sync any changes that are made. Browse contexts on other domains can't access the same storage objects.
