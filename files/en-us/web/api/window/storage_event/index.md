...
> **Note:** This won't work on the same [browser context](/en-US/docs/Glossary/Browsing_context) that is making the changes — it is really a way for other browser contexts on the domain using the storage to sync any changes that are made. Browser contexts on other domains can't access the same storage objects.
