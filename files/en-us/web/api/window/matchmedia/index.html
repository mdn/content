...
<p>The {{domxref("Window")}} interface's <code><strong>matchMedia()</strong></code> method
  returns a new {{domxref("MediaQueryList")}} object that can then be used to determine if
  the {{domxref("document")}} matches the <a
    href="/en-US/docs/Web/CSS/Media_Queries/Using_media_queries">media query</a> string,
...
<pre
  class="brush: js"><em>mqList</em> = window.matchMedia(<em>mediaQueryString</em>)</pre>

<h3 id="Parameters">Parameters</h3>

<dl>
  <dt><code>mediaQueryString</code></dt>
  <dd>A string specifying the media query to parse into a {{domxref("MediaQueryList")}}.
  </dd>
</dl>

<h3 id="Return_value">Return value</h3>

<p>A new {{domxref("MediaQueryList")}} object for the media query. Use this object's
  properties and events to detect matches and to monitor for changes to those matches over
  time.</p>

<h2 id="Usage_notes">Usage notes</h2>

<p>You can use the returned media query to perform both instantanteous and event-driven
  checks to see if the document matches the media query.</p>

<p>To perform a one-time, instantaneous check to see if the document matches the media
  query, look at the value of the {{domxref("MediaQueryList.matches", "matches")}}
  property, which will be <code>true</code> if the document meets the media query's
  requirements.</p>

<p>If you need to be kept aware of whether or not the document matches the media query at
  all times, you can instead watch for the {{domxref("MediaQueryList.change_event",
  "change")}} event to be delivered to the object. There's <a
    href="/en-US/docs/Web/API/Window/devicePixelRatio#example">a good example of this</a>
  in the article on {{domxref("Window.devicePixelRatio")}}.</p>

<h2 id="Examples">Examples</h2>

<p>This example runs the media query <code>(max-width: 600px)</code> and  displays the
  value of the resulting <code>MediaQueryList</code>'s <code>matches</code> property in a
...
<p>See <a href="/en-US/docs/Web/CSS/Media_Queries/Testing_media_queries">Testing media
    queries programmatically</a> for additional code examples.</p>
...
<ul>
  <li><a href="/en-US/docs/Web/CSS/Media_Queries/Using_media_queries">Media queries</a>
  </li>
