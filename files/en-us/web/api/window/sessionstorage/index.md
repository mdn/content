...
The read-only **`sessionStorage`** property accesses a session {{DOMxRef("Storage")}} object for the current {{glossary("origin")}}. `sessionStorage` is similar to {{DOMxRef("Window.localStorage", "localStorage")}}; the difference is that while `localStorage` data has no expiration time and is shared across all browser tabs of the same origin, `sessionStorage` is partitioned by both origin and browser tabs (top-level browsing contexts) and is cleared when the page session ends. Both `localStorage` and `sessionStorage` are also subject to [state partitioning](/en-US/docs/Web/Privacy/Guides/State_Partitioning) in modern browsers, which further partitions storage by top-level site when accessed from third-party contexts.
