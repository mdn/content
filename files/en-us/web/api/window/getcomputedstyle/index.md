...
- The returned {{DOMxRef("CSSStyleDeclaration")}} object contains active values for CSS property _longhand_ names as well as [shorthand property names](/en-US/docs/Web/CSS/CSS_cascade/Shorthand_properties). For example, the returned object contains entries for {{cssxref("border-bottom-width")}} in addition to the {{cssxref("border-width")}} and {{cssxref("border")}}.
