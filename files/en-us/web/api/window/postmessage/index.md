...
- `origin`
  - : The {{Glossary("origin")}} of the window that sent the message at the time `postMessage` was called. This string is the concatenation of the protocol and "://", the host name if one exists, and ":" followed by a port number if a port is present and differs from the default port for the given protocol.
    ...
    Note that this origin is _not_ guaranteed to be the current or future origin of that window, which might have been navigated to a different location since `postMessage` was called.
- `source`
...
If `postMessage()` throws when used with {{jsxref("SharedArrayBuffer")}} objects, you might need to make sure you cross-site isolated your site properly.
...
To check if cross origin isolation has been successful, you can test against the {{domxref("crossOriginIsolated")}} property available to window and worker contexts:
...
After `postMessage()` is called, the {{domxref("MessageEvent")}} will be dispatched _only after all pending execution contexts have finished_.
For example, if `postMessage()` is invoked in an event handler, that event handler will run to completion, as will any remaining handlers for that same event, before the {{domxref("MessageEvent")}} is dispatched.
...
Content scripts should use {{WebExtAPIRef("runtime.sendMessage")}} to communicate with the background script.
