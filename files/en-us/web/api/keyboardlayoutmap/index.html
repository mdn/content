...
<p>The <strong><code>KeyboardLayoutMap</code></strong> interface of the <a href="/en-US/docs/Web/API/Keyboard_API">Keyboard API</a> is a map-like object with functions for retrieving the string associated with specific physical keys.</p>

<p>A list of valid keys is found in the <a href="https://www.w3.org/TR/uievents-code/#key-alphanumeric-writing-system">UI Events KeyboardEvent code Values</a> specification.</p>
...
<dl>
	<dt>{{domxref('KeyboardLayoutMap.forEach()')}} {{readonlyinline}} {{experimental_inline}}</dt>
	<dd>Executes a provided function once for each element of <code>KeyboardLayoutMap</code>.</dd>
	<dt>{{domxref('KeyboardLayoutMap.get()')}} {{experimental_inline}}</dt>
	<dd>Returns the element with the given key from the <code>KeyboardLayoutMap</code> object.</dd>
	<dt>{{domxref('KeyboardLayoutMap.has()')}} {{experimental_inline}}</dt>
	<dd>Returns a boolean indicating whether the <code>KeyboardLayoutMap</code> object has an element with the specified key.</dd>
</dl>
