...
<p>The <strong>{{domxref("TextTrackList")}}</strong> property
    ...
    the media element whose text tracks the <code>TextTrackList</code> represents.</p>
...
<pre
  class="brush: js"><em>TextTrackList</em>.onaddtrack = <em>eventHandler</em>;</pre>
...
<p>Set <code>onaddtrack</code> to a function that accepts as input a
  ...
  "track")}} property which video track has been added to the media.</p>

<h2 id="Usage_notes">Usage notes</h2>

<p>The {{event("addtrack")}} event is called whenever a new track is added to the media
  element whose video tracks are represented by the <code>TextTrackList</code> object.
  This happens when tracks are added to the element when the media is first attached to
  the element; one <code>addtrack</code> event will occur for each video track in the
  media resource.</p>

<h2 id="Example">Example</h2>

<p>This snippet establishes a handler for the {{event("addtrack")}} event that calls a
  function, <code>addToTrackList()</code>, passing in the {{domxref("VideoTrack")}} object
  representing the newly-added track. In this scenario, that function's role is to add the
  new track to a list of video tracks available to choose from.</p>

<pre class="brush: js">document.querySelector("video").textTracks.onaddtrack = function(event) {
