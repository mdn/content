...
One common task performed by the `iceconnectionstatechange` event listener: to trigger ICE restart when the state changes to `failed`. See [Lifetime of a WebRTC session, ICE restart](/en-US/docs/Web/API/WebRTC_API/Session_lifetime#ice_restart) for further information.
