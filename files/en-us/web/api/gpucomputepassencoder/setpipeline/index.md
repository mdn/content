...
In our [basic compute demo](https://webgpu-compute-render.glitch.me/), several commands are recorded via a {{domxref("GPUCommandEncoder")}}. Most of these commands originate from the {{domxref("GPUComputePassEncoder")}} created via `beginComputePass()`. The `setPipeeline()` call is used as appropriate to set the pipeline to use for this pass.
