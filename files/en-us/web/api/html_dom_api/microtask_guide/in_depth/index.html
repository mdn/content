...
<pre class="brush: js">let outputElem = document.getElementById("output");
...
<ul>
 <li>Upon starting the program, the global context is created.
  <ul>
   <li>When <code>greetUser("Mike")</code> is reached, a context is created for the <code>greetUser()</code> function; this execution context is pushed onto the execution context stack.
   ...
   <li>When <code>greetUser("Teresa")</code> is reached, a context is created for it and pushed onto the stack.
    <ul>
   ...
   <li>When <code>greetUser("Veronica")</code> is reached, a context is created for it and pushed onto the stack.
    <ul>
 ...
 <li>The main program exits and its execution context is removed from the execution stack; as there are no contexts remaining on the stack, program execution ends.</li>
</ul>
...
<dl>
 <dt>Window event loop</dt>
 <dd>The window event loop is the one that drives all of the windows sharing a similar origin (though there are further limits to this as described elsewhere in this article XXXX ????).</dd>
 <dt>Worker event loop</dt>
 <dd>A worker event loop is one which drives a worker; this includes all forms of workers, including basic <a href="/en-US/docs/Web/API/Web_Workers_API">web workers</a>, <a href="/en-US/docs/Web/API/SharedWorker">shared workers</a>, and <a href="/en-US/docs/Web/API/Service_Worker_API">service workers</a>. Workers are kept in one or more agents that are separate from the "main" code; the browser may use a single event loop for all of the workers of a given type or may use multiple event loops to handle them.</dd>
 <dt>Worklet event loop</dt>
 <dd>A <a href="/en-US/docs/Web/API/Worklet">worklet</a> event loop is the event loop used to drive agents which run the code for the worklets for a given agent. This includes worklets of type {{domxref("Worklet")}}, {{domxref("AudioWorklet")}}, and {{domxref("PaintWorklet")}}.</dd>
</dl>
...
<h4 id="Tasks_vs_microtasks">Tasks vs microtasks</h4>
