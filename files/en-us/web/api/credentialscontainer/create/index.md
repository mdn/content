...
- `NotAllowedError` {{domxref("DOMException")}}
  - : Possible causes include:
    ...
    - `mediation` is `"conditional"`, but the user agent did not recently mediate a sign-in ceremony, the user has not consented to credential creation, or the call lacks {{glossary("transient activation")}}.
...
Conditional creation lets a user agent create a passkey without additional prominent modal interaction during a sign-in ceremony the user agent has mediated, provided the user has consented to credential creation. First use {{domxref("PublicKeyCredential.getClientCapabilities_static", "PublicKeyCredential.getClientCapabilities()")}} to check whether the client supports conditional creation.
