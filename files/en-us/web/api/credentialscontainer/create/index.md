...
- `NotAllowedError` {{domxref("DOMException")}}
  - : Possible causes include:
    ...
    - `mediation` is `"conditional"`, but the user agent did not just mediate a passkey sign-in or the user has not previously agreed to passkey creation on this site.
...
Conditional creation lets a user agent create a passkey without additional prominent modal interaction during a passkey sign-in, provided the user has previously agreed to passkey creation on this site. First use {{domxref("PublicKeyCredential.getClientCapabilities_static", "PublicKeyCredential.getClientCapabilities()")}} to check whether the client supports conditional creation.
...
Client support does not guarantee that conditional creation succeeds. The user agent rejects the promise with `NotAllowedError` if it did not just mediate a passkey sign-in or the user has not previously agreed to passkey creation on this site. For the full flow and its requirements, see [Automatic passkey creation](/en-US/docs/Web/API/Web_Authentication_API#automatic_passkey_creation).
