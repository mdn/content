...
<p>The WebXR event <code><strong>select</strong></code> is sent to an {{domxref("XRSession")}} when one of the session's input sources has completed a <a href="/en-US/docs/Web/API/WebXR_Device_API/Inputs#Primary_actions">primary action</a>. Examples of common kinds of primary action are users pressing triggers or buttons, tapping a touchpad, speaking a command, or performing a recognizable gesture when using a video tracking system or handheld controller with an accelerometer.</p>
