...
When the `width` property is set, even when set to the same value as current, the drawing buffer is always reset to blank, for all context types. If you need to restore the previous content, you can save it via {{domxref("CanvasRenderingContext2D.getImageData")}} and restore it via {{domxref("CanvasRenderingContext2D.putImageData")}}.
