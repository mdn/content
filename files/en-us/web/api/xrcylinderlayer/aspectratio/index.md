...
The {{domxref("XRWebGLBinding.createCylinderLayer()")}} method creates a cylinder layer and allows you to specify an `aspectRatio`. The `XRCylinder.aspectRatio` property can be used after layer creation to get the {{glossary("aspect ratio")}} used or to set it to a new one.
