...
<pre
  class="brush: js"><em>FileSystemDirectoryEntry</em>.getFile([<em>path</em>][, <em>options</em>][, <em>successCallback</em>][, <em>errorCallback</em>]);</pre>

<h3 id="Parameters">Parameters</h3>

<dl>
  <dt><code>path</code> {{optional_inline}}</dt>
  <dd>A {{domxref("USVString")}} specifying the path, relative to the directory on which
    the method is called, describing which file's entry to return.</dd>
  ...
  <dd>A method to be called once the {{domxref("FileSystemFileEntry")}} has been created.
    The method receives a single parameter: the <code>FileSystemFileEntry</code> object
    representing the file in question.</dd>
...
<dl>
  ...
  <dd>The <code>create</code> option was not specified (or was specified as
    <code>false</code>), and the file doesn't exist.</dd>
  <dt><code>SecurityError</code></dt>
  <dd>The request to access the file was denied for security reasons.</dd>
  <dt><code>TypeMismatchError</code></dt>
  <dd>The path specified is not a file; it's probably a directory, but might be an
...
<p>The <code>options</code> parameter is an object which is based on the
  ...
  adjust the behavior of the <code>getFile()</code> method.</p>
