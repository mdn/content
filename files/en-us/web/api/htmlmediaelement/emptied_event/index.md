...
<p>The <code>emptied</code> event is fired when the media has become empty; for example, this event is sent if the media has already been loaded (or partially loaded), and the <code>load()</code> method is called to reload it.</p>
...
<p>These examples add an event listener for the HTMLMediaElement's <code>emptied</code> event, then post a message when that event handler has reacted to the event firing.</p>
...
<p>Using the <code>onemptied</code> event handler property:</p>
