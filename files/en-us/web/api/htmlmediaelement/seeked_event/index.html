...
<p>The <code>seeked</code> event is fired when a seek operation completed, the current playback position has changed, and the Boolean <code>seeking</code> attribute is changed to <code>false</code>.</p>
...
<p>These examples add an event listener for the HTMLMediaElement's <code>seeked</code> event, then post a message when that event handler has reacted to the event firing.</p>
...
<p>Using the <code>onseeked</code> event handler property:</p>
