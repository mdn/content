...
<p>The <code>pause</code> event is sent when a request to pause an activity is handled and the activity has entered its paused state, most commonly after the media has been paused through a call to the element's {{domxref("HTMLMediaElement.pause", "pause()")}} method.</p>

<p>The event is sent once the <code>pause()</code> method returns and after the media element's {{domxref("HTMLMediaElement.paused", "paused")}} property has been changed to <code>true</code>.</p>
...
<p>These examples add an event listener for the HTMLMediaElement's <code>pause</code> event, then post a message when that event handler has reacted to the event firing.</p>
...
<p>Using the <code>onpause</code> event handler property:</p>
