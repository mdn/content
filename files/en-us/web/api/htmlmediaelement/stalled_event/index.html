...
<p>The <code>stalled</code> event is fired when the user agent is trying to fetch media data, but data is unexpectedly not forthcoming.</p>
...
<p>These examples add an event listener for the HTMLMediaElement's <code>stalled</code> event, then post a message when that event handler has reacted to the event firing.</p>
...
<p>Using the <code>onstalled</code> event handler property:</p>
