...
<p>The {{domxref("HTMLMediaElement")}} method
    <code><strong>load()</strong></code> resets the media element to its initial state and
    begins the process of selecting a media source and loading the media in preparation
    for playback to begin at the beginning.</p>

<p>The amount of media data that is
  prefetched is determined by the value of the element's {{htmlattrxref("preload",
  "video")}} attribute.</p>

<p>This method is generally only useful when you've made dynamic changes to the set of
...
<pre class="brush: js"><em>mediaElement</em>.load();</pre>
...
<p>Calling <code>load()</code> aborts all ongoing operations involving this media element,
...
<p>Appropriate events will be sent to the media element itself as the load process
  proceeds:</p>

<ul>
  <li>If the element is already in the process of loading media, that load process is
    aborted and the <strong>{{event("abort")}}</strong> event is sent.</li>
  <li>If the element has already been initialized with media, the
    <strong>{{event("emptied")}}</strong> event is sent.</li>
  <li>If resetting the playback position to the beginning of the media actually changes
    the playback position (that is, it was not already at the beginning), a
    <strong>{{event("timeupdate")}}</strong> event is sent.</li>
  <li>Once media has been selected and loading is ready to begin, the
    <strong>{{event("loadstart")}}</strong> event is delivered.</li>
  <li>From this point onward, events are sent just like any media load.</li>
</ul>

<h2 id="Example">Example</h2>

<p>This example finds a {{HTMLElement("video")}} element in the document and resets it by
  calling <code>load()</code>.</p>
