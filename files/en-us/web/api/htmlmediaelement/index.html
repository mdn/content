...
<p>The <strong><code>HTMLMediaElement</code></strong> interface adds to {{domxref("HTMLElement")}} the properties and methods needed to support basic media-related capabilities that are common to audio and video.</p>

<p>The {{domxref("HTMLVideoElement")}} and {{domxref("HTMLAudioElement")}} elements both inherit this interface.</p>

<p>{{InheritanceDiagram(600, 180)}}</p>

<h2 id="Properties">Properties</h2>

<p><em>This interface also inherits properties from its ancestors {{domxref("HTMLElement")}}, {{domxref("Element")}}, {{domxref("Node")}}, and {{domxref("EventTarget")}}.</em></p>

<dl>
 <dt>{{domxref("HTMLMediaElement.audioTracks")}}</dt>
 <dd>A {{domxref("AudioTrackList")}} that lists the {{domxref("AudioTrack")}} objects contained in the element.</dd>
 <dt>{{domxref("HTMLMediaElement.autoplay")}}</dt>
 <dd>
 <p>A boolean value that reflects the {{htmlattrxref("autoplay", "video")}} HTML attribute, indicating whether playback should automatically begin as soon as enough media is available to do so without interruption.</p>

 <div class="note"><p><strong>Note:</strong> Automatically playing audio when the user doesn't expect or desire it is a poor user experience and should be avoided in most cases, though there are exceptions. See the <a href="/en-US/docs/Web/Media/Autoplay_guide">Autoplay guide for media and Web Audio APIs</a> for more information. Keep in mind that browsers may ignore autoplay requests, so you should ensure that your code isn't dependent on autoplay working.</p></div>
 </dd>
 ...
 <dd>A {{domxref("DOMString")}} indicating the <a href="/en-US/docs/Web/HTML/Attributes/crossorigin">CORS setting</a> for this media element.</dd>
 ...
 <dd>A double-precision floating-point value indicating the current playback time in seconds; if the media has not started to play and has not been seeked, this value is the media's initial playback time. Setting this value seeks the media to the new time. The time is specified relative to the media's timeline.</dd>
 <dt>{{domxref("HTMLMediaElement.defaultMuted")}}</dt>
 <dd>A {{jsxref('Boolean')}} that reflects the {{htmlattrxref("muted", "video")}} HTML attribute, which indicates whether the media element's audio output should be muted by default.</dd>
 <dt>{{domxref("HTMLMediaElement.defaultPlaybackRate")}}</dt>
 <dd>A <code>double</code> indicating the default playback rate for the media.</dd>
 <dt>{{domxref("HTMLMediaElement.disableRemotePlayback")}}</dt>
 <dd>A {{jsxref('Boolean')}} that sets or returns the remote playback state, indicating whether the media element is allowed to have a remote playback UI.</dd>
 <dt>{{domxref("HTMLMediaElement.duration")}} {{readonlyinline}}</dt>
 <dd>A read-only double-precision floating-point value indicating the total duration of the media in seconds. If no media data is available, the returned value is <code>NaN</code>. If the media is of indefinite length (such as streamed live media, a WebRTC call's media, or similar), the value is <code>+Infinity</code>.</dd>
 ...
 <dd>Returns the list of {{domxref("VideoTrack")}} objects contained in the element.</dd>
...
<dt>{{domxref("HTMLMediaElement.mediaGroup")}} {{deprecated_inline}}</dt>
 ...
 <dt>{{domxref("HTMLMediaElement.mozFragmentEnd")}} {{non-standard_inline}} {{deprecated_inline}}</dt>
 ...
 <p>The <code>mozFrameBufferLength</code> property can be set to a new value for lower latency, larger amounts of data, etc. The size given <em>must</em> be a number between 512 and 16384. Using any other size results in an exception being thrown. The best time to set a new length is after the <a href="/en-US/docs/Web/API/HTMLMediaElement/loadedmetadata_event">loadedmetadata</a> event fires, when the audio info is known, but before the audio has started or <code>MozAudioAvailable</code> events have begun firing.</p>
 </dd>
...
<dl>
 ...
 <dt>{{domxref("HTMLMediaElement.seekToNextFrame()")}} {{non-standard_inline}} {{experimental_inline}} {{deprecated_inline}}</dt>
...
<p><em>These methods are obsolete and should not be used, even if a browser still supports them.</em></p>

<dl>
 <dt>{{domxref("HTMLMediaElement.mozCaptureStream()")}} {{non-standard_inline}}</dt>
 <dd>[enter description]</dd>
 <dt>{{domxref("HTMLMediaElement.mozCaptureStreamUntilEnded()")}} {{non-standard_inline}} {{deprecated_inline}}</dt>
 <dd>[enter description]</dd>
...
<p><em>Inherits methods from its parent, {{domxref("HTMLElement")}} </em>, defined in the {{domxref('GlobalEventHandlers')}} mixin. Listen to these events using <code><a href="/en-US/docs/Web/API/EventTarget/addEventListener">addEventListener()</a></code> or by assigning an event listener to the <code>on<em>eventname</em></code> property of this interface.</p>

<dl>
 <dt>{{domxref("HTMLMediaElement.abort_event", 'abort')}}</dt>
 <dd>Fired when the resource was not fully loaded, but not as the result of an error.</dd>
 <dt>{{domxref("HTMLMediaElement.canplay_event", 'canplay')}}</dt>
 <dd>Fired when the user agent can play the media, but estimates that <strong>not</strong> enough data has been loaded to play the media up to its end without having to stop for further buffering of content</dd>
 <dt>{{domxref("HTMLMediaElement.canplaythrough_event", 'canplaythrough')}}</dt>
 <dd>Fired when the user agent can play the media, and estimates that enough data has been loaded to play the media up to its end without having to stop for further buffering of content.</dd>
 <dt>{{domxref("HTMLMediaElement.durationchange_event", 'durationchange')}}</dt>
 <dd>Fired when the duration attribute has been updated.</dd>
 <dt>{{domxref("HTMLMediaElement.emptied_event", 'emptied')}}</dt>
 <dd>Fired when the media has become empty; for example, when the media has already been loaded (or partially loaded), and the {{domxref("HTMLMediaElement.load()")}} method is called to reload it.</dd>
 <dt>{{domxref("HTMLMediaElement.ended_event", 'ended')}}</dt>
 <dd>Fired when playback stops when end of the media (&lt;audio&gt; or &lt;video&gt;) is reached or because no further data is available.</dd>
 <dt>{{domxref("HTMLMediaElement.error_event", 'error')}}</dt>
 <dd>Fired when the resource could not be loaded due to an error.</dd>
 <dt>{{domxref("HTMLMediaElement.loadeddata_event", 'loadeddata')}}</dt>
 <dd>Fired when the first frame of the media has finished loading.</dd>
 <dt>{{domxref("HTMLMediaElement.loadedmetadata_event", 'loadedmetadata')}}</dt>
 <dd>Fired when the metadata has been loaded</dd>
 <dt>{{domxref("HTMLMediaElement.loadstart_event", 'loadstart')}}</dt>
 <dd>Fired when the browser has started to load a resource.</dd>
 <dt>{{domxref("HTMLMediaElement.pause_event", 'pause')}}</dt>
 <dd>Fired when a request to pause play is handled and the activity has entered its paused state, most commonly occurring when the media's {{domxref("HTMLMediaElement.pause()")}} method is called.</dd>
 <dt>{{domxref("HTMLMediaElement.play_event", 'play')}}</dt>
 <dd>Fired when the <code>paused</code> property is changed from <code>true</code> to <code>false</code>, as a result of the {{domxref("HTMLMediaElement.play()")}} method, or the <code>autoplay</code> attribute</dd>
 <dt>{{domxref("HTMLMediaElement.playing_event", "playing")}}</dt>
 <dd>Fired when playback is ready to start after having been paused or delayed due to lack of data</dd>
 <dt>{{domxref("HTMLMediaElement.progress_event", "progress")}}</dt>
 <dd>Fired periodically as the browser loads a resource.</dd>
 <dt>{{domxref("HTMLMediaElement.ratechange_event", 'ratechange')}}</dt>
 <dd>Fired when the playback rate has changed.</dd>
 <dt>{{domxref("HTMLMediaElement.seeked_event", 'seeked ')}}</dt>
 <dd>Fired when a seek operation completes</dd>
 <dt>{{domxref("HTMLMediaElement.seeking_event", 'seeking')}}</dt>
 <dd>Fired when a seek operation begins</dd>
 <dt>{{domxref("HTMLMediaElement.stalled_event", 'stalled')}}</dt>
 <dd>Fired when the user agent is trying to fetch media data, but data is unexpectedly not forthcoming.</dd>
 <dt>{{domxref("HTMLMediaElement.suspend_event", 'suspend')}}</dt>
 <dd>Fired when the media data loading has been suspended.</dd>
 <dt>{{domxref("HTMLMediaElement.timeupdate_event", 'timeupdate')}}</dt>
 <dd>Fired when the time indicated by the {{domxref("HTMLMediaElement.currentTime", "currentTime")}} attribute has been updated.</dd>
 <dt>{{domxref("HTMLMediaElement.volumechange_event", 'volumechange')}}</dt>
 <dd>Fired when the volume has changed.</dd>
 <dt>{{domxref("HTMLMediaElement.waiting_event", 'waiting')}}</dt>
 <dd>Fired when playback has stopped because of a temporary lack of data.</dd>
</dl>
...
<ul>
 ...
 <li>Learning area: <a href="/en-US/docs/Learn/HTML/Multimedia_and_embedding/Video_and_audio_content">Video and audio content</a></li>
