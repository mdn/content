...
6. When the transition animations have reached their end states, the snapshots are destroyed and the {{domxref("ViewTransition.finished")}} promise fulfills, allowing you to respond. You can delay a view transition from getting to its finished state until a specified {{domxref("Promise")}} is resolved, using the {{domxref("ViewTransition.waitUntil()")}} method, which has some utility in advanced use cases.
