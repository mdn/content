...
2. On the current (old) view, the API captures static image **snapshots** of elements that have a non-`none` {{cssxref("view-transition-name")}} declared on them that are inside the view transition scope. By default, the scope is the entire document in the case of document-scoped view transitions, and the element `startViewTransition()` is called on and all its descendants in the case of element-scoped view transitions.
