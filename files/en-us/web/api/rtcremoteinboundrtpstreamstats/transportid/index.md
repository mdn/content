...
The `RTCRemoteInboundRtpStreamStats` and `RTCTransportStats` objects are correlated by iterating the {{domxref("RTCStatsReport")}} to find a `RTCTransportStats` object with a matching value for `transportId` in {{domxref("RTCTransportStats.id")}}.
