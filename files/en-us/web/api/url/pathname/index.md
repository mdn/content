...
The **`pathname`** property of the {{domxref("URL")}} interface is a string containing an initial `/` followed by the path of the URL, not including the query string or fragment. If there is no path, the **`pathname`** would be an empty string.
