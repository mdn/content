...
You can check to see if you already have permission by checking the value of the {{domxref("Notification.permission_static")}} read only property. It can have one of three possible values:
...
If permission to display notifications hasn't been granted yet, the application needs to use the {{domxref("Notification.requestPermission_static")}} method to request this from the user. In its simplest form, we just include the following:
...
> **Note:** Before version 37, Chrome doesn't let you call {{domxref("Notification.requestPermission_static")}} in the `load` event handler (see [issue 274284](https://crbug.com/274284)).
