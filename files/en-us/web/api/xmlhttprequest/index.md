...
- {{domxref("XMLHttpRequest.statusText")}} {{readonlyinline}}
  - : Returns a {{domxref("DOMString")}} containing the response string returned by the HTTP server. Unlike {{domxref("XMLHttpRequest.status")}}, this includes the entire text of the response message ("`200 OK`", for example).
    ...
    > **Note:** According to the HTTP/2 specification ([8.1.2.4](https://httpwg.org/specs/rfc7540.html#rfc.section.8.1.2.4) [Response Pseudo-Header Fields](https://httpwg.org/specs/rfc7540.html#HttpResponse)), HTTP/2 does not define a way to carry the version or reason phrase that is included in an HTTP/1.1 status line.
