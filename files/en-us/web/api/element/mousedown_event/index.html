...
<p>The <strong><code>mousedown</code></strong> event is fired at an {{domxref("Element")}} when a pointing device button is pressed while the pointer is inside the element.</p>

<div class="notecard note">
<p><strong>Note:</strong> This differs from the {{domxref("Element/click_event", "click")}} event in that <code>click</code> is fired after a full click action occurs; that is, the mouse button is pressed and released while the pointer remains inside the same element. <code>mousedown</code> is fired the moment the button is initially pressed.</p>
</div>
