...
<p><span class="seoSummary">The <code>emulatedPosition</code> read-only attribute of the
...
<pre
  class="brush: js">let <em>emulated</em> = <em>xrPose</em>.emulatedPosition;</pre>

<h3 id="Value">Value</h3>

<p>A Boolean which is <code>true</code> if the pose's position is computed based on
  estimates or is derived from sources other than direct sensor data. If the position is
  precisely gbased on direct sensor inputs, the value is <code>false</code>.</p>

<h2 id="Usage_notes">Usage notes</h2>

<p>There are two basic categories of XR tracking systems. A basic XR headset provides
  ...
  As such, the position is considered to be emulated, so the <code>emulatedPosition</code>
  property is <code>true</code>.</p>

<p>Contrariwise, XR devices which can also track movement forward and backward as well as
  laterally—six degree of freedom (6DoF) devices—don't require any information from other
  sources to determine the user's position, so the value of <code>emulatedPosition</code>
  is <code>false</code>.</p>

<p>The same notion applies not just to the user's head, but to any object. A hand
  controller that can directly report its position would have a value of
  <code>false</code> for this property as well. If its position is computed as an offset
  from another object (such as by basing it off the model representing the user's body),
  then this value is <code>true</code>.</p>

<p>This information is important because devices whose position is emulated are prone to
  ...
  errors which accumulate over time.</p>
