...
<p>The <strong><code>permissionState()</code></strong> method of the
  ...
  values are  <code>'prompt'</code>, <code>'denied'</code>, or <code>'granted'</code>.</p>
...
<div class="note">
  <p><strong>Note:</strong> As of Firefox 44, the permissions for <a
    ...
    granted for notifications, push will also be enabled.</p>
</div>

<h2 id="Syntax">Syntax</h2>

<pre class="brush: js">PushManager.permissionState(options).then(function(PushMessagingState) { ... });
</pre>
  ...
  <dd>An object containing optional configuration parameters. It can have the following
    ...
    <ul>
      <li><code>userVisibleOnly</code>: A boolean indicating that the returned push
        ...
        user.</li>
      <li><code>applicationServerKey</code>: A public key your push server will use to
        ...
        digital signature (ECDSA) over the P-256 curve. </li>
    </ul>
  </dd>
</dl>
...
<p>A {{jsxref("Promise")}} that resolves to a {{domxref("DOMString")}} with a value of
  <code>'prompt'</code>, <code>'denied'</code>, or <code>'granted'</code>.</p>
