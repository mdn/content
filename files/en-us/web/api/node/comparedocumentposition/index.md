...
position of its argument node relative to the node on which it is called.
...
- `otherNode`
  - : The {{domxref("Node")}} for which position should be reported, relative to `node`.
...
> `compareDocumentPosition()` is a bitmask, the
