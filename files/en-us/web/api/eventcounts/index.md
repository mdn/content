...
`EventCounts` is a read-only [`Map`-like object](/en-US/docs/Glossary/Maplike_object), in which each key is the name string for an event type, and the corresponding value is an integer indicating the number of events that have been dispatched for that event type.
