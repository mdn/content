...
<p>The <strong><code>MediaTrackSettings</code></strong> dictionary is used to return the current values configured for each of a {{domxref("MediaStreamTrack")}}'s settings. These values will adhere as closely as possible to any constraints previously described using a {{domxref("MediaTrackConstraints")}} object and set using {{domxref("MediaStreamTrack.applyConstraints", "applyConstraints()")}}, and will adhere to the default constraints for any properties whose constraints haven't been changed, or whose customized constraints couldn't be matched.</p>

<p>To learn more about how constraints and settings work, see <a href="/en-US/docs/Web/API/Media_Streams_API/Constraints">Capabilities, constraints, and settings</a>.</p>

<h2 id="Properties">Properties</h2>

<p>Some or all of the following will be included in the object, either because it's not supported by the browser or because it's not available due to context. For example, because {{Glossary("RTP")}} doesn't provide some of these values during negotiation of a WebRTC connection, a track associated with a {{domxref("RTCPeerConnection")}} will not include certain values, such as {{domxref("MediaTrackSettings.facingMode", "facingMode")}} or {{domxref("MediaTrackSettings.groupId", "groupId")}}.</p>
...
<dl>
 ...
 <dd>A Boolean which indicates the current value of the {{domxref("MediaTrackConstraints.noiseSuppression", "noiseSuppression")}} property, which is <code>true</code> if noise suppression is enabled and is <code>false</code> otherwise.</dd>
...
<dl>
 ...
 <dt>{{domxref("MediaTrackSettings.resizeMode", "resizeMode")}}</dt>
 <dd>A {{domxref("DOMString")}} indicating the current value of the <code>{{domxref("MediaTrackConstraints.resizeMode", "resizeMode")}}</code> property, specifying the mode used by the user agent to derive the resolution of the track. The value will be one of:
 <dl>
  <dt><code>"none"</code></dt>
  <dd>The track has the resolution offered by the camera, its driver or the OS.</dd>
  <dt><code>"crop-and-scale"</code></dt>
  <dd>The track's resolution might be the result of the user agent using cropping or downscaling from a higher camera resolution.</dd>
 </dl>
 </dd>
</dl>
...
<p>Tracks containing video shared from a user's screen (regardless of whether the screen data comes from the entire screen or a portion of a screen, like a window or tab) are generally treated like video tracks, with the exception that they also support the following added settings:</p>

<dl>
 <dt>{{domxref("MediaTrackSettings.cursor", "cursor")}}</dt>
 <dd>
 <p>A {{domxref("DOMString")}} which indicates whether or not the mouse cursor is being included in the generated stream and under what conditions. Possible values are:</p>
 ...
 <dt>{{domxref("MediaTrackSettings.displaySurface", "displaySurface")}}</dt>
 <dd>
 <p>A {{domxref("DOMString")}} which specifies the type of source the track contains; one of:</p>
 ...
 <dt>{{domxref("MediaTrackSettings.logicalSurface", "logicalSurface")}}</dt>
 <dd>A Boolean value which, if <code>true</code>, indicates that the video contained in the stream's video track contains a background rendering context, rather than a user-visible one. This is <code>false</code> if the video being captured is coming from a foreground (user-visible) source.</dd>
</dl>
