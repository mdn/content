...
<p>The {{domxref("MediaTrackSettings")}} dictionary's
  <code><strong>deviceId</strong></code> property is a {{domxref("DOMString")}} which
...
<pre class="brush: js">var <em>deviceId</em> = <em>MediaTrackSettings</em>.deviceId;
</pre>

<h3 id="Value">Value</h3>

<p>A {{domxref("DOMString")}} whose value is an origin-unique identifier for the track's
  ...
  the same source be used for multiple sessions, for example.</p>
...
<p>Since there is a one-to-one pairing of ID with each source, all tracks with the same
  ...
  for <code>deviceId</code>. That makes the device ID not useful for any changes to
  constraints when calling {{domxref("MediaStreamTrack.applyConstraints()")}}.</p>
