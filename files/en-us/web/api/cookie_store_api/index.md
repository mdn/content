...
- {{domxref("ExtendableCookieChangeEvent")}}
  - : An `ExtendableCookieChangeEvent` named `change` is dispatched against {{domxref("ServiceWorkerGlobalScope")}} events when any script-visible cookie changes have occurred which match the service worker's cookie change subscription list.
