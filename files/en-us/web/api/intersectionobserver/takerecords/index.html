...
<p>The {{domxref("IntersectionObserver")}} method
    <code><strong>takeRecords()</strong></code> returns an array of
    ...
    call to the observer's callback. </p>

<div class="note">
  <p><strong>Note:</strong> If you use the callback to monitor these changes, you don't
    need to call this method. Calling this method clears the pending intersection list, so
    the callback will not be run.</p>
</div>

<h2 id="Syntax">Syntax</h2>

<pre
  class="brush: js"><em>intersectionObserverEntries</em> = <em>intersectionObserver</em>.takeRecords();</pre>
...
<p>An array of {{domxref("IntersectionObserverEntry")}} objects, one for each target
  element whose intersection with the root has changed since the last time the
  intersections were checked.</p>
