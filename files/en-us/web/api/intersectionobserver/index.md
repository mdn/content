...
<p>The <code><strong>IntersectionObserver</strong></code> interface of the <a href="/en-US/docs/Web/API/Intersection_Observer_API">Intersection Observer API</a> provides a way to asynchronously observe changes in the intersection of a target element with an ancestor element or with a top-level document's {{Glossary('viewport')}}. The ancestor element or viewport is referred to as the root.</p>
...
<dl>
 ...
 <dd>Creates a new <code>IntersectionObserver</code> object which will execute a specified callback function when it detects that a target element's visibility has crossed one or more thresholds.</dd>
</dl>
...
<dl>
 ...
 <dd>The {{domxref("Element")}} or {{domxref("Document")}} whose bounds are used as the bounding box when testing for intersection. If no <code>root</code> value was passed to the constructor or its value is <code>null</code>, the top-level document's viewport is used.</dd>
