...
<p>The <code>animationiteration</code> event is sent when a <a
    href="/en-US/docs/Web/CSS/CSS_Animations">CSS animation</a> reaches the end of an
  iteration. An iteration ends when a single pass through the sequence of animation
  instructions is completed by executing the last animation step.</p>

<h2 id="Syntax">Syntax</h2>

<pre class="brush: js">var <em>animIterationHandler</em> = <em>target</em>.onanimationiteration;
...
<p>A {{jsxref("Function")}} to be called when an {{event("animationiteration")}} event
  occurs indicating that a CSS animation has reached the end of an iteration while running
  on the <em><code>target</code></em>, where the target object is an HTML element
  ...
  {{domxref("AnimationEvent")}} object describing the event which occurred.</p>

<h2 id="Example">Example</h2>

<p>Let's create an animation which automatically pauses at the end of each iteration,
  allowing the user to choose whether or not to start the next iteration. Much of this
  code is the same as in other examples of animation events, so it may look familiar.</p>
...
<p>The animation's keyframes are defined next; they describe an animation which causes the
  box to migrate from the top-left corner of the container to the bottom-right corner.</p>
...
<p>Some JavaScript code will need to be written to handle the click on the button to start
  the next iteration. Let's have a look.</p>
...
<p>This sets up two global variables: <code>box</code>, which references the
  <code>"box"</code> element that we're animating, and <code>iterationCounter</code>,
...
<p>This sets the box element's {{cssxref("animation-play-state")}} to "running" and
  increments the iteration counter. That's all there is to it!</p>
...
<p>Each time the box reaches the opposing corner, it stops, with the button reflecting
