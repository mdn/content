...
<dl>
 <dt>{{domxref('FileSystemDirectoryHandle.entries()')}}</dt>
 <dd>Returns an {{jsxref('Array')}} of a given object's own enumerable property <code>[key, value]</code> pairs</dd>
 <dt>{{domxref('FileSystemDirectoryHandle.getFileHandle()')}}</dt>
 <dd>Returns a {{domxref('FileSystemFileHandle')}} for a file with the specified name, within the directory the method is called.</dd>
