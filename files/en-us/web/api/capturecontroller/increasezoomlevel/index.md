...
The `increaseZoomLevel()` method must be invoked via [transient activation](/en-US/docs/Glossary/Transient_activation), in which case the user is asked for permission to zoom the captured page. If the relevant permission is already `"granted"`, transient activation and permission gathering is not needed.
