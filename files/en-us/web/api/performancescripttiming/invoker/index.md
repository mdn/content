...
| `"event-listener"`                        | The {{domxref("Element.tagName", "tagName")}} of the element, followed by a hash and its `id` (`#id`) or `src=` followed by its `src` value in square brackets (`[src=url]`) if an `id` is not present, followed by a dot, followed by the event handler property. | `"IMG#hero.onload"`, `"IMG[src=https://example.com/img.jpg].onload"`, `"BUTTON#updateCart.onclick"` |
