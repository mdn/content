...
- `"resolve-promise"`
  - : A handler function for the resolved state of a web platform promise, such as {{domxref("fetch()")}}. Note that in the case of promises, all the handlers of the same promises are grouped together as a single `"script"` entry type.
- `"reject-promise"`
