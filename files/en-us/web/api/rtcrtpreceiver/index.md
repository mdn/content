...
- {{domxref("RTCRtpReceiver.getSynchronizationSources()")}}
  - : Returns an array including one {{domxref("RTCRtpReceiver.getSynchronizationSources")}} instance for each unique SSRC (synchronization source) identifier received by the current `RTCRtpReceiver` in the last ten seconds.
