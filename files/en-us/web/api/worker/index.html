...
<p>The <strong><code>Worker</code></strong> interface of the <a href="/en-US/docs/Web/API/Web_Workers_API">Web Workers API</a> represents a background task that can be created via script, which can send messages back to its creator.</p>

<p>Creating a worker is done by calling the <code>Worker("path/to/worker/script")</code> constructor.</p>
