...
- `options` {{optional_inline}}
  ...
  - : An object providing configuration options for the new `XRWebGLLayer`. The available options
    ...
    - `framebufferScaleFactor`
      - : A floating-point value which is used to scale the image during compositing, with a value of 1.0 represents the default pixel size for the frame buffer. The static {{domxref("XRWebGLLayer")}} function {{domxref("XRWebGLLayer.getNativeFramebufferScaleFactor_static")}} returns the scale that would result in a 1:1 pixel ratio, thereby ensuring that the rendering is occurring at the device's native resolution. The default is 1.0.
    - `ignoreDepthValues`
