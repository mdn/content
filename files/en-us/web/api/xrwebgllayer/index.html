...
<p>The <strong><code>XRWebGLLayer</code></strong> interface of the WebXR Device API provides a linkage between the WebXR device (or simulated XR device, in the case of an inline session) and a WebGL context used to render the scene for display on the device. In particular, it provides access to the WebGL framebuffer and viewport to ease access to the context.</p>

<p>Although <code>XRWebGLLayer</code> is currently the only type of framebuffer layer supported by <a href="/en-US/docs/Web/API/WebGL_API">WebGL</a>, it's entirely possible that future updates to the WebXR specification may allow for other layer types and corresponding image sources.</p>
