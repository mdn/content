...
- A document loaded into a sandboxed {{HTMLElement("iframe")}} element cannot call
  ...
  [`sandbox`](/en-US/docs/Web/HTML/Reference/Element/iframe#sandbox) attribute set to `allow-same-origin`.
...
- Any other situation in which there is no origin, such as when the
  [`srcdoc`](/en-US/docs/Web/HTML/Reference/Element/iframe#srcdoc) attribute is used to specify the contents of a
  frame.
