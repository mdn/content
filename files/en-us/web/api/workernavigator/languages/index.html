...
<p>The value of {{domxref("WorkerNavigator.language","navigator.language")}} is the
  first element of the returned array.</p>

<p>When its value changes, as the user's preferred languages are changed a
  {{event("languagechange")}} event is fired on the {{domxref("WorkerGlobalScope")}} object.</p>
...
<ul>
  <li>{{domxref("NavigatorWorker.language")}}</li>
  <li>{{domxref("NavigatorWorker")}}</li>
