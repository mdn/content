...
<dl>
	...
	<dd>Returns a {{domxref("DOMString")}} which indicates the track's in-band metadata track dispatch type.</dd>
	<dt>{{domxref("TextTrack.kind")}}{{readonlyInline}}</dt>
	<dd>Returns a {{domxref("DOMString")}} indicating what kind of text track the <code>TextTrack</code> describes. It must be one of the permitted values.</dd>
	...
	<dd>A {{domxref("DOMString")}} which specifies the text language in which the text track's contents is written. The value must adhere to the format specified in the <a href="https://datatracker.ietf.org/doc/html/bcp47">Tags for Identifying Languages</a> (<a href="https://datatracker.ietf.org/doc/html/bcp47">BCP 47</a>) document from the IETF, just like the HTML {{htmlattrxref("lang")}} attribute. For example, this can be <code>"en-US"</code> for United States English or <code>"pt-BR"</code> for Brazilian Portuguese.</dd>
	<dt>{{domxref("TextTrack.mode")}}</dt>
	<dd>A {{domxref("DOMString")}} specifying the track's current mode, which must be one of the permitted values. Changing this property's value changes the track's current mode to match. The default is <code>disabled</code>, unless the {{HTMLElement("track")}} element's {{htmlattrxref("default", "track")}} Boolean attribute is specified, in which case the default mode is <code>started</code>.</dd>
</dl>
...
<p><strong>Note:</strong> The {{domxref("TextTrackCue")}} interface is an abstract class used as the parent for other cue interfaces such as {{domxref("VTTCue")}}. Therefore, when adding or removing a cue you will be passing in one of the cue types that inherit from <code>TextTrackCue</code>.</p>
</div>
...
<p>The following example adds a new <code>TextTrack</code> to a video, then sets it to display using {{domxref("TextTrack.mode")}}.</p>
