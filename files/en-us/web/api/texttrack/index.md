...
- {{domxref("TextTrack.language")}} {{ReadOnlyInline}}
  - : A string specifying the language in which the text track's contents is written. The value must be a valid {{glossary("BCP 47 language tag")}}, for example `"en-US"` for United States English or `"pt-BR"` for Brazilian Portuguese.
