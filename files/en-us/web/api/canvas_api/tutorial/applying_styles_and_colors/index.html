...
<p>Up until now we have only seen methods of the drawing context. If we want to apply colors to a shape, there are two important properties we can use: <code>fillStyle</code> and <code>strokeStyle</code>.</p>
...
<p><code>color</code> is a string representing a CSS {{cssxref("&lt;color&gt;")}}, a gradient object, or a pattern object. We'll look at gradient and pattern objects later. By default, the stroke and fill color are set to black (CSS color value <code>#000000</code>).</p>

<div class="note">
<p><strong>Note:</strong> When you set the <code>strokeStyle</code> and/or <code>fillStyle</code> property, the new value becomes the default for all shapes being drawn from then on. For every shape you want in a different color, you will need to reassign the <code>fillStyle</code> or <code>strokeStyle</code> property.</p>
</div>

<p>The valid strings you can enter should, according to the specification, be CSS {{cssxref("&lt;color&gt;")}} values. Each of the following examples describe the same color.</p>
...
<h3 id="A_fillStyle_example">A <code>fillStyle</code> example</h3>
...
<p>In addition to drawing opaque shapes to the canvas, we can also draw semi-transparent (or translucent) shapes. This is done by either setting the <code>globalAlpha</code> property or by assigning a semi-transparent color to the stroke and/or fill style.</p>
...
<p>Because the <code>strokeStyle</code> and <code>fillStyle</code> properties accept CSS rgba color values, we can use the following notation to assign a transparent color to them.</p>
...
<h3 id="A_globalAlpha_example">A <code>globalAlpha</code> example</h3>
...
<h3 id="A_lineWidth_example">A <code>lineWidth</code> example</h3>
...
<p>In this example we are creating a marching ants effect. It is an animation technique often found in selection tools of computer graphics programs. It helps the user to distinguish the selection border from the image background by animating the border. In a later part of this tutorial, you can learn how to do this and other <a href="/en-US/docs/Web/API/Canvas_API/Tutorial/Basic_animations">basic animations</a>.</p>
...
<p>Just like any normal drawing program, we can fill and stroke shapes using linear, radial and conic gradients. We create a {{domxref("CanvasGradient")}} object by using one of the following methods. We can then assign this object to the <code>fillStyle</code> or <code>strokeStyle</code> properties.</p>
...
<dl>
 ...
 <dt>{{domxref("CanvasRenderingContext2D.createConicGradient", "createConicGradient(angle, x, y)")}}</dt>
 <dd>Creates a conic gradient object with a starting angle of <code>angle</code> in radians, at the position (<code>x</code>, <code>y</code>).</dd>
</dl>
...
<h3 id="A_createLinearGradient_example">A <code>createLinearGradient</code> example</h3>
...
<h3 id="A_createConicGradient_example">A <code>createConicGradient</code> example</h3>

<p>In this example, we'll define two different conic gradients. A conic gradient differs from a radial gradient as, instead of creating circles, it circles around a point.</p>
...
<pre class="brush: html hidden">&lt;canvas id="canvas" width="250" height="150"&gt;&lt;/canvas&gt;</pre>

<pre class="brush: js hidden">draw();</pre>

<p>The first gradient is positioned in the center of the first rectangle and moves a green color stop at the start, to a white one at the end. The angle starts at 2 radians, which is noticeable because of the beginning/end line pointing south east.</p>

<p>The second gradient is also positioned at the center of it's second rectangle. This one has multiple color stops, alternating from black to white at each quarter of the rotation. This gives us the checkered effect.</p>

<p>{{EmbedLiveSample("A_createConicGradient_example", "180", "180", "canvas_conicgrad.png")}}</p>
...
<h3 id="A_createPattern_example">A <code>createPattern</code> example</h3>
...
<div class="note">
<p><strong>Note:</strong> Shadows are only drawn for <code>source-over</code> <a href="/en-US/docs/Web/API/Canvas_API/Tutorial/Compositing">compositing operations</a>.</p>
</div>
...
<ul>
 <li><code><strong>"nonzero</strong></code>": The <a class="external external-icon" href="https://en.wikipedia.org/wiki/Nonzero-rule">non-zero winding rule</a>, which is the default rule.</li>
 <li><code><strong>"evenodd"</strong></code>: The <a class="external external-icon" href="https://en.wikipedia.org/wiki/Even%E2%80%93odd_rule">even-odd winding rule</a>.</li>
</ul>
