...
![Canvas grid: center is at top left corner. Each tiny square corresponds to 1 pixel on the canvas. A diagram of a square is at 'x' units from Y-axis and 'y' units from X-axis and it is filled in royal blue color. The height of the diagram is taken along Y-axis and the width of the diagram is taken along X-axis.](canvas_default_grid.png)
...
![Quadratic curve and Bezier curves: Quadractic curve has a start and an end point and one control point while a Bezier curve uses 2 control points. The start and end points are indicated by the blue dots while the control points are indicated by red dots.](canvas_curves.png)
