...
We track the dragged element and the source column this way: on `dragstart`, we mark the dragged task with an `id`. Then on `drop`, we can use this ID to identify the task, and then find its source column by walking up the DOM tree. Finally we remember to remove the ID on `dragend` so we don't create duplicate IDs on a later drag.
