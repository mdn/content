...
- Named properties
  - : Some elements in the document are also exposed as properties:
    - For each {{HTMLElement("embed")}}, {{HTMLElement("form")}}, {{HTMLElement("iframe")}}, {{HTMLElement("img")}}, and {{HTMLElement("object")}} element, its `name` (if non-empty) is exposed. For example, if the document contains `<form name="my-form">`, then `document["my-form"]` returns a reference to that element.
