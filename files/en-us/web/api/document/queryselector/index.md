...
[CSS selector](/en-US/docs/Web/CSS/Guides/Selectors), or group of CSS selectors.
If no matches are found, `null` is returned.

The matching is done using depth-first pre-order traversal of the document's nodes
starting with the first element in the document's markup and iterating through sequential
...
See {{domxref("Element.querySelector()")}} for details on element-scoped searches.
...
Note that the HTML specification does not require attribute values to be valid CSS
identifiers. If a class

or id
 attribute value is
not a valid CSS identifier, then you must escape it before using it in a selector,
either by calling {{domxref("CSS.escape_static", "CSS.escape()")}} on the value, or
using one of the techniques described in
Escaping characters
.
See Escaping attribute values
 for an example.
...
Selectors can also be powerful, as demonstrated in the following example.
Here, the first {{HTMLElement("input")}} element with the name "login"
(<input name="login" />) located inside a {{HTMLElement("div")}}
...
This example shows that if an HTML document contains an
id
 which is not a valid
CSS identifier
, then we must escape
the attribute value before using it in querySelector().
