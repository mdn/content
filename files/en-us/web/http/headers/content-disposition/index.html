...
<dl>
 ...
 <dd>
    <p>
      ...
      (for example, the {{htmlattrxref("multiple", "input")}} attribute of an <code>{{HTMLElement("input","&lt;input type=\"file\"&gt;")}}</code> element),
      there can be several subparts with the same name.
    </p>
    <p>
      A <code>name</code> with a value of <code>'_charset_'</code> indicates
  ...
  <dt><code>filename*</code></dt>
  <dd>The parameters <code>filename</code> and <code>filename*</code> differ only in that <code>filename*</code> uses the encoding defined in <a href="https://datatracker.ietf.org/doc/html/rfc5987">RFC 5987</a>. When both <code>filename</code> and <code>filename*</code> are present in a single header field value, <code>filename*</code> is preferred over <code>filename</code> when both are understood.</dd>
</dl>
