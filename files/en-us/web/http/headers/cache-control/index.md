...
<pre>Cache-Control: max-age=&lt;seconds&gt;
...
<pre>Cache-Control: must-revalidate
Cache-Control: no-cache
Cache-Control: no-store
Cache-Control: no-transform
...
<pre class="example-good">Cache-Control: no-store</pre>

<div class="notecard note">
  <p><strong>Note:</strong> The <code>no-store</code> directive will prevent a new resource being cached,
    ...
    Setting <code>max-age=0</code> as well forces the cache to revalidate (clears the cache).</p>

  <pre>Cache-Control: no-store, max-age=0</pre>
</div>

<p>On the opposite, this is a bad way to achieve this:</p>
<pre class="example-bad">Cache-Control: private,no-cache,no-store,max-age=0,must-revalidate,pre-check=0,post-check=0</pre>
...
<pre>Cache-Control: max-age=0, must-revalidate</pre>
