---
title: Using Permissions Policy
slug: Web/HTTP/Permissions_Policy/Using_Permissions_Policy
tags:
  - Permissions Policy
  - Permissions-Policy
  - HTTP
...
Permissions Policy provides two ways to specify policies to control features:
...
You can send the `Permissions-Policy` HTTP header with the response of a page. The value of this header is a policy to be enforced by the browser for the given page. It has the following structure.

```bash
Permissions-Policy: <feature name> <allowlist of origin(s)>
```
...
```bash
Permissions-Policy: unsized-media 'none'; geolocation 'self' https://example.com; camera *;

Permissions-Policy: unsized-media 'none'
...
The second way to use Permissions Policy is for controlling content within an iframe. Use the `allow` attribute to specify a policy list for embedded content.
