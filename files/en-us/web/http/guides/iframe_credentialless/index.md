...
To opt in to cross-origin isolation, a resource must be served with a {{httpheader("Cross-Origin-Opener-Policy")}} with a value of `same-origin` (protects your origin from attackers) and {{httpheader("Cross-Origin-Embedder-Policy")}} with a value of `credentialless` or `require-corp` (protects victims from your origin). The latter prevents a document from loading any credentialed cross-origin resources that don't explicitly grant the document permission using {{httpheader("Cross-Origin-Resource-Policy")}} or [Cross-Origin Resource Sharing](/en-US/docs/Web/HTTP/Guides/CORS).
