...
As with other resources, when using [separate (standalone) dictionaries](#separate_dictionary) via [`<link rel="compression-dictionary">`](/en-US/docs/Web/HTML/Reference/Attributes/rel/compression-dictionary) tags, if the website has a {{HTTPHeader("Content-Security-Policy")}} header, the `connect-src` directive (or `default-src`, if `connect-src` is not set) must permit the location of the dictionary to avoid the request being blocked.
