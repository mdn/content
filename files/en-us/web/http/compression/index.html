...
<p class="summary"><strong>Compression</strong> is an important way to increase the performance of a Web site. For some documents, size reduction of up to 70% lowers the bandwidth capacity needs. Over the years, algorithms also got more efficient, and new ones are supported by clients and servers.</p>
...
<p>Each data type has some redundancy, that is <em>wasted space</em>, in it. If text can typically have as much as 60% redundancy, this rate can be much higher for some other media like audio and video. Unlike text, these other media types use lot of space to store their data and the need to optimize storage and regain space was apparent very early. Engineers designed the optimized compression algorithm used by file formats designed for this specific purpose. Compression algorithms used for files can be grouped into two broad categories:</p>

<ul>
 <li><em>Loss-less compression</em>, where the compression-uncompression cycle doesn't alter the data that is recovered. It matches (byte to byte) with the original.<br>
  For images, <code>gif</code> or <code>png</code> are using loss-less compression.</li>
 <li><em>Lossy compression</em>, where the cycle alters the original data in a (hopefully) imperceptible way for the user.<br>
