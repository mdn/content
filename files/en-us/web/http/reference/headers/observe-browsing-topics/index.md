...
The HTTP **`Observe-Browsing-Topics`** {{Glossary("response header")}} is used to mark topics of interest inferred from a calling site's URL (i.e., the site where the ad tech {{HTMLElement("iframe")}} is embedded) as observed in the response to a request generated by a feature that enables the Topics API.
