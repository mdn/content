...
> [!NOTE]
> The **`frame-ancestors`** directive [checks each ancestor](https://w3c.github.io/webappsec-csp/#frame-ancestors-and-frame-options). If any ancestor doesn't match, the load is canceled. Therefore all ancestors should be allowed by the **`frame-ancestors`** directive of leaf frames when using nested frames.
