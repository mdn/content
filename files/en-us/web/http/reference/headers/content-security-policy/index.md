...
- {{CSP("sandbox")}}
  - : Enables a sandbox for the requested resource similar to the
    {{HTMLElement("iframe")}} [`sandbox`](/en-US/docs/Web/HTML/Reference/Element/iframe#sandbox) attribute.
...
If the element loads an external resource (for example, using the [`src`](/en-US/docs/Web/HTML/Reference/Element/script#src) attribute), then the element must also have the [`integrity`](/en-US/docs/Web/HTML/Reference/Element/script#integrity) attribute set.
...
By default, if a CSP contains a `default-src` or a `script-src` directive, then inline JavaScript is not allowed to execute. The `'inline-speculation-rules'` allows the browser to load inline `<script>` elements that have a [`type`](/en-US/docs/Web/HTML/Reference/Element/script/type) attribute of [`speculationrules`](/en-US/docs/Web/HTML/Reference/Element/script/type/speculationrules).
