...
The HTTP {{HTTPHeader("Permissions-Policy")}} header `deferred-fetch-minimal` directive controls whether or not the document is permitted to use [Screen Capture API](/en-US/docs/Web/API/Screen_Capture_API), that is, {{domxref("MediaDevices.getDisplayMedia", "getDisplayMedia()")}} to capture the screen's contents.
