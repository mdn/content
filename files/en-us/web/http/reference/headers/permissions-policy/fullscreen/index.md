...
SecureCorp Inc. wants to disallow `fullscreen` within all browsing contexts except for its own origin and those whose origin is `https://example.com`. It can do so by delivering the following HTTP response header to define a Permissions Policy:
...
> [!NOTE]
> Specifying the `Permissions-Policy` header in this manner disallows `fullscreen` for other origins, even if they would be allowed by the `<iframe>` `allow` attribute.

### Using the default policy

FastCorp Inc. wants to allow `fullscreen` in a specific cross-origin child frame. Since the default allowlist for `fullscreen` is `self`, it can do so by including an {{HTMLElement('iframe','allow','#Attributes')}} attribute on the `<iframe>` element:
