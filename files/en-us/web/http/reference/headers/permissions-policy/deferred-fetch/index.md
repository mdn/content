...
The `deferred-fetch` and {{HTTPHeader("Permissions-Policy/deferred-fetch-minimal", "deferred-fetch-minimal")}} {{HTTPHeader("Permissions-Policy")}} header determine how the overall 640KB limit for the [`fetchLater()` API](/en-US/docs/Web/API/fetchLater_API) is distributed between the top level origin and its cross-origin subframes.
