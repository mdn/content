...
- `102 Processing` {{deprecated_inline}}
  - : This code was used in {{Glossary("WebDAV")}} contexts to indicate that a request has been received by the server, but no status was available at the time of the response. The response code has been deprecated and it is no longer used.
