...
Common causes of this error can be from server directories running out of available space, not enough available RAM for an operation, or internal limits reached (such as `memory_limit`).
The request causing this error does not necessarily need to include a payload, as it may be a request that would create an entity on the server if it was successful.

This issue is considered temporary, as opposed to a {{HTTPStatus(413)}}, which indicates that the client request is too large for the server to process regardless of server resource constraints.
