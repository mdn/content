...
The HTTP **`404 Not Found`** status response code is part of the `400`-`499` class of [client error responses](/en-US/docs/Web/HTTP/Status#client_error_responses) and indicates that the server cannot find the requested resource.
...
A 404 status code only indicates that the resource is missing and won't indicate if this is temporary or permanent.
...
The example below uses `notfound.html` as a page to show visitors on 404s:
