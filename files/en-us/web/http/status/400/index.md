...
The HTTP **`400 Bad Request`** status response code is part of the `400`-`499` class of [client error responses](/en-US/docs/Web/HTTP/Status#client_error_responses) and indicates that the server will not process the request due to something the server considers to be a client error.
This is typically due to malformed request syntax, invalid request message framing, or deceptive request routing.
