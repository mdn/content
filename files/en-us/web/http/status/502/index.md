...
The HTTP **`502 Bad Gateway`** [server error response](/en-US/docs/Web/HTTP/Status#server_error_responses) status code indicates that a server was acting as a gateway or proxy and that it received an invalid response from the upstream server.

This response is similar to a {{HTTPStatus(500)}} response in the sense that it is a generic "catch-call" for server errors, except that it is specific to the point in the request chain the error is occurring.
...
If the proxy or gateway did not receive any HTTP response from the origin, it instead sends a {{HTTPStatus(504)}} to the client.

There are many causes of 502 errors, and fixing such problems likely requires investigation and debugging by server administrators.
