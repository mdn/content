...
Some structural roles, like [`suggestion`](/en-US/docs/Web/Accessibility/ARIA/Roles/suggestion_role), don't have HTML equivalents, and therefor have separate documentation. Some structural roles with HTML equivalents, like [`heading`](/en-US/docs/Web/Accessibility/ARIA/Roles/heading_role)), have ARIA attribute requirements. They are listed below with links to their individual role documentation.
...
| **`caption`** <br>Visible content that names, and may also describe, a `figure`, `table`, `grid`, or `treegrid`.<br/> Only found in those 4 roles captions `id` is generally referenced by the `figure`, `grid`, `table`, or `treegrid`'s `aria-labeledby` attribute <br/> Prohibited attributes: `aria-label` and `aria-labelledby`   | {{HTMLElement('caption')}} <br/> {{HTMLElement('figcaption')}} | 
