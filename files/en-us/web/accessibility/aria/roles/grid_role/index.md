...
document.querySelector("table").addEventListener("keydown", (event) => {
  const currentCol = parseInt(event.target.dataset.col, 10);
  const currentRow = parseInt(event.target.dataset.row, 10);

  switch (event.key) {
    case "ArrowRight": {
      const nextCol = currentCol === 6 ? 0 : currentCol + 1;
      const nextRow = currentCol === 6 ? currentRow + 1 : currentRow;
      moveto(nextRow, nextCol);
      break;
    }
    case "ArrowLeft": {
      const nextCol = currentCol === 0 ? 6 : currentCol - 1;
      const nextRow = currentCol === 0 ? currentRow - 1 : currentRow;
      moveto(nextRow, nextCol);
      break;
    }
    case "ArrowDown":
