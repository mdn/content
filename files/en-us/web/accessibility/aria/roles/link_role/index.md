...
When this role is added to an element, tab can be used to change focus to the link, and enter used to execute the link. When not using semantic HTML for its intended purpose, interactive features must be re-implemented. Use the {{HTMLattrRef('tabindex')}} attribute with a value of `0` to may optionally be used with this role to directly specify the position of the element in the tab order.
