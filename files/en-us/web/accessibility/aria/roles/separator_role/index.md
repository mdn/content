...
A seperator is a divider that separates and distinguishes sections of content or groups of menuitems. There are two types of separators: a static structure that provides a visible boundary, identical to the HTML {{HTMLElement('hr')}} element, and a focusable, moveable widget. 

Elements with the role `separator` have an implicit `aria-orientation` value of `horizontal`.

### Non-focusable seperator
...
The separator role can be used to identifies the element as a visual separator between groups of items within a menu, such as groups of `menuitemradio` or `menuitemcheckbox` elements.
...
- `aria-orientation` (default is horizontal for seperator)
...
- `aria-valuenow` 
  - : If the seperator is focusable and has a known value, the `aria-valuenow` define the current vaue. If not focusable or the value is unknown, do not include this attribute.

- `aria-valuemin` (default is 0)
  - : If the seperator is focusable, and the minimum value is not 0, include the minimum value with `aria-valuemin`. If the 

- `aria-valuemax` (default is 100)
  - : If the seperator is focusable, and the maximum value is not 100, include `aria-valuemax` with a value equal to or larger than `aria-valuemin`.

- `aria-valuetext`
  - : If the seperator is focusable, and the `aria-valuenow` is not optimal in providing the user with usable information, included  `aria-valuetext` content will be read instead of the `aria-valuenow` value.
