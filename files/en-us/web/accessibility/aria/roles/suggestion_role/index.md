...
Browsers tend to provide a default black strikethrough for deletions, and a black underline for insertions if one is using the HTML elements that implicitly expose these roles. But if one is using explicit ARIA role to modify HTML elements, such as divs, you'll need to use CSS to customize the visual styling for such deletions and insertions.
