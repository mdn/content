...
- article
  - : Each section of content in a feed should be contained in an {{HTMLElement('article')}} or element with role [`article`](/en-US/docs/Web/Accessibility/ARIA/Roles/Article_Role). Each `article` should have an `aria-labelledby` referring to the article title or other child that can serve as a distinguishing label. Each article should preferably have [`aria-describedby`](/en-US/docs/Web/Accessibility/ARIA/Attributes/aria-describedby) referring to one or more elements inside the article that serve as the primary content of the article. Each `article` element has `aria-posinset` set to a value that represents its position in the feed and an [`aria-setsize`](/en-US/docs/Web/Accessibility/ARIA/Attributes/aria-setsize) set to a value that represents either the total number of articles that have been loaded or the total number in the feed, depending on which value is more helpful to users. If the total number in the feed is not known, set `aria-setsize="-1"`.
