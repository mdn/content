...
A row can contain a number of attributes clarifying the row's role, including [`aria-colindex`](/en-US/docs/Web/Accessibility/ARIA/Attributes/aria-colindex), `aria-level`, [`aria-rowindex`](/en-US/docs/Web/Accessibility/ARIA/Attributes/aria-rowindex), and `aria-selected`.

If the row is within a treegrid, rows can include the `aria-expanded` attribute, using the attribute to indicate the present status. This is not the case for an ordinary table or grid, in which the [`aria-expanded`](/en-US/docs/Web/Accessibility/ARIA/Attributes/aria-expanded) attribute is not present.
