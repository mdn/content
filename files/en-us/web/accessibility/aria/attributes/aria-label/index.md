...
> **Note:** `aria-label` is intended for interactive elements or for those made interactive via other ARIA declarations when there's no visible text in the DOM to serve as a label.
...
- If you assign a `title` to an {{HTMLElement('iframe')}}, define an `alt` attribute for an {{HTMLElement('img')}}, and add {{HTMLElement('label')}} for an {{HTMLElement('input')}}, `aria-label` is not necessary. However, if `aria-label` is used, it will take precedence over the iframe's `title`, the image's `alt`, and the input's `<label>` text as the accessible name for these elements.
