...
Mandatory {{htmlelement("input")}}s, {{htmlelement("selects")}}s, and {{htmlelement("textarea")}}s,should have the {{ htmlattrxref("required", "input") }} attribute applied to them. This ensures those navigating with the aid of assistive technology are able to understand which semantic form controls need valid content to ensure a successful submission. When using non-semantic elements as form widgets, such as a {{HTMLElement('div')}} with a [role]((/en-US/docs/Web/Accessibility/ARIA/Roles/) of [`checkbox`](/en-US/docs/Web/Accessibility/ARIA/Roles/checkbox_role), the `aria-required` attribute should be applied to indicate to assistive technologies that user input is required on the element before a form may be submitted.

Similar to the HTML `required` attribute set on semantic HTML form controls, the `aria-required` attribute explicitly conveys to assistive technologies that the element is required before a form may be submitted. For example, if the user needs to fill in an email address [`textbox`](/en-US/docs/Web/Accessibility/ARIA/Roles/textbox_role), include `aria-required="true"` on the textbox.

While the `required` attribute on a semantic HTML form control will prevent the form control from being submitted if no value is present, the `aria-required` attribute, like all ARIA states and properties, has no impact on element functionality. Functionality and behavior must be added in with JavaScript.
