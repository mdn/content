...
Web pages should be sequentially navigable. For this reason, developers are dissuaded from using the global {{htmlattrdef('tabindex')}} attribute which can change tab order and the CSS {{CSSXRef('order')}} property which can change visual order from the DOM order. However, sometimes a reading path different from the source order is required (like when `order` is use). For such cases, the `aria-flowto` attribute can make the content more accessible for users of assistive technologies. 

The global `aria-flowto` attribute let's the developer indicate to assistive technology and their users which element or elements might want to be focused on next providing an alternate reading order to the source order. At the user's discretion, the attribute allows assistive technology to override the general default document source reading order.

When `aria-flowto` has a single {{htmlattrdef('id')}} reference, it allows assistive technologies to, at the user's request, go to the targeted object instead of reading the document in the order of the DOM. When the `aria-flowto` value is provided a space separated list of multiple `id` references, assistive technology should provide the user with a list of path choices with each `id` referenced being a choice. In supporting assitive technologies, the path choice names in determined by the name of each target element of the `aria-flowto` attribute. 
