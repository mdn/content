...
The `aria-haspopup` attribute indicates the availability and type of interactive popup element, such as menu or dialog, that can be triggered by the element on which the attribute is set.

## Description

In ARIA, interactive menus, listboxes, trees, grids, and dialogs that appear on top of other content when triggered to appear are considered "popups". Popups are triggered by one or more interactive elements on the page. 

The presence of the `aria-haspopup` with one of six enumerated values - `menu`, `listbox`, `tree`, `grid`, `dialog`, or `true` - indicates the element can trigger a popup and what kind of popup will be displayed. The value `true` is the same as `menu`. In turn, the element that pops up must be of the role indicated. Any other value, including an empty string or other [role](/en-US/docs/Web/Accessibility/ARIA/Roles/), is treated as if `false` were set. 

A tooltip, being not interactive, is not considered to be a popup in this context.
...
- {{domxref("ElementInternals.ariaHasPopup")}}
  - : The [`ariaHasPopup`](https://developer.mozilla.org/en-US/docs/Web/API/ElementInternals/ariaHasPopup) property of the {{domxref("ElementInternals")}} interface reflects the value of the `aria-haspopup` attribute.
