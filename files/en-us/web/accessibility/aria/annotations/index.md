...
- `aria-description=""` is analogous to `aria-label=""` in that it accepts a text string, but is used when you want to give an element a description, in addition to its short label.
...
- `aria-details=""` references a more complex sets of details, disctinct from simple text descriptions. You can learn more about this in the next section.
