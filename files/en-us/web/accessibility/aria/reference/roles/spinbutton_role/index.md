...
> [!NOTE]
> It is recommended to use [`<input type="number">`](/en-US/docs/Web/HTML/Reference/Element/input/number) element, or other input types for dates and time that also implicitly have the `role="spinbutton"` semantic, rather than the `spinbutton` role. User agents provide stylized widget for the these input elements which provide default increment, decrement, and native range limiting functionality. When using non-semantic elements, all features of the native semantic element need to be recreated with ARIA attributes, JavaScript, and CSS.
...
For `spinbutton`, unless using semantic HTML elements like [`<input type="number">`](/en-US/docs/Web/HTML/Reference/Element/input/number), if the value is updated, the `aria-valuenow` value must also be updated programmatically.
...
It is recommended to use the native {{HTMLElement("input")}} element of type `number`, [`<input type="number">`](/en-US/docs/Web/HTML/Reference/Element/input/number), rather than the `spinbutton` role.
...
- [`<input type="time">`](/en-US/docs/Web/HTML/Reference/Element/input/time)
