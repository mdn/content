...
- `method` {{Optional_Inline}}
  - : The [HTTP request method](/en-US/docs/Web/HTTP/Methods) to use. Either [`GET`](/en-US/docs/Web/HTTP/Methods/GET) or [`POST`](/en-US/docs/Web/HTTP/Methods/POST). Use `POST` if the shared data changes the target app. For example, if it creates data point like a bookmark.
- `params`
  - : An object to configure the share parameters. The object keys correspond to the [`data` object in `navigator.share()`](/en-US/docs/Web/API/Navigator/share#parameters). The object values can be specified and will be used as query parameters:
    ...
    - `files` {{Optional_Inline}}: An object (or array of objects) defining which files are accepted by the share target. The object requires the following properties:
      - `name`: Name of the form field used to share files.
      - `accept`: A string (or an array strings) of accepted MIME types or file extensions.
...
When a user selects your app in the system's share dialog, your PWA is launched, and a `GET` HTTP request is made to the provided URL including the specified query parameters. It will look like this: `/shared-content-receiver/?name=a shared name&description=a shared description&link=a shared link`.
...
If the share request causes a side effect in your application, the HTTP [`POST`](/en-US/docs/Web/HTTP/Methods/POST) method should be used. For example, if your application wants to save a shared link as a bookmark in your database.
...
To handle shared file data, see the `POST` example above and the [`FileReader`](/en-US/docs/Web/API/FileReader) API to read the files.
