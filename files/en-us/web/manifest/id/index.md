...
A valid `id` needs to be same-origin as `start_url`. If the `id` is a relative URL, it is resolved using the origin of [`start_url`](/en-US/docs/Web/Manifest/start_url). The fragment is always ignored.

If the `id` is unspecified or is invalid in any way (such as not a string, not a valid URL, not same-origin as `start_url`), the `start_url` is used.
