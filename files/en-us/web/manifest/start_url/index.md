...
A valid `start_url` needs to be same-origin as the document that references the manifest. If `start_url` is unspecified or is invalid in any way (such as not a string, not a valid URL, or not a same-origin as the document), the document URL is used.
