...
- `id`
  - : A string that takes the form of a URL.
    The URL must be same-origin with the [`start_url`](/en-US/docs/Web/Manifest/Reference/start_url) of your web app.
...
> [!NOTE]
> While the `id` is processed like a URL, it does not point to a resource that can be accessed, so it is not required to be within the app's [scope](/en-US/docs/Web/Manifest/Reference/scope).
