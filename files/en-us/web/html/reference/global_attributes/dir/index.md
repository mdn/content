...
Browsers might allow users to change the directionality of {{ HTMLElement("input") }} and {{ HTMLElement("textarea") }}s in order to assist with authoring content. Chrome and Safari provide a directionality option in the contextual menu of input fields. In Firefox on Windows, pressing <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>X</kbd> inside a `<textarea>` toggles text direction and also updates the `dir` attribute value.
