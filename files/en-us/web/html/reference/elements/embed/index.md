...
- `type`
  - : The {{glossary("MIME type")}} of the embedded resource.
- `width`
...
Modern browsers no longer support browser plug-ins such as Flash, but `<embed>` can still display content handled by the browser itself, such as PDFs in a built-in PDF viewer.

For PDF previews, consider {{HTMLElement("object")}} or {{HTMLElement("iframe")}}. Unlike `<object>`, `<embed>` cannot contain fallback content. Unlike `<iframe>`, it has no attributes that control loading.
