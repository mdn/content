...
An `<input type="button">` elements' [`value`](/en-US/docs/Web/HTML/Reference/Elements/input#value) attribute contains a string that is used as the button's label. The `value` provides the {{glossary("accessible description")}} for the button.
