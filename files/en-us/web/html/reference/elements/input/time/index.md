...
The `step` attribute is a number that specifies the granularity that the value must adhere to, or the special value `any`, which is described below. Only values which are a whole number of steps from the step base are valid. The step base is [`min`](#min) if specified, [`value`](/en-US/docs/Web/HTML/Reference/Elements/input#value) otherwise, or `00:00:00` (midnight) if neither is provided.
...
By setting a [`min`](/en-US/docs/Web/HTML/Reference/Elements/input#min) attribute greater than the [`max`](/en-US/docs/Web/HTML/Reference/Elements/input#max) attribute, the valid time range will wrap around midnight to produce a valid time range. This functionality is not supported by any other input types.
...
In addition, you can use the [`required`](/en-US/docs/Web/HTML/Reference/Elements/input#required) attribute to make filling in the time mandatory. Browsers will display an error if you try to submit a time that is outside the set bounds, or an empty time field.
...
<table class="properties">
  <tbody>
    <tr>
      <td><strong><a href="/en-US/docs/Web/HTML/Reference/Elements/input#value">Value</a></strong></td>
