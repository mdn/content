...
slug: Web/HTML/Reference/Element/meta/name
...
The {{htmlelement("meta")}} element can be used to provide document metadata in terms of name-value pairs, with the [`name`](/en-US/docs/Web/HTML/Reference/Element/meta#name) attribute giving the metadata name, and the [`content`](/en-US/docs/Web/HTML/Reference/Element/meta#content) attribute giving the value.
...
- [`theme-color`](/en-US/docs/Web/HTML/Reference/Element/meta/name/theme-color): indicates a suggested color that user agents should use to customize the display of the page or of the surrounding user interface. The `content` attribute contains a valid CSS {{cssxref("&lt;color&gt;")}}. The `media` attribute with a valid media query list can be included to set the media the theme color metadata applies to.
- <a id="color-scheme" href="#color-scheme">`color-scheme`</a>: specifies one or more color schemes with which the document is compatible.
  ...
  The value of the [`content`](/en-US/docs/Web/HTML/Reference/Element/meta#content) property for `color-scheme` may be one of the following:
...
- [Viewport `<meta>` tag](/en-US/docs/Web/HTML/Guides/Viewport_meta_tag)
