...
slug: Web/HTML/Reference/Element/thead
...
In order to associate and line up the header cells with the correct columns and rows, the [`colspan`](/en-US/docs/Web/HTML/Reference/Element/th#colspan) and [`rowspan`](/en-US/docs/Web/HTML/Reference/Element/th#rowspan) attributes are used on the {{HTMLElement("th")}} elements. The values set in these attributes specify how many cells each header cell ({{HTMLElement("th")}}) spans. Due to the way these attributes are set, the two second-row header cells are lined up with the columns they head. These each span one row and one column as the default values for the [`colspan`](/en-US/docs/Web/HTML/Reference/Element/th#colspan) and [`rowspan`](/en-US/docs/Web/HTML/Reference/Element/th#rowspan) attributes are both `1`.
