...
slug: Web/HTML/Reference/Element/a
...
If an icon is used to signify link behavior, make sure it has an [`alt` attribute](/en-US/docs/Web/HTML/Reference/Element/img#alt) to describe its purpose. In case the icon is missing, the `alt` attribute's content will still convey the link's behavior.
