...
> [!NOTE]
> Setting `target="_blank"` on `<a>`, `<area>` and `<form>` elements now implicitly provides the same `rel` behavior as setting `rel="noopener"` which does not set `window.opener`. See browser compatibility ([`<a>`](/en-US/docs/Web/HTML/Element/a#browser_compatibility), [`<area>`](/en-US/docs/Web/HTML/Element/area#browser_compatibility)) for support status.
