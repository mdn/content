...
<table class="standard-table">
	...
	<thead>
		<tr>
			...
			<th><code>{{htmlelement('a')}}</code> and <code>{{htmlelement('area')}}</code></th>
...
<p>The <code>rel</code> attribute is relevant to the {{htmlelement('link')}}, {{htmlelement('a')}}, {{htmlelement('area')}}, and {{htmlelement('form')}} elements, but some values only relevant to a subset of those elements. Like all HTML keyword attribute values, these values are case-insenstive.</p>
...
&lt;link rel="alternate stylesheet" href="highcontrast.css" title="High contrast"&gt;</pre>
		...
		<li>With an <a href="hreflang">hreflang</a> attribute that differs from the document language, it indicates a translation.</li>
		<li>With the <a href="type">type</a> attribute, it indicates that the referenced document is the same content in a different format. For example, with <code>type="application/rss+xml"</code> it creates a hyperlink referencing a syndication feed.
		<pre class="brush:html">&lt;link rel="alternate" type="application/atom+xml" href="posts.xml" title="Blog"&gt;</pre>
		</li>
		<li>Both the <a href="hreflang">hreflang</a> and <a href="type">type</a> attributes specify links to versions of the document in an alternative format and language, intended for other media:
...
&lt;link rel=alternate href="/fr/pdf" hreflang=fr type=application/pdf title="French PDF"&gt;</pre>
  ...
  <div class="note"><p><strong>Note:</strong> The obsolete <code>rev="made"</code> is treated as <code>rel="alternate"</code></p></div>
	</dd>
	...
	<dd>Valid for {{htmlelement('link')}}, it defines the preferred URL for the current document, which is useful for search engines.</dd>
	<dt>{{htmlattrdef("dns-prefetch")}}</dt>
	<dd>Relevant for the {{htmlelement('link')}} element both in the {{htmlelement('body')}} and {{htmlelement('head')}}, it tells the browser to preemptively perform DNS resolution for the target resource's origin. Useful for resources the user will likely need, it helps reduce latency and thereby improves performance when the user does access the resources as the browser preemptively performed DNS resolution for the origin of the specified resource. See <a href="/en-US/docs/Web/Performance/dns-prefetch">dns-prefetch</a> described in <a href="">resource hints</a>.</dd>
	<dt>{{htmlattrdef("external")}}</dt>
	<dd>Relevant to {{htmlelement('form')}}, {{htmlelement('a')}}, and {{htmlelement('area')}}, it indicates the referenced document is not part of the current site. This can be used with attribute selectors to style external links in a way that indicates to the user that they will be leaving the current site.</dd>
	...
	<div class="note"><p><strong>Note:</strong> The <code>shortcut</code> link type is often seen before <code>icon</code>, but this link type is non-conforming, ignored and <strong>web authors must not use it anymore</strong>.</p></div>
	</dd>
	...
	<p><strong>Note:</strong> Although recognized, the synonym <code>copyright</code> is incorrect and must be avoided.</p>
	</dd>
	<dt>{{htmlattrdef("manifest")}}</dt>
	<dd><a href="/en-US/docs/Web/Manifest">Web app manifest</a>. Requires the use of the CORS protocol for cross-origin fetching.</dd>
	<dt>{{htmlattrdef("modulepreload")}}</dt>
	<dd>Useful for improved performance, and relevant to the {{htmlelement('link')}} anywhere in the document, setting <code>rel="modulepreload"</code> tells the browser to preemptively fetch the script (and dependencies) and store it in the document's module map for later evaluation. <code>modulepreload</code> links can ensure network fetching is done with the module ready (but not evaluated) in the module map before it is necessarily needed. See also <a href="/en-US/docs/Web/HTML/Link_types/modulepreload">link types: <code>modulepreload</code></a>.</dd>
	<dt>{{htmlattrdef("next")}}</dt>
	<dd>Relevant to {{htmlelement('form')}}, {{htmlelement('link')}}, {{htmlelement('a')}}, and {{htmlelement('area')}}, the <code>next</code> values indicates that the current document is a part of a series, and that the next document in the series is the referenced document. When included in a <code>&lt;link&gt;</code>, browsers may assume that document will be fetched next, and treat it as a resource hint.</dd>
	<dt>{{htmlattrdef("nofollow")}}</dt>
	<dd>Relevant to {{htmlelement('form')}}, {{htmlelement('a')}}, and {{htmlelement('area')}}, the <code>nofollow</code> keyword tells search engine spiders to ignore the link relationship. The nofollow relationship may indicate the current document's owner does not endorse the referenced document. It is often included by Search Engine Optimizers pretending their link farms are not spam pages.</dd>
	<dt>{{htmlattrdef("noopener")}}</dt>
	<dd>Relevant to {{htmlelement('form')}}, {{htmlelement('a')}}, and {{htmlelement('area')}}, creates a top-level browsing context that is not an auxiliary browsing context if the hyperlink would create either of those to begin with (i.e., has an appropriate<code> target </code>attribute value). In other words, it makes the link behave as if <code><a href="/en-US/docs/Web/API/Window/opener">window.opener</a></code> were null and <code><a href="target">target</a>="_parent"</code> were set.<br>
	<br>
	This is the opposite of {{anch("opener")}}.</dd>
	<dt>{{htmlattrdef("noreferrer")}}</dt>
	<dd>Relevant to {{htmlelement('form')}}, {{htmlelement('a')}}, and {{htmlelement('area')}}, including this value makes the referrer unknown (no <code>Referer</code> header will be included), and creates a top-level browsing context as if <code>noopener</code> were also set.</dd>
	<dt>{{htmlattrdef("opener")}}</dt>
	<dd>Creates an auxiliary browsing context if the hyperlink would otherwise create a top-level browsing context that is not anauxiliary browsing context (i.e., has "<code>_blank</code>" as <code>target</code> attribute value). Effectively, the opposite of {{anch("noopener")}}.</dd>
	<dt>{{htmlattrdef("pingback")}}</dt>
	<dd>Gives the address of the pingback server that handles pingbacks to the current document.</dd>
	<dt>{{htmlattrdef("preconnect")}}</dt>
	<dd>Specifies that the user agent should preemptively connect to the target resource's origin.</dd>
	<dt>{{htmlattrdef("prefetch")}}</dt>
	<dd>Specifies that the user agent should preemptively fetch and cache the target resource as it is likely to be required for a followup navigation.</dd>
	<dt>{{htmlattrdef("preload")}}</dt>
	<dd>Specifies that the user agent must preemptively fetch and cache the target resource for current navigation according to the potential destination given by the <code><a href="as">as</a></code> attribute (and the priority associated with the corresponding destination).</dd>
	<dt>{{htmlattrdef("prerender")}}</dt>
	<dd>Specifies that the user agent should preemptively fetch the target resource and process it in a way that helps deliver a faster response in the future.</dd>
	<dt>{{htmlattrdef("prev")}}</dt>
	<dd>
	<p>Similar to the {{anch("next")}} keyword, relevant to {{htmlelement('form')}}, {{htmlelement('link')}}, {{htmlelement('a')}}, and {{htmlelement('area')}}, the <code>prev</code> values indicates that the current document is a part of a series, and that the link references a previous document in the series is the referenced document.</p>

	<p>Note: The synonym <code>previous</code> is incorrect and should not be used.</p>
	</dd>
	<dt>{{htmlattrdef("search")}}</dt>
	<dd>
	<p>Relevant to {{htmlelement('form')}}, {{htmlelement('link')}}, {{htmlelement('a')}}, and {{htmlelement('area')}} elements, the <code>search</code> keywords indicates that the hyperlink references a document whose interface is specially designed for searching in the current document, site, and related resources, providing a link to a resource that can be used to search.</p>

	<p>If the <code><a href="type">type</a></code> attribute is set to <code>application/opensearchdescription+xml</code> the resource is an <a href="/en-US/docs/Web/OpenSearch">OpenSearch</a> plugin that can be easily added to the interface of some browsers like Firefox or Internet Explorer.</p>
	</dd>
	<dt>{{htmlattrdef("stylesheet")}}</dt>
	<dd>
	<p>Valid for the {{htmlelement('link')}} element, it imports an external resource to be used as a stylesheet. The <code><a href="type">type</a></code> attribute is not needed as it's a <code>text/css</code> stylesheet, as that is the default value. If it's not a stylesheet of type <code>text/css</code> it is best to declare the type.</p>

	<p>While this attribute defines the link as being a stylesheet, the interaction with other attributes and other key terms within the rel value impact whether the stylesheet is downloaded and/or used.</p>

	<p>When used with the {{anch('alternate')}} keyword, it defines an alternative style sheet. In this case, include a non-empty <code><a href="type">title</a></code>.</p>

	<p>The external stylesheet will not be used or even downloaded if the media does not match the value of the <code><a href="media">media</a></code> attribute.</p>

	<p>Requires the use of the CORS protocol for cross-origin fetching.</p>
	</dd>
	<dt>{{htmlattrdef("tag")}}</dt>
	<dd>Valid for the {{htmlelement('a')}}, and {{htmlelement('area')}} elements, it gives a tag (identified by the given address) that applies to the current document. The tag value denotes that the link refers to a document describing a tag applying to the document on which it is located. This link type is not meant for tags within a tag cloud, as those tags apply to a group of pages, whereas the <code>tag</code> value of the <code>rel</code> attribute is for a single document.</dd>
</dl>
