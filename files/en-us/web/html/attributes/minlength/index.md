...
`minlength` does not imply [`required`](/en-US/docs/Web/HTML/Attributes/required). An input only violates `minlength` if the user has input something. If an input is not required, it can be empty even if `minlength` is set.
