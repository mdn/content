...
<p>The <code><strong>accesskey</strong></code> <a href="/en-US/docs/Web/HTML/Global_attributes">global attribute</a> provides a hint for generating a keyboard shortcut for the current element. The attribute value must consist of a single printable character (which includes accented and other characters that can be generated by the keyboard).</p>
...
<table class="standard-table">
 <tbody>
  ...
  <tr>
   ...
   <td rowspan="3"><kbd>Alt</kbd> + <kbd><em>key</em></kbd><br>
  ...
  <tr>
   ...
   <td>N/A</td>
   <td rowspan="3">
    <kbd>Control</kbd> + <kbd>Option</kbd> + <kbd><em>key</em></kbd><br>
    <kbd>Control</kbd> + <kbd>Option</kbd> + <kbd>Shift</kbd> + <kbd><em>key</em></kbd>
   </td>
  </tr>
  <tr>
   <th>Google Chrome</th>
...
<p>In addition to poor browser support, there are numerous concerns with the <code>accesskey</code> attribute:</p>

<ul>
 <li>An <code>accesskey</code> value can conflict with a system or browser keyboard shortcut, or assistive technology functionality. What may work for one combination of operating system, assistive technology, and browser may not work with other combinations.</li>
 <li>Certain <code>accesskey</code> values may not be present on certain keyboards, especially when internationalization is a concern. So adapting to specific languages could cause further problems.</li>
 <li><code>accesskey</code> values that rely on numbers may be confusing to individuals experiencing cognitive concerns, where the number doesn't have a logical association with the functionality it triggers. </li>
 <li>Informing the user that <code>accesskey</code>s are present, so that they are aware of the functionality. If the system lacks a method of notifying the user about this feature, the user might accidentally activate <code>accesskey</code>s.</li>
</ul>

<p>Because of these issues, it is generally advised not to use <code>accesskey</code>s for most general-purpose websites and web apps.</p>
