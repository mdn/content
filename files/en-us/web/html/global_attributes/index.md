...
- [`role`](/en-US/docs/Web/Accessibility/ARIA/Roles)
  - : roles provide semantic meaning to content, allowing screen readers and other tools to present and support interaction with object in a way that is consistent with user expectations of that type of object. `roles` are added to HTML elements using `role="role type"`, where role type is the name of a role in the ARIA specification.
