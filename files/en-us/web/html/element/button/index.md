...
The {{cssxref(":focus-visible")}} pseudo-class can be used to apply styles to an element that has {{cssxref(":focus")}} only when the User Agent's heuristics determine that the focus should be highlighted, such as when a `<button>` receives keyboard focus. See [[:focus vs :focus-visible](/en-US/docs/Web/CSS/:focus-visible#focus_vs_focus-visible)](/en-US/docs/Web/CSS/:focus-visible#focus_vs_focus-visible) for more information.
