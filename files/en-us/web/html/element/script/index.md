...
Scripts without [`async`](/en-US/docs/Web/HTML/Element/script#async), [`defer`](/en-US/docs/Web/HTML/Element/script#defer) or `type="module"` attributes, as well as inline scripts without the `type="module"` attribute, are fetched and executed immediately, before the browser continues to parse the page.
...
Browsers that support the `module` value for the [`type`](/en-US/docs/Web/HTML/Element/script#type) attribute ignore any script with a `nomodule` attribute. That enables you to use module scripts while also providing `nomodule`-marked fallback scripts for non-supporting browsers.
