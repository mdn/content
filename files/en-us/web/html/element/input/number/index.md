...
The implicit [role](/en-US/docs/Web/Accessibility/ARIA/Roles) for `<input type="number">` is [`spinbutton`](/en-US/docs/Web/Accessibility/ARIA/Roles/spinbutton). If the spinbutton is not an important feature for your form control, consider *not* using `type="number"`, opting for [`inputmode="numeric"`](/en-US/docs/Web/HTML/Global_attributes/inputmode) along with a [`pattern`](/en-US/docs/Web/HTML/Attributes/pattern) attribute limiting the characters to numbers and associated characters instead. With `<input type="number">` there is a risk of users accidentally incrementing a number when they’re trying to do something else and, if the user tries to enter something that’s not a number, there’s no explicit feedback about what they’re doing wrong.
