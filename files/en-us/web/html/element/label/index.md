...
To explicitly associate a `<label>` element with an `<input>` element, you first need to add the `id` attribute to the `<input>` element. Next, add the `for` attribute to the `<label>` element. Lastly, make the value of `for` the same as the `id` in the `<input>` element.
