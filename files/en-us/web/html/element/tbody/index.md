...
- `align` {{deprecated_inline}}
  ...
  - : This [enumerated](/en-US/docs/Glossary/Enumerated) attribute specifies how horizontal alignment of each cell content will be handled. Possible values are:
    ...
    - `char`, aligning the textual content on a special character with a minimal offset, defined by the [`char`](/en-US/docs/Web/HTML/Element/tbody#char) and [`charoff`](/en-US/docs/Web/HTML/Element/tbody#charoff) attributes.
...
- `char` {{deprecated_inline}}
  - : This attribute is used to set the character to align the cells in a column on. Typical values for this include a period (`.`) when attempting to align numbers or monetary values. If [`align`](/en-US/docs/Web/HTML/Element/tbody#align) is not set to `char`, this attribute is ignored.
...
Notice that each major is placed in a separate `<tbody>` block, the first row of which contains a single {{HTMLElement("th")}} element with a [`colspan`](/en-US/docs/Web/HTML/Element/th#colspan) attribute that spans the entire width of the table. That heading lists the name of the major contained within the `<tbody>`.
