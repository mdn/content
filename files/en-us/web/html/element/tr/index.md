...
- If the `<tr>` is placed as a direct child of its parent {{HTMLElement("table")}} element, the `<tbody>` parent is implied and browsers will actually add the `<tbody>` to the markup.
- This implied `<tbody>` parent is only supported if the `<table>` otherwise has no child {{HTMLElement("tbody")}} elements, and only if the `<tr>` is included after any {{HTMLElement("caption")}}, {{HTMLElement("colgroup")}}, and {{HTMLElement("thead")}} elements.
