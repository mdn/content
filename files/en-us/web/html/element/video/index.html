...
<p>The <strong><code>&lt;video&gt;</code></strong> <a href="/en-US/docs/Web/HTML">HTML</a> element embeds a media player which supports video playback into the document. You can use <code>&lt;video&gt;</code> for audio content as well, but the {{HTMLElement("audio")}} element may provide a more appropriate user experience.</p>

<div>{{EmbedInteractiveExample("pages/tabbed/video.html", "tabbed-standard")}}</div>

<p>The above example shows simple usage of the <code>&lt;video&gt;</code> element. In a similar manner to the {{htmlelement("img")}} element, we include a path to the media we want to display inside the <code>src</code> attribute; we can include other attributes to specify information such as video width and height, whether we want it to autoplay and loop, whether we want to show the browser's default video controls, etc.</p>
...
<dl>
 ...
 <dd>A Boolean attribute; if specified, the video automatically begins to play back as soon as it can do so without stopping to finish loading the data.
...
<p>Browsers don't all support the same video formats; you can provide multiple sources inside nested {{htmlelement("source")}} elements, and the browser will then use the first one it understands.</p>
...
<p>We offer a substantive and thorough <a href="/en-US/docs/Web/Media/Formats">guide to media file types</a> and the <a href="/en-US/docs/Web/Media/Formats/Video_codecs">guide to the codecs supported for video</a>. Also available is a guide to <a href="/en-US/docs/Web/Media/Formats/Audio_codecs">audio codecs that can be used with them</a>.</p>
...
<ul>
 ...
 <li>You can play audio files using a <code>&lt;video&gt;</code> element. This can be useful if, for example, you need to perform audio with a <a href="/en-US/docs/Web/API/WebVTT_API">WebVTT</a> transcript, since the {{HTMLElement("audio")}} element doesn't allow captions using WebVTT.</li>
 <li>To test the fallback content on browsers that support the element, you can replace <code>&lt;video&gt;</code> with a non-existing element like <code>&lt;notavideo&gt;</code>.</li>
</ul>
...
<p>There are no special considerations for styling <code>&lt;video&gt;</code>; a common strategy is to give it a <code>display</code> value of <code>block</code> to make it easier to position, size, etc., and then provide styling and layout information as required. <a href="/en-US/docs/Web/Guide/Audio_and_video_delivery/Video_player_styling_basics">Video player styling basics</a> provides some useful styling techniques.</p>
...
<p>First <a href="/en-US/docs/Web/Media/Formats/Containers#webm">WebM</a> is tried. If that can't be played, then <a href="/en-US/docs/Web/Media/Formats/Containers#mp4">MP4</a> is tried. Finally, <a href="/en-US/docs/Web/Media/Formats/Containers#ogg">Ogg</a> is tried. A fallback message is displayed if the video element isn't supported, but not if all sources fail.</p>

<p>Some media file types let you provide more specific information using the <code><a href="/en-US/docs/Web/Media/Formats/codecs_parameter">codecs</a></code> parameter as part of the file's type string. A relatively simple example is <code>video/webm; codecs="vp8, vorbis"</code>, which says that the file is a <a href="/en-US/docs/Web/Media/Formats/Containers#webm">WebM</a> video using <a href="/en-US/docs/Web/Media/Formats/Video_codecs#vp8">VP8</a> for its video and <a href="/en-US/docs/Web/Media/Formats/Audio_codecs#vorbis">Vorbis</a> for audio.</p>
...
<p>Videos should provide both captions and transcripts that accurately describe its content (see <a href="/en-US/docs/Web/Guide/Audio_and_video_delivery/Adding_captions_and_subtitles_to_HTML5_video">Adding captions and subtitles to HTML5 video</a> for more information on how to implement these). Captions allow people who are experiencing hearing loss to understand a video's audio content as the video is being played, while transcripts allow people who need additional time to be able to review audio content at a pace and format that is comfortable for them.</p>

<p>It's worth noting that while you can caption audio-only media, you can only do so when playing audio in a {{HTMLElement("video")}} element, since the video region of the element is used to present the captions. This is one of the special scenarios in which it's useful to play audio in a video element.</p>
...
<ul>
 <li><a href="/en-US/docs/Web/Media/Formats">Guide to media types and formats on the web</a>
 ...
 <li><a href="/en-US/docs/Web/HTTP/Configuring_servers_for_Ogg_media">Configuring servers for Ogg media</a></li>
</ul>
