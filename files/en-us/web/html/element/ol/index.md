...
The `<ol>`, {{HTMLElement("ul")}} and {{HTMLElement("menu")}} elements may nest as deeply as desired, alternating between `<ol>`, `<ul>` and `<menu>` however you like.
...
To determine which list to use, try changing the order of the list items; if the meaning changes, use the `<ol>` element — otherwise you can use {{HTMLElement("menu")}} for interactive items or {{HTMLElement("ul")}} otherwise.
