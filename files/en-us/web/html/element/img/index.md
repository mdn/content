...
- `sizes`
  ...
  - : One or more strings separated by commas, indicating a set of source sizes. Each source size consists of:
    ...
    A source size value can be any non negative [length](https://developer.mozilla.org/en-US/docs/Web/CSS/length) apart from {percentage](https://drafts.csswg.org/css-values/#percentage-value) and must not use CSS functions other than the [math functions](https://drafts.csswg.org/css-values/#math-function). Note that all relative length units are relative to the document root rather than the img element, so an `em` value is relative to the root font size, rather than the font size of the image.
