...
To bind the `<datalist>` element to the control, we give it a unique identifier in the [`id`](/en-US/docs/Web/HTML/Global_attributes/id) attribute, and then add the {{htmlattrxref("list","input")}} attribute to the {{HTMLElement("input")}} element with the same identifier as value.
