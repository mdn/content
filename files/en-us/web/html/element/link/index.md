...
- `sizes` {{Experimental_Inline}}
  ...
  - : This attribute defines the sizes of the icons for visual media contained in the resource.
    ...
    > **Note:** Most icon formats are only able to store one single icon; therefore, most of the time, the [`sizes`](/en-US/docs/Web/HTML/Element/link#sizes) attribute contains only one entry.
