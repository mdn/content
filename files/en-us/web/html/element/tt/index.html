...
<p>The <strong><code>&lt;tt&gt;</code></strong> <a href="/en-US/docs/Web/HTML">HTML</a> element creates inline text which is presented using the {{Glossary("user agent", "user agent's")}} default monospace font face. This element was created for the purpose of rendering text as it would be displayed on a fixed-width display such as a teletype, text-only screen, or line printer.</p>

<p>The terms <strong>non-proportional</strong>, <strong>monotype</strong>, and <strong>monospace</strong> are used interchangeably and have the same general meaning: they describe a typeface whose characters are all the same number of pixels wide.</p>
...
<p>This example uses <code>&lt;tt&gt;</code> to show text entered into, and output by, a terminal application.</p>
...
<h3 id="Overriding_the_default_font">Overriding the default font</h3>

<p>You can override the browser's default font—if the browser permits you to do so, which it isn't required to do—using CSS:</p>
...
<ul>
 <li>The semantic {{HTMLElement("code")}}, {{HTMLElement("var")}}, {{HTMLElement("kbd")}}, and {{HTMLElement("samp")}} elements</li>
 <li>The {{HTMLElement("pre")}} element for displaying preformatted text blocks</li>
</ul>
