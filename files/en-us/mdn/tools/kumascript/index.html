...
<pre class="brush: js">&lt;% for (let i = 0; i &lt; $0; i++) { %&gt;
  Hello #&lt;%= i %&gt;
&lt;% } %&gt;</pre>
...
<pre class="brush: js">\{{hello(3)}}</pre>

<p>The output of the macro would look like this:</p>

<pre class="brush: html">Hello #0
Hello #1
Hello #2</pre>
...
<pre class="brush: js">\{{templateName("arg0", "arg1", ..., "argN")}}</pre>
...
<pre class="brush: js">\{{templateName({ &quot;Alpha&quot;: &quot;one&quot;, &quot;Beta&quot;: [&quot;a&quot;, &quot;b&quot;, &quot;c&quot;], &quot;Foo&quot;: &quot;https:\/\/mozilla.org\/&quot; })}}</pre>
...
<dl>
 ...
 <dd>The path to the current MDN document</dd>
 <dt><code>env.url</code></dt>
 <dd>The full URL to the current MDN document</dd>
 <dt><code>env.id</code></dt>
 <dd>A short, unique ID for the current MDN document</dd>
 <dt><code>env.files</code></dt>
 <dd>An array of the files attached to the current MDN document; each object in the array is as described under {{ anch("File objects") }} below</dd>
 ...
 <dd>The locale of the current MDN document</dd>
 <dt><code>env.title</code></dt>
 <dd>The title of the current MDN document</dd>
 <dt><code>env.slug</code></dt>
 <dd>The URL slug of the current MDN document</dd>
 <dt><code>env.tags</code></dt>
 <dd>An array list of tag names for the current MDN document</dd>
 <dt><code>env.modified</code></dt>
 <dd>Last modified timestamp for the current MDN document</dd>
 <dt><code>env.cache_control</code></dt>
 <dd><code>Cache-Control</code> header sent in the request for the current MDN document, useful in deciding whether to invalidate caches</dd>
</dl>
...
<p>This manually-maintained documentation is likely to fall out of date with the code. With that in mind, <a class="link-https" href="https://github.com/mdn/yari/tree/master/kumascript/src/api">you can always check out the latest state of built-in APIs in the KumaScript source</a>. But here is a selection of useful methods exposed to templates:</p>
...
<dl>
 ...
 <dd>Loads another template as a module; any output is ignored. Anything assigned to <code>module.exports</code> in the template is returned. Used in templates like so: <code>&lt;% const my_module = require('MyModule'); %&gt;</code>.</dd>
 ...
 <dd>Access to <a class="link-https" href="https://github.com/request/request"><code>request/request</code></a>, a library for making HTTP requests. Using this module in KumaScript templates is not yet very friendly, so you may want to wrap usage in module APIs that simplify things.</dd>
...
<pre class="brush: js">&lt;%- log.debug("Some text goes here"); %&gt;</pre>
...
<pre class="brush: plain">Kumascript service failed unexpectedly: &lt;class 'httplib.BadStatusLine'&gt;</pre>
...
<pre class="brush: plain">Syntax error at line 436, column 461: Expected valid JSON object as the parameter of the preceding macro but...</pre>

<p>If you edit the page, you’ll probably see a macro like this at the bottom of the page:</p>

<pre class="brush: plain">\{{ wiki.languages({ &quot;zh-tw&quot;: &quot;zh_tw/Core_JavaScript_1.5_教學/JavaScript_概要&quot;, ... }) }}</pre>
...
<pre class="brush: html">&lt;!-- wiki.languages({ "zh-tw": "zh_tw/Core_JavaScript_1.5_教學/JavaScript_概要", ... }) --&gt;</pre>
...
<pre class="brush: js">const lang = env.locale;</pre>
...
<pre class="brush: js">&lt;%- mdn.getFileContent(fileObject); %&gt;
</pre>
...
<pre class="brush: js">&lt;%- mdn.getFileContent(env.files[0]); %&gt;
</pre>
...
<pre class="brush: plain">%s#&lt;span#^M&lt;span#g
