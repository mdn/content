...
<p>MDN supports turning sample code displayed in articles into running samples the reader can look at in action. These live samples can include HTML, CSS, and JavaScript in any combination. Note that "live" samples are <em>not interactive</em>; however, they do ensure that the output displayed for a sample matches the code of the sample exactly, because it is actually generated by the code sample.</p>
...
<p>In many cases, you may be able to add the <code>EmbedLiveSample</code> or <code>LiveSampleLink</code> macro to pages with little or no additional work! As long as the sample can be identified by a heading's ID or is in a block with an ID you can use, adding the macro should do the job.</p>
...
<dl>
 ...
 <dd>
   ...
   <div class="warning">
   <p><strong>Warning:</strong> To show a live sample from one code-containing page in a different, target page, the code-containing page must itself use the <code><a href="https://github.com/mdn/kumascript/blob/master/macros/EmbedLiveSample.ejs">EmbedLiveSample</a></code> macro to embed a live sample in its own page. Otherwise, if the code-containing page doesn't itself use the <code><a href="https://github.com/mdn/kumascript/blob/master/macros/EmbedLiveSample.ejs">EmbedLiveSample</a></code> macro its own page, the live sample will fail to display at all on the target page. (See <a href="https://github.com/mdn/yari/issues/2243">Yari issue #2243</a>.)</p>
   </div>
 </dd>
</dl>
...
<p>Each piece of code must be in a {{HTMLElement("pre")}} block, with a separate block for each language, properly marked as to which language it is. Most of the time, this has already been done, but it's always worth double-checking to be sure each piece of code is configured with the correct syntax. This is done with a class on the <code>&lt;pre&gt;</code> element of <code>brush:<em>language-type</em></code>, where <em>language-type</em> is the type of language the block contains, e.g. <code>html</code>, <code>css</code>, or <code>js</code>.</p>
...
<h2 id="Conventions_regarding_live_samples">Conventions regarding live samples</h2>
