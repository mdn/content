...
>
> - **title**
>   - : Title heading is displayed at the top of the page. The title format is _NameOfTheProperty_.
...
>
> - Formal definition and Formal syntax sections: The content of the _Formal definition_ and _Formal syntax_ sections is generated in place of the `\{{CSSInfo}}` and `\{{CSSSyntax}}` macros, respectively. For these sections to populate, you must ensure an appropriate entry has been filled in for the property in our [properties.json](https://github.com/mdn/data/blob/main/css/properties.json) data file. See [properties.md](https://github.com/mdn/data/blob/main/css/properties.md) for more information.
> - Specifications and Browser compatibility sections: The toolchain automatically uses the `browser-compat` key-value pair to populate the Specifications and Browser compatibility sections (replacing the `\{{Specifications}}` and `\{{Compat}}` macros in those sections, respectively).
>
...
Follow up the introductory paragraph with a call to the interactive example macro `\{{EmbedInteractiveExample}}`. This will generate an example section with the title "Try it". See the [Interactive example demo](/en-US/docs/MDN/Writing_guidelines/Page_structures/Code_examples#interactive_example_demo) section in our writing guidelines for more information.

## Constituent properties

This section needs to be added only for shorthand properties like [animation](/en-US/docs/Web/CSS/animation) to list all the related long-hand properties.
...
- subvalue1
  - : Include a description of the subvalue, its datatype, and what it represents.
- subvalue2
  - : Include a description of the subvalue, its datatype, and what it represents.
...
This is an optional section. You can include here any warnings for any accessibility concerns that developers should be aware of while using this property. You can also include if there are workarounds for these accessibility concerns.
...
Include here a list of other properties or guides related to the current property page
