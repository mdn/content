...
### Adding content to your sidebar before or after the quicklink

Each of the quicklink macros listed above creates a predefined sidebar structure.
If you need to append links before or after the sidebar structure generated by the macro, you can do so by nesting both the macro and any additional links within an HTML {{htmlelement("section")}} element that has the `id="Quick_links"`.
This section must be appended to the end of the page, instead of between the frontmatter and the page content, and any sidebar quicklink macro below the frontmatter must be removed (there can only be one sidebar defined in each document).
...
You can also fully craft a sidebar without using any quicklink macro.
Note however that links that you add that aren't generated by a quicklinks macro should be inserted as an ordered markdown list.
