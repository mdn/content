...
<p><strong>Article tags</strong> are an important way to put visitors in touch with helpful content. Each page should normally have several tags to help keep content organized. This page explains the best way to tag pages so that our readers can find information and we can keep ourselves organized.</p>
...
<div class="warning">
  <p><strong>Warning:</strong> Note to translators and localizers: You should <em>not</em> translate any tags listed on this page. They are used for specific purposes such as certain site management tasks and automated data processing, and translating them would break these processes.</p>
</div>
...
<ul>
 ...
 <li><code>CSS</code></li>
 <li><code>JavaScript</code> (notice the capital "S"!)</li>
 <li><code>Document</code></li>
 <li><code>DOM</code></li>
 <li><code>API</code> for each API's overview, interface, method, and property.</li>
 <li><code>Method</code> for each method of an API</li>
 <li><code>Property</code> for each property of an API</li>
 ...
 <li><code>Extensions</code> and <code>WebExtensions</code> for WebExtension documentation.</li>
</ul>
...
<dl>
 ...
 <dd>Include the tag <code>RTCPeerConnection</code> along with the other relevant tags (<code>Interface</code>, <code>WebRTC</code>, <code>WebRTC API</code>, <code>API</code>, <code>Reference</code>, and so forth).</dd>
 <dt>The method {{DOMxRef("RTCPeerConnection.createOffer()")}}</dt>
 <dd>Include the tags <code>RTCPeerConnection</code> and <code>createOffer</code> (note <em>no</em> parentheses in tag names!) along with the other relevant tags, including <code>Method</code>, <code>WebRTC</code>, <code>WebRTC API</code>, <code>API</code>, <code>Reference</code>, and so forth. Consider including things like <code>Offer</code> and <code>SDP</code>, which are also relevant here.</dd>
 <dt>The property {{DOMxRef("RTCPeerConnection.iceConnectionState")}}</dt>
 <dd>Include the tags <code>RTCPeerConnection</code> and <code>iceConnectionState</code> along with the other relevant tags, including <code>Property</code>, <code>WebRTC</code>, <code>WebRTC API</code>, <code>API</code> and <code>Reference</code>. Also consider including <code>ICE</code>.</dd>
</dl>
...
<dl>
 ...
 <dd>The technology is not standardized, and is an experimental technology or API that may or may not ever become part of a standard. It is also subject to change in the browser engine (typically only one) that implements it. If the technology isn't part of any specification (even in draft form), it should also have the Non-standard tag.</dd>
 <dt><code>Needs Privileges</code></dt>
 <dd>The API requires privileged access to the device on which the code is running.</dd>
 <dt><code>Certified Only</code></dt>
 <dd>The API only works in certified code.</dd>
</dl>

<p>These tags are no excuse to leave out the <a href="/en-US/docs/MDN/Structures/Compatibility_tables">compatibility table</a> in your article! That should always be present.</p>
...
<dl>
 ...
 <dt><code>NeedsCompatTable</code></dt>
 <dd>The article needs a table to specify compatibility of a feature across browsers. <a href="/en-US/docs/MDN/Structures/Compatibility_tables">See here</a> for a guide on contributing to browser compatibility.</dd>
...
<dl>
 ...
 <dd>WebGL, Graphics, Guide, Beginner</dd>
 <dt>Reference page for {{HTMLElement("canvas")}}</dt>
 <dd>Canvas, HTML, Element, Graphics, Reference</dd>
 <dt>A landing page for Firefox developer tools</dt>
 <dd>Tools, Firefox, Landing</dd>
</dl>
