...
Switches and routers do different jobs rather than one replacing the other. A switch only needs to know about devices plugged into its local network. If we kept connecting more and more switches into one enormous flat network, messages would pass through more relays on the way to their destination, and failures would affect more machines at once. Routers avoid that by joining many small switched local networks instead of growing one giant one. They forward packets between those separate networks using IP addresses, while switches keep delivering messages within each local network using MAC addresses. IP addresses are explained further in the [Finding computers](#finding_computers) section below. That is why large interconnected networks are made of many small switched networks linked by routers.
