...
We can now begin to add actual content to our `ToDoItem`. Vue templates are currently only allowed a single root element — one element needs to wrap everything inside the template section (this will change when Vue 3 comes out). We'll use a [`<div>`](/en-US/docs/Web/HTML/Reference/Element/div) for that root element.
...
1. Underneath the [`<h1>`](/en-US/docs/Web/HTML/Reference/Element/Heading_Elements), create an unordered list ([`<ul>`](/en-US/docs/Web/HTML/Reference/Element/ul)) containing a single list item ([`<li>`](/en-US/docs/Web/HTML/Reference/Element/li)).
