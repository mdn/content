...
This is OK so far, but our component should return something useful! Go back to `src/App.jsx`, copy the first [`<li>`](/en-US/docs/Web/HTML/Reference/Element/li) from inside the unordered list, and paste it into `Todo.jsx` so that it reads like this:
...
- When `id`s are not unique, the functionality of [label elements](/en-US/docs/Web/HTML/Reference/Element/label) can break.
