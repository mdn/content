...
This expression represents an HTML [`<header>`](/en-US/docs/Web/HTML/Reference/Element/header) element with an [`<h1>`](/en-US/docs/Web/HTML/Reference/Element/Heading_Elements) element inside. The curly braces around `{subject}` tell the application to read the value of the `subject` constant and insert it into our `<h1>`.
...
This will ultimately render the following [`<figure>`](/en-US/docs/Web/HTML/Reference/Element/figure) element in the browser, with its structure as defined in the `AuthorCredit` component, and its content as defined in the props included on the `AuthorCredit` component call:
