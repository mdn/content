...
3. Below the `// Part 2` comment, start a `for...of()` loop to go through every item in the `products` array. Place the opening `{` before `// Part 3` so that Parts 3–6 run once for each product.
