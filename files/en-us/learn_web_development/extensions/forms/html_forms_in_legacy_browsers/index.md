...
All HTML input types are useable in all browsers, even ancient ones, because the way they degrade is highly predictable. If a browser does not know the value of the [`type`](/en-US/docs/Web/HTML/Reference/Element/input#type) attribute of an {{HTMLElement("input")}} element, it will fall back as if the value were `text`.
...
- The {{HTMLElement("input")}} element with its attribute [`type`](/en-US/docs/Web/HTML/Reference/Element/input#type) set to the values `button`, `submit`, `reset` or `image`
