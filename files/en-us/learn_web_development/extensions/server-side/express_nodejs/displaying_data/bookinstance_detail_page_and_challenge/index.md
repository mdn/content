...
{{PreviousMenu("Learn_web_development/Extensions/Server-side/Express_Nodejs/Displaying_data/Author_detail_page", "Learn_web_development/Extensions/Server-side/Express_Nodejs/Displaying_data")}}
...
Find the exported `bookInstanceDetail()` controller method and replace it with the following code.
...
Create **views/bookinstance_detail.pug** and paste in the content below.
...
> [!NOTE]
> You can use the same approach as we used for the _Book Instance List_ (adding the virtual property for the lifespan to the `Author` model and use {{jsxref("Intl")}} to format the date strings).
