...
The date value has to be set in the format `YYYY-MM-DD` because this is expected by [`<input>` elements with `type="date"`](/en-US/docs/Web/HTML/Reference/Element/input/date), however the date is not stored in this format so we have to convert it before setting the value in the control.
